...
- [ ] 扩展 `memory_graph.py` 与 Hybrid/Sharded 兼容层：新增 `TOOL_CALL`、`TOOL_RESULT`、`APPROVAL` 三类节点类型。
- [ ] 扩展 `memory_mirror.py`：将 tool action 映射为 `TOOL_CALL`，tool observation 映射为 `TOOL_RESULT`，审批请求/决策映射为 `APPROVAL`。
- [ ] 在 `_persist_web_visible_message()` 后调用 mirror，确保 Web 卡片事件同步进入 MemoryGraph。
...
- [ ] 在 `pipeline.agent_done/agent_error` 时生成 trace 并写入 MemoryGraph summary 节点 metadata。
