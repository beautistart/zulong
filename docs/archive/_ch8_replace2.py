...
print(f"Found match: {len(match.group(0))} chars")

NEW_CH8 = """# 第 8 章：记忆系统架构
...
|   +-- 知识验证机制
|
+-- ToolRAG (工具库) <-- v2.7 新增文档记录
    +-- 冷工具发现 (非 CoreToolManager 热加载工具)
    +-- 工具描述向量化索引
    +-- 按 query 语义匹配推荐工具
```

> **ToolRAG** (`zulong/memory/tool_rag.py`, 233 行) 是第 4 个 RAG 库，用于冷工具的语义发现。当 CoreToolManager 的热工具集未命中时，通过向量检索从完整工具库中推荐匹配工具。
...
512 维向量 (已归一化)
  |
FAISS.search(query_vector, top_k=5) x 4 库
  |
...
print(f"OK: {old_len} -> {len(new_content)} chars")
