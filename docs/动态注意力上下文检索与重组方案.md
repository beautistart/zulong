...
> 核心约束：**不维护 backing_pool、不建设全局上下文去重层、不全量注入全图、不把动态注意力做成压缩器**。
...
8. active skeleton 只作为后台活跃骨架，不作为 LLM 上下文整体注入。
...
2. [来源: graph_memory_id=dialogue:session_xxx/round_12] 用户要求不维护 backing_pool。
...
5. **预算裁剪**
   - 按模式、距离、激活分、任务状态、重要度、新鲜度排序。
   - 达到 `target_tokens` 停止。
   - 不维护 backing_pool，不建设全局上下文去重层。
...
| 字段 | 含义 |
|---|---|
| 当前注意力模式 | GLOBAL / SINGLE_CHAIN / FOCUS |
| 当前上下文压力 | 本轮实际注入上下文 / 阈值预算 |
| 当前焦点地址 | `tg:{graph_id}/{node_id}` 或 `graph_memory_id` |
| 思维路径 | root › parent › current |
| BFS 种子数 | 本轮从几个地址/节点开始扩散 |
| BFS 扩散节点数 | 本轮激活多少候选节点 |
| 最终注入片段数 | 实际给 LLM 的上下文片段数量 |
| 触发原因 | YELLOW / RED / LLM 主动 / 任务上浮下探 |

不展示：

- backing_pool pressure。
- 去重数量。
- evicted to backing。
- 原始工具噪声。
...
旧 `AttentionWindow.apply_window()` 后续应保留为兼容兜底，不再作为动态注意力主机制。
...
3. 注意力切换时导航地图包含当前地址、脊柱、直接子节点、迷雾、总计。
...
8. active skeleton 不被当作 backing_pool。
9. 完成前验证能指出未覆盖的 in_progress 节点地址。
10. 前端能看到模式、压力、焦点地址、BFS 数量、注入片段数量。
...
> **注意力切换就是一次新的上下文定位与检索，而不是对旧上下文池做压缩、去重或反复裁剪。**
---
...
2. 不建设全局上下文去重层。
3. 不把动态注意力写成上下文压缩器。
4. 不把 GLOBAL 解释成全图全文注入。
5. 不绕过 `memory_graph_factory` 直接创建旧 NetworkX / 单 JSON 记忆后端。
6. 不用固定普通工具名作为 GLOBAL / FOCUS / SINGLE_CHAIN 的主切换规则。
7. 不把 active skeleton 当作 LLM 上下文池或隐藏 backing pool。
8. 不每轮注入导航地图；只在真实切换、上浮、下探、压力导致切换时注入。
...
因此，旧 `AttentionWindow.apply_window()` 只保留为兼容兜底；后续主链路应变成：
...
不包含：全图所有节点全文、所有历史消息、全量 backing pool
```
...
```text
实际阈值预算 tokens = LLM 原始上下文窗口 × threshold_budget_ratio
...
5. telemetry 可以记录完整字段，包括 `threshold_budget_ratio`、`threshold_budget_tokens`、`active_window_tokens` 等。
...
4. `render_attention_context()` 统一 active context 格式。
5. 明确 `AttentionWindow.apply_window()` 降级为兼容兜底，而不是动态注意力主机制。
6. 明确不建设 `backing_pool` 和全局去重层，靠检索 + BFS 天然淘汰无关上下文。
### 18.4 启动前检查清单
...
3. 确认 `AttentionWindowManager.apply_window()` 当前仍是旧窗口兜底，不应作为新主链路。
...
| 文件 | 预期职责 |
|---|---|
| `zulong/l2/attention_window.py` | 保留兼容兜底；不再作为新主机制唯一入口 |
| `zulong/l2/pressure_detector.py` | 压力口径和提前预测策略修正 |
| `zulong/l2/attention_pressure_view.py` | 统一压力展示口径 |
| `zulong/ide/ide_fc_runner.py` | 主链路接入：计划构建、导航地图、压力阶段、工具限制、telemetry |
| `zulong/l2/inference_engine.py` | 非 IDE / Web 路径的记忆与注意力上下文注入统一 |
...
| `zulong_web/static/index.html` | 前端压力颜色、注意力 telemetry 显示对齐 90%/100% |
...
5. 压力回归：压力 89% 不触发，91% 只引导，101% 第二次仍 RED 才受限恢复。
6. 浏览器/真实服务：用 Web UI 触发一次多步任务，日志中能看到：
   - `ATTENTION_UPDATE`
   - 压力百分比
   - focus address
   - navigation map injected
   - retrieve_context 命中
   - BFS activated
   - coverage check
7. 前端颜色：压力黄/红阈值对齐 90%/100%，不再使用 50%/60% 兜底。
