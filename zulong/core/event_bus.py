...
class EventBus:
    ...
    def __init__(self):
        ...
        if not hasattr(self, '_initialized'):
            self._subscribers = {}
            self._event_queue = []  # heapq 优先级队列: (-priority_value, seq, event)
            self._event_seq = 0  # 单调递增序号，保证同优先级 FIFO
            self._running = False
            self._thread = None
    ...
    def _dispatch_loop(self):
        """事件分发循环（优先级队列：高优先级事件先处理）"""
        logger.debug("[TRACE-EB] dispatch_loop started")
        while self._running:
            if self._event_queue:
                with self._lock:
                    if self._event_queue:
                        ...
                        event = None
                if event:
                    logger.debug(f"[TRACE-EB] dispatching: {event.type}")
                    self._dispatch_event(event)
                    logger.debug(f"[TRACE-EB] dispatch done: {event.type}")
            else:
    ...
    def publish(self, event: ZulongEvent):
        ...
        Args:
            event: 事件对象
        """
        # 流式输出和思考步骤事件静默处理，不输出日志
        if event.type not in (EventType.L2_OUTPUT_STREAM, EventType.L2_THINKING_STEP):
            logger.debug(f"\n{'='*80}")
        ...
        if event.type in (EventType.L2_OUTPUT_STREAM, EventType.L2_THINKING_STEP):
            self._dispatch_event(event)
            return
        
        # 系统事件放入优先级队列，由后台线程按优先级分发
        with self._lock:
            # heapq 是最小堆，取反 priority.value 使高优先级先出队
            # seq 保证同优先级事件按 FIFO 顺序处理
            heapq.heappush(self._event_queue, (-event.priority.value, self._event_seq, event))
            self._event_seq += 1
        logger.debug(f"[TRACE-EB] enqueued: {event.type} (queue_size={len(self._event_queue)}, thread_alive={self._thread.is_alive() if self._thread else 'N/A'})")
