...
class SnapshotManager:
    ...
    def _initialize(self):
        ...
        # 磁盘持久化路径
        self._snapshot_dir = os.path.join("data", "snapshots")
    ...
    def freeze(self, task_id: str) -> Optional[TaskSnapshot]:
        ...
        with self._snapshot_lock:
            ...
            # 持久化到磁盘（防止程序崩溃丢失快照）
            self._save_to_disk(task_id, snapshot)
    ...
    def _get_snapshot_path(self, task_id: str) -> str:
        """获取快照文件路径"""
        safe_name = task_id.replace("/", "_").replace("\\", "_").replace(":", "_")
        return os.path.join(self._snapshot_dir, f"{safe_name}.json")
    
    def _save_to_disk(self, task_id: str, snapshot: TaskSnapshot):
        """将冻结的快照持久化到磁盘
        
        使用先写临时文件再原子重命名的方式，防止写入中断导致文件损坏。
        """
        file_path = self._get_snapshot_path(task_id)
        ...
        try:
            ...
            logger.info(f"Snapshot saved to disk: {file_path}")
        except Exception as e:
            logger.warning(f"Failed to save snapshot to disk: {e}")
    ...
    def _remove_from_disk(self, task_id: str):
        ...
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                logger.info(f"Snapshot removed from disk: {file_path}")
        except Exception as e:
            logger.warning(f"Failed to remove snapshot from disk: {e}")
    ...
    def _load_all_from_disk(self):
        ...
        for filename in os.listdir(self._snapshot_dir):
            if not filename.endswith(".json"):
                continue
            file_path = os.path.join(self._snapshot_dir, filename)
            try:
                ...
                loaded_count += 1
                logger.info(f"Loaded frozen snapshot from disk: {snapshot.task_id} ({snapshot.task_name})")
            except Exception as e:
                logger.warning(f"Failed to load snapshot {filename}: {e}")
        
        if loaded_count > 0:
            logger.info(f"Loaded {loaded_count} frozen snapshot(s) from disk")
