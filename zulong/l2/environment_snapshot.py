...
class EnvironmentSnapshotManager:
    ...
    def create_snapshot(self, task_id: str = None) -> EnvironmentSnapshot:
        ...
        # 基础环境状态（最小可用实现，不依赖视觉/听觉硬件）
        now = time.time()
        
        # 用户状态：根据最后交互时间判断活跃度
        try:
            from zulong.core.state_manager import state_manager
            last_input_time = getattr(state_manager, '_last_input_time', now)
        ...
        snapshot.set_user(UserState(
            user_id="user_001",
            activity=activity,
            last_interaction=now - idle_seconds,
        ))
        
        # 场景描述：记录时间和基础系统状态
        from datetime import datetime
        time_str = datetime.fromtimestamp(now).strftime("%Y-%m-%d %H:%M:%S")
        snapshot.scene_description = f"快照时间: {time_str}, 用户状态: {activity}"
