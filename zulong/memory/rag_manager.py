...
# RAG 管理器 - 统一管理四个 RAG 库（经验/记忆/知识/工具）
...
class RAGManager:
    ...
    def __init__(self, config: Optional[RAGConfig] = None):
        ...
        if self.config.tool_rag_enabled:
            self.rag_libraries["tool"] = ToolRAG(
                dimension=self.config.vector_dimension,
                vector_store_type=self.config.vector_store_type
            )
            logger.info("[RAGManager] Tool RAG initialized")
    ...
    def get_tool_rag(self) -> Optional['ToolRAG']:
        """获取 ToolRAG 实例（供 CoreToolManager / SearchToolsTool 使用）"""
        return self.rag_libraries.get("tool")
