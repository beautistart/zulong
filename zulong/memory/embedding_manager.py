...
class EmbeddingModelManager:
    ...
    def load_model(self):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"[EmbeddingModelManager] 依赖缺失：sentence-transformers 未安装")
                logger.error("[EmbeddingModelManager] 请运行：pip install sentence-transformers")
                return False
            ...
            logger.info(f"[EmbeddingModelManager] 加载模型：{self.model_name} (cache={cache_path})")
    ...
    def _mock_encode(self, texts: Union[str, List[str]]) -> np.ndarray:
        ...
        使用 TF-IDF 生成有意义的向量，而不是随机向量。
        
        Args:
            texts: 输入文本
            
        Returns:
            np.ndarray: TF-IDF 向量表示
        """
        if isinstance(texts, str):
            texts = [texts]
        ...
        try:
            ...
            logger.info("[EmbeddingModelManager] 使用 TF-IDF 降级方案（比随机向量优）")
            
            # 创建 TF-IDF 向量器
            vectorizer = TfidfVectorizer(
                max_features=512,  # 与 BGE 维度一致
            ...
            # 如果维度不足 512，填充到 512
            if embeddings_dense.shape[1] < 512:
                padding = np.zeros((len(texts), 512 - embeddings_dense.shape[1]), dtype=np.float32)
                embeddings_dense = np.hstack([embeddings_dense, padding])
            
            # 归一化
            embeddings_dense = embeddings_dense / (np.linalg.norm(embeddings_dense, axis=1, keepdims=True) + 1e-8)
            
            logger.info(f"[EmbeddingModelManager] TF-IDF 编码完成：{len(texts)} 个文本，维度={embeddings_dense.shape}")
            
            return embeddings_dense
            
        except ImportError:
            # TF-IDF 也不可用，降级到随机向量
            logger.warning("[EmbeddingModelManager] sklearn 不可用，降级到随机向量")
