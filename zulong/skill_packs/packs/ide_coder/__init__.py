# File: zulong/skill_packs/packs/ide_coder/__init__.py
"""
IDE 编程技能包（基础版）
...
class FileReadTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.description = (
            "读取指定文件的内容并返回文本。当需要查看代码文件、配置文件、日志文件等内容时使用。"
            "支持限制读取行数以避免输出过大。返回文件内容、总行数和文件大小。"
        )
...
class FileWriteTool(BaseTool):
    ...
    def __init__(self, workspace: str = "."):
        ...
        self.description = (
            "将内容写入文件（创建新文件或覆盖/追加已有文件）。当需要生成代码文件、保存输出结果、创建配置文件时使用。"
            "支持覆盖写入和追加写入两种模式。文件路径必须在工作目录内（安全限制）。"
        )
...
class CodeEditTool(BaseTool):
    ...
    def __init__(self, workspace: str = "."):
        ...
        self.description = (
            "精确替换代码文件中的指定片段，实现局部修改而非整文件覆盖。当需要修复 bug、重构代码、"
            "添加功能到现有文件时使用。需提供要替换的原始代码和新代码，确保精确匹配。"
        )
        self.workspace = Path(workspace).resolve()
...
class TerminalTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.description = (
            "在安全沙箱中执行终端命令。当需要运行 Python 脚本、安装依赖包、执行测试、"
            "使用 git 操作等场景时使用。仅允许白名单内的命令（python/pip/git/node/npm 等），"
            "有超时保护，返回 stdout/stderr 和退出码。"
        )
...
class CodeSearchTool(BaseTool):
    ...
    def __init__(self, workspace: str = "."):
        ...
        self.description = (
            "在代码仓库中搜索包含指定文本的文件和代码行。当需要查找函数定义、变量引用、"
            "特定代码模式时使用。支持按文件类型过滤（如仅搜索 *.py），返回匹配文件列表和具体行号。"
        )
        self.workspace = Path(workspace).resolve()
...
class IDECoderPack(ISkillPack):
    ...
    def __init__(self):
        ...
        self._workspace = "."
        self._tools = []
        logger.info("[IDECoderPack] Initialized")
    ...
    def get_manifest(self) -> SkillPackManifest:
        ...
        if self._manifest is None:
            self._manifest = SkillPackManifest(
                ...
                name="IDE编程",
                version="1.0.0",
                description="基于 IDE 集成层的编程助手能力，支持文件操作、代码编辑、命令执行",
        ...
        try:
            ...
            for tool in self._tools:
                try:
                    ...
                    logger.warning(f"[IDECoderPack] 工具注册失败: {e}")
            
            logger.info(f"[IDECoderPack] 已注册 {len(self._tools)} 个工具")
            return True
        except Exception as e:
            logger.error(f"[IDECoderPack] Install failed: {e}", exc_info=True)
            return False
    ...
    def uninstall(self) -> bool:
        """卸载技能包"""
        self._tools = []
        logger.info("[IDECoderPack] Uninstalled")
        return True
