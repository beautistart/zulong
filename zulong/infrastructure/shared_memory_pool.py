...
class SharedMemoryPool:
        ...
        if cls._instance is None:
            async with cls._lock:
                if cls._instance is None:
                    ...
                    # 🔥 启动后台任务（保存引用，便于关机时取消）
                    t1 = asyncio.create_task(cls._instance._cleanup_loop())
                    t2 = asyncio.create_task(cls._instance._snapshot_loop())
                    cls._instance._background_task_refs = [t1, t2]
    ...
    def __init__(self):
        ...
        self._background_task_refs: List[asyncio.Task] = []  # 后台任务引用，用于关机时取消
    ...
    async def start_background_tasks(self):
        ...
        # 创建后台任务（保存引用，便于关机时取消）
        t1 = asyncio.create_task(self._cleanup_loop())
        t2 = asyncio.create_task(self._snapshot_loop())
        self._background_task_refs = [t1, t2]
    ...
    async def get_recent(self, time_window_sec: Optional[float] = None, 
        ...
        if zone:
            ...
            # 🔥 未指定分区时，使用时间索引（向后兼容）
            # 🔥 [BUG-11 修复] 先在锁内收集 trace_id，再在锁外读取数据
            # 避免在 _index_lock 内 await self.read() 导致死锁
            candidate_trace_ids = []
            with self._index_lock:
                # 从时间索引查找（移除 [-100:] 限制）
                for timestamp, trace_id, env_zone in reversed(self._time_index):
                    if limit and len(candidate_trace_ids) >= limit:
                        break
    ...
    async def shutdown(self):
        ...
        # 🔥 主动取消后台任务，避免 "Task was destroyed but it is pending!" 警告
        for task in self._background_task_refs:
            if not task.done():
                task.cancel()
        # 等待任务实际结束（忽略 CancelledError）
        for task in self._background_task_refs:
            try:
                await task
            except asyncio.CancelledError:
                pass
        self._background_task_refs.clear()
