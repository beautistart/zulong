...
# ================================================================================
# 导入配置管理器
from zulong.config.config_manager import init_config, get_config, get_llm_config, get_l2_inference_config
...
logger_info.append(f"✅ [CONFIG] 配置系统已初始化")
...
# 输出配置初始化信息
for info_msg in logger_info:
    logger.info(info_msg)

# 输出 LLM 配置信息
llm_config = get_llm_config()
logger.info(f"🤖 [CONFIG] LLM 后端：{llm_config['backend']}")
logger.info(f"   API 地址：{llm_config['base_url']}")
logger.info(f"   模型 ID: {llm_config['model_id']}")

# 输出 L2 推理配置
l2_config = get_l2_inference_config()
logger.info(f"🧠 [CONFIG] L2 核心模型：{l2_config['core_model']}")
logger.info(f"🧠 [CONFIG] L2 备用模型：{l2_config['backup_model']}")

# 🔥 关键修复：在导入任何依赖共享池的模块之前，先初始化共享池单例
# 这样确保 MemoryGraph、DataIngestion 等都能使用同一个实例
...
class SystemBootstrap:
    ...
    def initialize(self):
        ...
        try:
            ...
            # 修复：注入真实的 ExperienceStore 单例（而非 None）
            experience_store = EnhancedExperienceStore()
            
            # 初始化 CoreToolManager + ToolRAG 联动
            core_tool_manager = None
            try:
                from zulong.tools.core_tool_manager import CoreToolManager
                
                # 从 RAGManager 获取 ToolRAG 实例
                tool_rag = None
                rag_manager = getattr(inference_engine, 'rag_manager', None)
                ...
                # 将 ToolRAG 连接到 search_tools 元工具
                search_tools_tool = inference_engine.tool_engine.registry.get("search_tools")
                if search_tools_tool and tool_rag:
                    search_tools_tool.set_tool_rag(tool_rag)
                    logger.info("✅ [BOOTSTRAP] SearchToolsTool 已连接到 ToolRAG")
                
                logger.info(f"✅ [BOOTSTRAP] CoreToolManager 已初始化 (ToolRAG={'有' if tool_rag else '无'})")
            except Exception as e:
                logger.warning(f"⚠️ [BOOTSTRAP] CoreToolManager 初始化失败（非致命）: {e}")
        ...
        try:
            ...
            logger.info("✅ [BOOTSTRAP] RecoveryNotifier 检查完成")
        except Exception as e:
    ...
    def stop(self):
        ...
        # 挂起活跃任务（在保存持久化数据之前，确保焦点上下文写入 MemoryGraph）
        logger.info("  检查活跃任务...")
        try:
            from zulong.l2.task_state_manager import task_state_manager
            ...
            if active_task:
                ...
                logger.info(f"  已保存活跃任务检查点: {active_task}")
            else:
                logger.info("  无活跃任务")
        except Exception as e:
            logger.warning(f"  挂起活跃任务失败: {e}")
        ...
        try:
            ...
            if pool_exists:
                ...
                logger.info("   ⚠️ 没有需要保存的 SharedMemoryPool 实例")
...
def main():
    ...
    # Windows: SIGBREAK (Ctrl+Break / 关闭控制台窗口)
