...
class IDEPromptHandler:
    ...
    def _build_zulong_prompt_unified(
        ...
        parts.append(
            ...
            "      ╚══════════════════════════════════════════════════════════╝\n"
            "      - 写代码/创建文件 → 必须调用: write_to_file(path='文件绝对路径（工作目录+文件名）', content='本片内容', mode='overwrite'|'append')\n"
            "        长文件必须按 800-1200 字符分片：第一片 mode='overwrite'，后续 mode='append'，不要一次性传完整长代码。\n"
            "        示例: write_to_file(path='<工作目录>/index.html', content='...', mode='overwrite')\n"
            ...
            "- ⚠️ 【最重要】写代码/创建文件时必须调用 write_to_file！口头叙述不算执行；长文件必须分片 append！\n"
