...
class AsyncL1BScheduler:
    ...
    def __init__(self):
        ...
        if not hasattr(self, '_initialized'):
            ...
            self.short_term_memory = None  # 已废弃，保留属性避免其他引用报错
    ...
    def initialize(self):
        ...
        self.short_term_memory = None  # 已废弃，由 MemoryGraph 统一替代
        self._initialized = True
        logger.info("[AsyncL1BScheduler] 组件初始化完成 (LibraryManager, TaggingEngine, HotStorage)")
...
5. 立即开始回答，不要说"好的"、"让我想想"等废话""")
    ...
    def finalize_task(self, task: TaskItem, ai_response: str):
        ...
        try:
            ...
            # 3. 写入 MemoryGraph（统一记忆写入，替代旧 ShortTermMemory）
            try:
                ...
                if mg:
                    ...
                    round_id = adapter.add_round(mg, task.task_id, user_input, session_id=session_id)
                    adapter.add_sub_dialogue(mg, round_id, turn=1, content=ai_response, role="assistant")
                    logger.debug(f"[AsyncL1BScheduler] ✅ 对话已写入 MemoryGraph: {round_id}")
            except Exception as e:
                logger.warning(f"[AsyncL1BScheduler] MemoryGraph 写入失败: {e}")
    ...
    def _restore_conversation_history(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # 2. 优先从 MemoryGraph 热窗口恢复
            try:
                from zulong.memory.memory_graph import get_memory_graph, NodeType
                mg = get_memory_graph()
                if mg:
                    # 遍历 DIALOGUE 类型热节点，按 last_accessed 倒序取最近 limit 个
                    dialogue_nodes = [
                        n for n in mg.get_nodes_by_type(NodeType.DIALOGUE)
                        if mg.is_recent(n.node_id, window_seconds=1800)
                    ]
                    dialogue_nodes.sort(key=lambda n: n.last_accessed, reverse=True)
                    hot_nodes = dialogue_nodes[:limit]
                    if hot_nodes:
                        recent_messages = []
                        for node in hot_nodes:
                            user_text = node.metadata.get("user_text", "")
                            bot_text = node.metadata.get("bot_text", "")
                            if user_text:
                                recent_messages.append({"role": "user", "content": user_text})
                            if bot_text:
                                recent_messages.append({"role": "assistant", "content": bot_text})
                        if recent_messages:
                            logger.info(f"[AsyncL1BScheduler] 从 MemoryGraph 恢复 {len(recent_messages)} 条历史对话")
                            return recent_messages
            except Exception as e:
                logger.warning(f"[AsyncL1BScheduler] MemoryGraph 读取失败: {e}")
            
            # 3. 回退到 HotStorage（如果 MemoryGraph 为空）
            logger.debug("[AsyncL1BScheduler] MemoryGraph 无热数据，尝试从 HotStorage 恢复...")
