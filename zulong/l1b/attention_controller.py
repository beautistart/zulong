...
class AttentionController:
    ...
    def _handle_interrupt(self, evt: AttentionEvent):
        ...
        # 1. 冻结 (Freeze) — 必须成功后再改状态
        if self.status == "BUSY" and self.current_task_id:
            # 请求 L2 生成摘要并保存状态
            try:
                snapshot = self._create_l2_snapshot()
                if snapshot is not None:
                    ...
                    logger.warning(
                        "[AttentionController] 冻结快照返回 None，"
                        "无活跃任务可冻结，将继续处理中断但不保存恢复点"
                    )
                    self.active_snapshot = None
            except Exception as e:
                logger.error(f"[AttentionController] 冻结快照异常: {e}")
                self.active_snapshot = None
    ...
    def _load_l2_snapshot(self, snapshot: ContextSnapshot):
        ...
        # 恢复快照（从 kv_cache_ptr 安全获取 token 数量）
        kv_cache_tokens = 0
        if snapshot.kv_cache_ptr is not None:
            if hasattr(snapshot.kv_cache_ptr, 'num_tokens'):
                kv_cache_tokens = snapshot.kv_cache_ptr.num_tokens
            elif isinstance(snapshot.kv_cache_ptr, dict):
                kv_cache_tokens = snapshot.kv_cache_ptr.get('num_tokens', 0)
