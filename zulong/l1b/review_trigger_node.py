...
class ReviewTriggerNode:
    ...
    def __init__(self, review_trigger=None):
        ...
        self._l2_output_subscribed = False
        logger.info("[ReviewTriggerNode] 初始化完成 - 🔥 修复版本")
    
    def _ensure_l2_output_subscription(self):
        """确保已订阅 L2_OUTPUT 事件（用于补录 L2 回复到缓冲区）"""
        if self._l2_output_subscribed:
            return
        try:
            from zulong.core.event_bus import event_bus
            from zulong.core.types import EventType
            event_bus.subscribe(EventType.L2_OUTPUT, self._on_l2_output, "ReviewTriggerNode_L2Recorder")
            self._l2_output_subscribed = True
            logger.info("[ReviewTriggerNode] 已订阅 L2_OUTPUT 事件（用于补录回复到缓冲区）")
        except Exception as e:
            logger.warning(f"[ReviewTriggerNode] 订阅 L2_OUTPUT 失败：{e}")
    
    def _on_l2_output(self, event):
        """L2 回复事件回调 — 将 L2 回复补录到复盘缓冲区
        ...
        try:
            ...
            review_stage = state_manager.get_context('review_stage', None)
            
            if not review_mode or review_stage != 'review_active':
                return
            
            # 获取 L2 回复文本
            payload = event.payload if hasattr(event, 'payload') else event
            l2_text = payload.get('text', '')
            source = payload.get('source', '') if isinstance(payload, dict) else ''
            
            # 跳过系统内部消息（如复盘提示本身）
            if source == 'ReplayIntegration' or source == 'ReviewTriggerNode':
                return
            
            if not l2_text or len(l2_text.strip()) == 0:
                return
            ...
            if buffer_manager.has_buffer():
                buffer_manager.add_system_response({
                    'role': 'assistant',
                    'content': l2_text,
                    'timestamp': datetime.utcnow().isoformat(),
                    'source': 'l2_output_recorder'
                })
                logger.debug(f"[ReviewTriggerNode] 已补录 L2 回复到缓冲区 ({len(l2_text)} 字)")
                
        except Exception as e:
            logger.debug(f"[ReviewTriggerNode] L2 回复补录失败（非关键）：{e}")
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # 确保 L2 回复补录订阅已激活
            self._ensure_l2_output_subscription()
            # 获取用户输入
