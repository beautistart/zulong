...
class ComplexTaskContext:
    ...
    status: str = "PENDING"                   # PENDING, EXECUTING, COMPLETED, FAILED, SUSPENDED
    progress: float = 0.0                     # 进度 0.0-1.0
    created_at: float = field(default_factory=time.time)
    completed_at: Optional[float] = None
    suspended_subtasks: Dict[str, str] = field(default_factory=dict)  # 挂起的子任务 {subtask_id: suspended_task_id}
...
class AsyncL1BSchedulerWithSharedPool:
    ...
    async def _execute_single_subtask(
        ...
        try:
            ...
            # 检查是否为挂起状态（L2 返回挂起通知）
            if isinstance(result, str) and "任务已挂起" in result:
                subtask["status"] = "SUSPENDED"
                # 提取挂起 ID
                import re
                id_match = re.search(r'ID:\s*(\S+)', result)
                if id_match:
                    ctx.suspended_subtasks[subtask_id] = id_match.group(1).rstrip('）')
                logger.info(f"[AsyncL1BSchedulerWithSharedPool] ⏸️ 子任务 {subtask_id} 已挂起")
                return {
                    "id": subtask_id,
                    "status": "SUSPENDED",
