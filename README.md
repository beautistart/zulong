...
ZULONG（祖龙）是一个事件驱动的多模态机器人操作系统。

它不是一个套壳聊天机器人。它有眼睛（摄像头实时视觉感知）、有耳朵（麦克风语音活动检测+情感识别）、有嘴（CosyVoice 语音合成）、有四肢（电机直连控制），以及一个分层的大脑：

- **反射脑（L1-A）** -- 有人突然靠近？毫秒级直接控制电机刹停，不等大模型思考。和人类的膝跳反射一样，紧急事件不过大脑皮层。
- **调度脑（L1-B）** -- 正在讲故事时用户突然说"停"。当前任务连同完整上下文被冻结压栈，新任务插入处理，处理完后从中断点精确恢复，不丢一个字。
- **视觉注意力（L1-C）** -- 安静时持续观察环境。用 MobileNetV4-TSM 双流架构识别用户动作意图（挥手、靠近、注视），不需要骨骼关键点数据，直接吃 RGB 帧。
- **推理脑（L2）** -- 需要深度思考时调用大模型 + 三层 RAG 记忆检索（经验库/交互记忆库/知识库），根据查询类型智能路由到特定库，避免全库搜索。
- **技能手（L3）** -- 三个冷备模型槽位支持热插拔，LRU 驱逐策略，GPU → RAM → 磁盘三层缓存，切换模型不中断对话。

82,000 行 Python，230 个模块，从传感器到执行器的完整链路。
