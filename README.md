...
## 🎯 核心故事 | The Story Behind Zulong

### 一个室内设计师，2个月，独立开发82,000+行代码

**我是一位室内设计师**，没有技术背景、没有团队、没有融资。

但我相信，AI 助手应该拥有**真正的记忆**，而不是每次对话都从零开始。

所以我用**设计空间的方式设计了 AI 的记忆系统** — 把记忆当作空间关系来设计 — 独立开发出了这个在以下方面**超越大厂**的多模态 AI Agent 系统：

- ✅ **异构图记忆系统**（9种节点 + 7种边 + 赫布学习 + 艾宾浩斯遗忘）
- ✅ **6信号死循环检测**（业界最完善）
- ✅ **跨天级任务恢复**（完整状态序列化）

> "这不是一个产品，而是一个普通人对抗技术垄断的故事。"
...
>
> 这是祖龙的首次正式发布，包含完整的异构记忆图谱、死循环检测、跨天级任务恢复等核心能力。
>
> **核心更新**：
> - ✅ **MemoryGraph 异构记忆图谱**（市场独有）- 9节点+7边+赫布学习+艾宾浩斯衰减
> - ✅ **CircuitBreaker 6信号检测**（业界最完善）- 信息增益检测等6种信号
> - ✅ **跨天级任务恢复**（无竞品）- 完整状态序列化
> - ✅ **5层防护链**（业界最精细）- 适配4B模型
> - ✅ **完整 VS Code 扩展** + **TTS/ASR语音交互**
>
...
**祖龙是一个具有异构记忆图谱、赫布学习、艾宾浩斯衰减、注意力控制和跨天级任务恢复的 AI Agent 认知系统**，在消费级硬件（GPU 6GB 显存）上即可运行完整的记忆+推理+多模态能力。
...
#### 1. 无限上下文
祖龙通过**三级记忆架构**（热/温/冷），让 AI 突破模型上下文窗口限制。即使经过数月、跨年级的对话，它依然能想起你曾经说过的话。

#### 2. 记忆关联发现
祖龙会**自动发现记忆之间的关联**，模拟人脑的"联想"能力。当你提到"上次那个项目"时，它会自动联想到相关的背景、人员和细节。

#### 3. 跨年级的完整记忆
不同于其他 AI 的"摘要式"记忆，祖龙保存的是**完整的对话、任务、经验细节**。一年后你问"去年那个装修项目用的什么颜色方案？"，它能给出具体答案。

---

## ✨ 核心亮点

### 1. 🔮 异构记忆图谱 (MemoryGraph) - **市场独有**
...
- Node.js 18+
- VS Code
- GPU: NVIDIA RTX 3060 6GB 或更高（可纯 CPU 运行）
...
|------|------|---------|--------|
| **记忆检索（热路径）** | <50ms | ~200MB | - |
| **记忆检索（冷路径）** | <200ms | ~500MB | - |
| **ASR (SenseVoice)** | 0.5-1s | ~240MB | >95% (中文) |
| **TTS (Kokoro)** | <0.3s | ~300MB | 自然度高 |
| **整体端到端** | 3-4s | - | - |
...
|------|------|-----------|--------|--------------|---------|
| **异构记忆图谱** | ✅ LMDB + GraphML | ❌ 内存 DAG | ❌ | ❌ 单路向量 | ❌ 基于文件 |
| **赫布学习** | ✅ 共激活增强 | ❌ | ❌ | ❌ | ❌ |
| **艾宾浩斯衰减** | ✅ exp 衰减 | ❌ | ❌ | ❌ | ❌ 基于年龄 |
...
### vs UI-TARS-desktop（字节跳动 GUI Agent）

| 维度 | 祖龙 | UI-TARS | 评判 |
|------|:---:|:---:|------|
| **GUI 自动化控制** | 2/10 | **10/10** | UI-TARS 领先 |
| **多模态记忆系统** | **9/10** | 1/10 | 祖龙 领先 |
| **长期对话记忆** | **9/10** | 0/10 | 祖龙 领先 |
| **任务规划与分解** | **9/10** | 5/10 | 祖龙 领先 |
| **死循环防护** | **10/10** | 3/10 | 祖龙 遥遥领先 |
| **语音交互 (ASR/TTS)** | **9/10** | 0/10 | 祖龙 领先 |
| **MCP 工具集成** | 6/10 | **9/10** | UI-TARS 领先 |
| **生态与社区** | 1/10 | **8/10** | UI-TARS 领先 |

**结论**：祖龙与 UI-TARS **定位完全不同** — 祖龙专注记忆与对话，UI-TARS 专注 GUI 控制。两者可**互补整合**（祖龙作为认知大脑 + UI-TARS 作为 GUI 执行专家）。
...
│   │   └── attention_window.py
│   ├── memory/                 # 记忆系统
│   │   ├── memory_graph.py     # 异构记忆图谱 (LMDB + GraphML)
│   │   ├── rag_manager.py
│   │   └── episodic_memory.py
│   ├── l1a/                    # L1-A 音频融合层
│   ├── l1b/                    # L1-B 事件调度层
