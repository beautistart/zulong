/**
 * AcpAgent - Thin wrapper that bridges stdio connection to ZulongAgent.
 *
 * This class wraps the ZulongAgent and connects it to an ACP AgentSideConnection
 * for stdio-based communication. It:
 * - Wires up the permission handler to call connection.requestPermission()
 * - Subscribes to ZulongAgent session events and forwards them to connection.sessionUpdate()
 * - Delegates all acp.Agent methods to the internal ZulongAgent
 *
 * For programmatic usage without stdio, use ZulongAgent directly.
 *
 * @module acp
 */
...
import { ZulongAgent } from "../agent/ZulongAgent.js"
import { type AcpAgentOptions, type SessionUpdateType } from "../agent/types.js"

/**
 * ACP Agent wrapper that bridges stdio connection to ZulongAgent.
 *
 * This is the class used by runAcpMode() for stdio-based ACP communication.
 * It creates an internal ZulongAgent and wires up the connection for:
...
export class AcpAgent implements acp.Agent {
	...
	private readonly zulongAgent: ZulongAgent
	...
	constructor(connection: acp.AgentSideConnection, options: AcpAgentOptions) {
		...
		// Create the internal ZulongAgent
		this.zulongAgent = new ZulongAgent(options)

		// Wire up the permission handler to use the connection
		this.zulongAgent.setPermissionHandler(async (request) => {
			try {
	...
	private subscribeToSessionEvents(sessionId: string): void {
		...
		const emitter = this.zulongAgent.emitterForSession(sessionId)
	...
	// ============================================================
	// acp.Agent Interface Implementation - Delegate to ZulongAgent
	// ============================================================

	async initialize(params: acp.InitializeRequest): Promise<acp.InitializeResponse> {
		return await this.zulongAgent.initialize(params, this.connection)
	}

	async newSession(params: acp.NewSessionRequest): Promise<acp.NewSessionResponse> {
		const response = await this.zulongAgent.newSession(params)
	...
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		return this.zulongAgent.prompt(params)
	}

	async cancel(params: acp.CancelNotification): Promise<void> {
		return this.zulongAgent.cancel(params)
	}

	async setSessionMode(params: acp.SetSessionModeRequest): Promise<acp.SetSessionModeResponse> {
		return this.zulongAgent.setSessionMode(params)
	}

	async unstable_setSessionModel(params: acp.SetSessionModelRequest): Promise<acp.SetSessionModelResponse> {
		return this.zulongAgent.unstable_setSessionModel(params)
	}

	async authenticate(params: acp.AuthenticateRequest): Promise<acp.AuthenticateResponse> {
		return this.zulongAgent.authenticate(params)
	}

	async shutdown(): Promise<void> {
		this.subscribedSessions.clear()
		return this.zulongAgent.shutdown()
	}
}
