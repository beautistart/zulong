 ...
 * and runs Zulong as an ACP-compliant agent communicating over stdio.
 *
 * This module exports:
 * - `ZulongAgent` - Decoupled agent for programmatic use (no stdio dependency)
 * - `AcpAgent` - Thin wrapper that bridges stdio connection to ZulongAgent
 * - `ZulongSessionEmitter` - Typed EventEmitter for per-session events
 * - `runAcpMode` - Function to run Zulong in stdio-based ACP mode
 *
 * @module acp
 */
...
export interface AcpModeOptions {
	/** Path to Zulong configuration directory */
	config?: string
