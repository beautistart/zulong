/**
 * Zulong CLI - TypeScript implementation with React Ink
 */
...
import { ZULONG_CLI_DIR, getCliBinaryPath } from "./utils/path"
...
async function runTaskInPlainTextMode(
	...
	if (!hasAuth) {
		printWarning("Not authenticated. Please run 'zulong auth' first to configure your API credentials.")
...
function setupSignalHandlers() {
	...
	// These occur when task is cancelled and async operations throw "Zulong instance aborted"
...
async function initializeCli(options: InitOptions): Promise<CliContext> {
	...
	const { extensionContext, storageContext, DATA_DIR, EXTENSION_DIR } = initializeCliContext({
		zulongDir: options.config,
		workspaceDir: workspacePath,
	})

	// Set up output channel and Logger early so ZulongEndpoint.initialize logs are captured
	const outputChannel = window.createOutputChannel("Zulong CLI")
	...
	await ZulongEndpoint.initialize(EXTENSION_DIR)
	...
	outputChannel.appendLine(
		`Zulong CLI initialized. Data dir: ${DATA_DIR}, Extension dir: ${EXTENSION_DIR}, Log dir: ${ZULONG_CLI_DIR.log}`,
	)
	...
	await telemetryService.captureHostEvent("zulong_cli", "initialized")
...
async function runTask(prompt: string, options: TaskOptions & { images?: string[] }, existingContext?: CliContext) {
	...
	// If prompt provided (zulong task "prompt"), ChatView will auto-submit
	// If no prompt (zulong interactive), user will type it in
	let taskError = false
...
program.name("zulong").description("Zulong CLI - AI coding assistant in your terminal").version(CLI_VERSION)
...
program
	...
	.option("--config <path>", "Path to Zulong configuration directory")
...
mcpCommand
	.command("add")
	.description("Add an MCP server shortcut to zulong_mcp_settings.json")
...
program
	.command("version")
	.description("Show Zulong CLI version number")
	.action(() => printInfo(`Zulong CLI version: ${CLI_VERSION}`))
...
async function resumeTask(taskId: string, options: TaskOptions & { initialPrompt?: string }, existingContext?: CliContext) {
	...
	if (!historyItem) {
		...
		printInfo("Use 'zulong history' to see available tasks.")
		await disposeCliContext(ctx)
		exit(1)
	}
...
async function continueTask(options: TaskOptions) {
	...
	if (!historyItem) {
		...
		printInfo("Start a new task or use 'zulong history' to browse previous tasks.")
		await disposeCliContext(ctx)
		exit(1)
	}
...
program
	...
	.action(async (prompt, options) => {
		...
		// This handles:
		// - `echo "" | zulong` -> error (empty stdin, no prompt)
		// - `zulong "prompt"` in GitHub Actions -> OK (empty stdin ignored, has prompt)
		// - `cat file | zulong "explain"` -> OK (has stdin AND prompt)
