...
import type { ZulongMemento } from "@/shared/storage/ZulongStorage"
...
/**
 * Memento adapter that wraps a ZulongFileStorage with optional key overrides.
 * Used for globalState where CLI needs to inject hardcoded overrides.
 */
class MementoAdapter implements ZulongMemento {
	constructor(
		private readonly store: ZulongMemento,
...
export interface CliContextResult {
	extensionContext: ZulongExtensionContext
 ...
 * and wraps it in a ZulongExtensionContext shell for legacy APIs that still
 * expect the VSCode ExtensionContext shape.
 */
export function initializeCliContext(config: CliContextConfig = {}): CliContextResult {
	const ZULONG_DIR = config.zulongDir || process.env.ZULONG_DIR || path.join(os.homedir(), ".zulong")

	// Create the shared StorageContext — this owns all ZulongFileStorage instances.
	// CLI, JetBrains, and VSCode all share this same file-backed implementation.
	let storageContext = createStorageContext({
		zulongDir: ZULONG_DIR,
	...
	const extension: ZulongExtensionContext["extension"] = {
	...
	// Build the ZulongExtensionContext shell. All storage delegates to storageContext —
	// there are NO separate ZulongFileStorage instances here.
	const extensionContext: ZulongExtensionContext = {
		extension: extension,
