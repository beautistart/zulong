...
function getPromptType(ask: ZulongAsk, text: string): PromptType {
	switch (ask) {
		case "followup": {
...
export const AskPrompt: React.FC<AskPromptProps> = ({ onRespond }) => {
	...
	useInput(
		(input, key) => {
			...
			const ask = lastAskMessage.ask as ZulongAsk
 ...
 */
function getCliMessagePrefixIcon(message: ZulongMessage): string {
	if (message.type === "ask") {
		switch (message.ask) {
			case "followup":
				return "❓"
