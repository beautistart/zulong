...
import { RuleScope } from "@shared/proto/zulong/file"
...
export const ConfigViewWrapper: React.FC<ConfigViewWrapperProps> = ({
	...
	// Rules state
	const [globalZulongRulesToggles, setGlobalZulongRulesToggles] = useState<Record<string, boolean>>({})
	const [localZulongRulesToggles, setLocalZulongRulesToggles] = useState<Record<string, boolean>>({})
	...
	useEffect(() => {
		const loadData = async () => {
			...
			setGlobalZulongRulesToggles(rulesData.globalZulongRulesToggles?.toggles || {})
			setLocalZulongRulesToggles(rulesData.localZulongRulesToggles?.toggles || {})
	...
	const handleToggleRule = useCallback(
		async (isGlobal: boolean, rulePath: string, enabled: boolean, ruleType: string) => {
			const { toggleZulongRule } = await import("@/core/controller/file/toggleZulongRule")

			// Determine scope based on isGlobal and rule type
			const scope = isGlobal ? RuleScope.GLOBAL : RuleScope.LOCAL

			// For non-zulong rules, we need different toggle functions
			if (ruleType === "cursor") {
				...
				// Zulong rules
				const result = await toggleZulongRule(controller, { metadata: undefined, rulePath, enabled, scope })
				if (result.globalZulongRulesToggles?.toggles) {
					setGlobalZulongRulesToggles(result.globalZulongRulesToggles.toggles)
				}
				if (result.localZulongRulesToggles?.toggles) {
					setLocalZulongRulesToggles(result.localZulongRulesToggles.toggles)
				}
			}
		},
		[controller],
	...
	const handleOpenFolder = useCallback(
		async (folderType: "rules" | "workflows" | "hooks" | "skills", isGlobal: boolean) => {
			...
			if (isGlobal) {
				// Global folders are in dataDir (e.g., ~/.zulong/)
				...
				// Local rules/workflows/hooks/skills are in .zulongrules or .zulong
				const subFolder = folderType === "rules" ? "rules" : folderType
				folderPath = path.join(primaryWorkspace, ".zulongrules", subFolder)
			}
