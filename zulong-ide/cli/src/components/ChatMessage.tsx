...
import type { ZulongAskUseMcpServer, ZulongMessage } from "@shared/ExtensionMessage"
...
/**
 * Render a tool call in webview style: "Zulong wants to read this file:" / "Zulong read this file:"
 */
const ToolCallText: React.FC<{
	...
	return (
		<Text>
			<Text color={toolColor}>Zulong {actionText}</Text>
			{mainArg && (
				<Text>
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, mode, isStreaming }) => {
	...
	if ((type === "ask" && ask === "command") || say === "command") {
		...
		const label = isAsk ? "Zulong wants to execute this command: " : "Zulong executed this command: "
	...
	if ((type === "ask" && ask === "use_mcp_server") || say === "use_mcp_server") {
		const isAsk = type === "ask"
		const parsed = text
			? jsonParseSafe<Partial<ZulongAskUseMcpServer> & { serverName: string }>(text, {
					type: undefined,
		...
		const actionLabel = isAsk ? "Zulong wants to use MCP" : "Zulong used MCP"
		const targetLine =
	...
	// Error messages
	if (say === "zulongignore_error") {
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color="red">
					<Text color="red" wrap="wrap">
						Zulong tried to access <Text bold>{text}</Text> which is blocked by the .zulongignore file.
					</Text>
				</DotRow>
			</Box>
		)
	...
	if (say === "error" || (type === "ask" && ask === "api_req_failed")) {
		...
		// Check for Zulong auth error to show sign-in instructions
		const isZulongAuthError = errorMessage.includes(ZULONG_ACCOUNT_AUTH_ERROR_MESSAGE)
	...
	if (say === "browser_action" || say === "browser_action_launch") {
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={toolColor} flashing={partial === true && isStreaming}>
					<Text>
						<Text color={toolColor}>Zulong used the browser</Text>
						{text && (
							<Text>
								<Text color={toolColor}>: </Text>
	...
	if (say === "mcp_server_request_started") {
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={toolColor} flashing={partial === true && isStreaming}>
					<Text>
						<Text color={toolColor}>Zulong is using an MCP tool</Text>
						{text && (
							<Text>
								<Text color={toolColor}>: </Text>
								<Text>{truncate(text, 50)}</Text>
			...
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={COLORS.primaryBlue}>
					<Text bold color={COLORS.primaryBlue}>
						Zulong wants to start a new task:
					</Text>
				</DotRow>
				<Box flexDirection="column" paddingLeft={2}>
					<Text color="gray">{text}</Text>
		...
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={COLORS.primaryBlue} flashing={partial === true && isStreaming}>
					<Text bold color={COLORS.primaryBlue}>
						Zulong wants to condense your conversation:
					</Text>
				</DotRow>
				<Box flexDirection="column" paddingLeft={2}>
					<Text color="gray">{text}</Text>
	...
	if (type === "ask" && ask === "summarize_task" && text) {
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={COLORS.primaryBlue} flashing={partial === true && isStreaming}>
					<Text bold color={COLORS.primaryBlue}>
						Zulong wants to summarize the task:
					</Text>
				</DotRow>
				<Box flexDirection="column" paddingLeft={2}>
					<Text color="gray">{text}</Text>
	...
	if (type === "ask" && ask === "report_bug" && text) {
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={COLORS.primaryBlue} flashing={partial === true && isStreaming}>
					<Text bold color={COLORS.primaryBlue}>
						Zulong wants to create a Github issue:
					</Text>
				</DotRow>
				<Box flexDirection="column" paddingLeft={2}>
					<Text color="gray">{text}</Text>
