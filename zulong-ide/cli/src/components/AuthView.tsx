...
export const AuthView: React.FC<AuthViewProps> = ({ controller, onComplete, onError, onNavigateToWelcome }) => {
	...
	const [zulongModelIndex, setZulongModelIndex] = useState(0)
	...
	const mainMenuItems: SelectItem[] = useMemo(() => {
		const items: SelectItem[] = [{ label: "Sign in with Zulong", value: "zulong_auth" }]
	...
	// Subscribe to auth status updates when in zulong_auth step
	useEffect(() => {
		if (step !== "zulong_auth") {
			return
		}
		...
		const responseHandler = async (authState: { user?: { email?: string } }, _isLast?: boolean) => {
			...
			if (authState.user?.email) {
				...
				setSelectedProvider("zulong")
				setModelId(openRouterDefaultModelId)
				setStep("zulong_model")
			}
		}
	...
	// Start Zulong auth flow
	const startZulongAuth = useCallback(async () => {
		try {
			setStep("zulong_auth")
	...
	const handleMainMenuSelect = useCallback(
		...
		[exit, onComplete, startZulongAuth, startOpenAiCodexAuth],
	)
	...
	const handleZulongModelSelect = useCallback(
		(modelId: string) => {
			setModelId(modelId)
			setStep("saving")
	...
	const goBack = useCallback(() => {
		switch (step) {
			...
			case "modelid":
				setModelId("")
				// Go back to zulong_model if we came from there (Zulong provider)
				if (selectedProvider === "zulong") {
					setStep("zulong_model")
			...
			case "zulong_model": {
				return (
					<Box flexDirection="column">
						...
						<Text> </Text>
						<FeaturedModelPicker featuredModels={featuredModels} selectedIndex={zulongModelIndex} />
					</Box>
				)
			}
	...
	useInput(
		(input, key) => {
			...
			} else if (step === "zulong_model") {
				const maxIndex = getFeaturedModelMaxIndex(featuredModels)

				if (key.upArrow) {
					setZulongModelIndex((prev) => (prev > 0 ? prev - 1 : maxIndex))
				} else if (key.downArrow) {
					setZulongModelIndex((prev) => (prev < maxIndex ? prev + 1 : 0))
				} else if (isEnterKey(input, key)) {
					if (isBrowseAllSelected(zulongModelIndex, featuredModels)) {
						setStep("modelid")
					} else {
						const selectedModel = getFeaturedModelAtIndex(zulongModelIndex, featuredModels)
						if (selectedModel) {
							handleZulongModelSelect(selectedModel.id)
						}
					}
				}
			}
		...
		{ isActive: isRawModeSupported && (step === "menu" || step === "provider" || step === "zulong_model" || canGoBack) },
	)

	return (
		<Box flexDirection="column" paddingLeft={1} paddingRight={1} width="100%">
			{/* Zulong robot - centered */}
			<StaticRobotFrame />
						...
						{mainMenuItems.map((item, index) => (
							<Box key={item.value}>
								<Text>
									...
									{item.value === "zulong_auth" && <Text color="yellow"> (try Opus 4.6!)</Text>}
								</Text>
							</Box>
						))}
						<Text> </Text>
