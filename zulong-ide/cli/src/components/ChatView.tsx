...
import type { ZulongAsk, ZulongMessage } from "@shared/ExtensionMessage"
import { getApiMetrics, getLastApiReqTotalTokens } from "@shared/getApiMetrics"
import { EmptyRequest, StringRequest } from "@shared/proto/zulong/common"
import type { SlashCommandInfo } from "@shared/proto/zulong/slash"
 ...
 */
const YOLO_INTERACTIVE_ASKS = new Set<ZulongAsk>([
	"completion_result",
...
function isYoloSuppressed(yolo: boolean, ask: ZulongAsk | undefined): boolean {
 ...
 */
function getAskPromptType(ask: ZulongAsk, text: string): "confirmation" | "text" | "options" | "none" {
	switch (ask) {
		case "followup":
...
export const ChatView: React.FC<ChatViewProps> = ({
	...
	// Different providers use different state keys (e.g., zulong uses actModeOpenRouterModelId)
	...
	const messages = taskState.zulongMessages || []
	...
	const askType = pendingAsk ? getAskPromptType(pendingAsk.ask as ZulongAsk, pendingAsk.text || "") : "none"
	...
	const handleCliOnlySlashCommand = useCallback(
		(commandName: string): boolean => {
			...
			if (commandName === "models") {
				...
				const initialMode = !provider ? undefined : provider === "zulong" ? "featured-models" : "model-picker"
	...
	const buttonConfig = useMemo(() => {
		const lastMsg = messages[messages.length - 1] as ZulongMessage | undefined
	...
	useInput((input, key) => {
		...
		if (
			...
			!isSpinnerActive &&
			textInput === "" &&
			!isYoloSuppressed(yolo, pendingAsk?.ask as ZulongAsk | undefined)
		) {
		...
		if (pendingAsk && !isYoloSuppressed(yolo, pendingAsk.ask as ZulongAsk | undefined)) {
