import type { ZulongAskUseSubagents, ZulongMessage, ZulongSaySubagentStatus } from "@shared/ExtensionMessage"
...
export const SubagentMessage: React.FC<SubagentMessageProps> = ({ message, mode, isStreaming }) => {
	...
	if ((type === "ask" && ask === "use_subagents") || say === "use_subagents") {
		const parsed = text
			? jsonParseSafe<ZulongAskUseSubagents>(text, {
					prompts: [],
				})
			: { prompts: [] }
			...
			return (
				<Box flexDirection="column" marginBottom={1} width="100%">
					<DotRow color={toolColor}>
						<Text color={toolColor}>Zulong wants to run subagents:</Text>
					</DotRow>
				</Box>
			)
		}
		...
		return (
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={toolColor} flashing={partial === true && isStreaming}>
					<Text color={toolColor}>{singular ? "Zulong wants to run a subagent:" : "Zulong wants to run subagents:"}</Text>
				</DotRow>
	...
	if (say === "subagent" && text) {
		const parsed = jsonParseSafe<ZulongSaySubagentStatus>(text, {
			status: "running",
			total: 0,
			completed: 0,
			successes: 0,
			...
			<Box flexDirection="column" marginBottom={1} width="100%">
				<DotRow color={toolColor} flashing={partial === true && isStreaming}>
					<Text color={toolColor}>
						{items.length === 1 ? "Zulong is running a subagent:" : "Zulong is running subagents:"}
					</Text>
				</DotRow>
				<Box flexDirection="column" marginLeft={2} width="100%">
