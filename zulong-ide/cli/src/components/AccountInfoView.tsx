 ...
 * Shows current provider, and for Zulong provider: credit balance and organization name
 */
...
export const AccountInfoView: React.FC<AccountInfoViewProps> = React.memo(({ controller }) => {
	...
	const [organization, setOrganization] = useState<ZulongAccountOrganization | null>(null)
	...
	const fetchAccountInfo = useCallback(async () => {
		try {
			...
			setProvider(currentProvider || "zulong")

			// If using Zulong provider, fetch additional info
			if (currentProvider === "zulong") {
	...
	// If not using Zulong provider, just show the provider name
	if (provider !== "zulong") {
		return (
			<Box>
	...
	// Zulong provider but not logged in
	if (!email) {
		return (
			<Box>
				<Text color="gray">Provider: </Text>
				<Text color="cyan">Zulong</Text>
				<Text color="gray"> • </Text>
				<Text color="yellow">Not logged in (run 'zulong auth' to sign in)</Text>
			</Box>
		)
	}

	// Zulong provider - show full account info
	return (
