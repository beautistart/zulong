...
interface ConfigViewProps {
	...
	// Rules toggles
	globalZulongRulesToggles?: Record<string, boolean>
	localZulongRulesToggles?: Record<string, boolean>
...
export const ConfigView: React.FC<ConfigViewProps> = ({
	...
	const ruleEntries = useMemo(() => {
		...
		entries.push(...buildToggleEntries(globalZulongRulesToggles, "global", "zulong"))
		entries.push(...buildToggleEntries(localZulongRulesToggles, "workspace", "zulong"))
	...
	const handleToggle = () => {
		if (currentTab === "rules" && ruleEntries[selectedIndex] && onToggleRule) {
			...
			onToggleRule(entry.source === "global", entry.path, !entry.enabled, entry.ruleType || "zulong")
			...
			case "rules": {
				if (ruleEntries.length === 0) {
					return (
						<Box>
							<Text color="gray">
								No rules configured. Add .zulongrules files to your workspace or global config.
							</Text>
						</Box>
					)
				}
