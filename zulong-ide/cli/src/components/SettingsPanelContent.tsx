...
import { ZulongAccountService } from "@/services/account/ZulongAccountService"
import { AuthService, ZulongAccountOrganization } from "@/services/auth/AuthService"
...
import { useZulongFeaturedModels } from "../hooks/useZulongFeaturedModels"
...
const FEATURE_SETTINGS = {
	subagents: {
		...
		default: false,
		label: "Subagents",
		description: "Let Zulong run focused subagents in parallel to explore the codebase for you",
	},
	autoCondense: {
...
export const SettingsPanelContent: React.FC<SettingsPanelContentProps> = ({
	...
	const featuredModels = useZulongFeaturedModels()
	...
	const [accountOrganization, setAccountOrganization] = useState<ZulongAccountOrganization | null>(null)
	const [accountOrganizations, setAccountOrganizations] = useState<ZulongAccountOrganization[] | null>(null)
	const [isAccountLoading, setIsAccountLoading] = useState(false)
	const [isPickingOrganization, setIsPickingOrganization] = useState(false)
	const [isWaitingForZulongAuth, setIsWaitingForZulongAuth] = useState(false)
	...
	const fetchAccountInfo = useCallback(async () => {
		...
		try {
			...
			const accountService = ZulongAccountService.getInstance()
	...
	// Handle Zulong login - starts OAuth flow
	const handleZulongLogin = useCallback(() => {
		if (!controller) {
			return
		}
		// Set waiting state first (synchronously) to show the waiting UI immediately
		setIsWaitingForZulongAuth(true)
		// Then start the auth request (async, but we don't need to await)
		AuthService.getInstance(controller)
			.createAuthRequest()
			.catch(() => {
				setIsWaitingForZulongAuth(false)
			})
	}, [controller])

	// Handle Zulong logout
	const handleZulongLogout = useCallback(async () => {
		if (!controller) {
			return
		}
	...
	const handleOrganizationSelect = useCallback(
		async (orgId: string | null) => {
			...
			try {
				await ZulongAccountService.getInstance().switchAccount(orgId || undefined)
	...
	// Subscribe to auth status updates when waiting for Zulong auth
	useEffect(() => {
		if (!isWaitingForZulongAuth || !controller) {
			return
		}
		...
		const responseHandler = async (authState: { user?: { email?: string } }) => {
			...
			if (authState.user?.email) {
				...
				await applyProviderConfig({ providerId: "zulong", controller })
				setProvider("zulong")
				refreshModelIds()
				fetchAccountInfo()
			}
	...
	}, [isWaitingForZulongAuth, controller, fetchAccountInfo])
	...
	const items: ListItem[] = useMemo(() => {
		...
		switch (currentTab) {
			...
			case "auto-approve": {
				...
				result.push(
					...
					{ key: "separator", label: "", type: "separator", value: false },
					{
						...
						description: "System alerts when Zulong needs your attention",
					},
				)
				return result
			}
			...
			case "other":
				return [
					{ key: "language", label: "Preferred language", type: "editable", value: preferredLanguage },
					{
						...
						description: "Help improve Zulong by sending anonymous usage data",
					},
					{ key: "separator", label: "", type: "separator", value: "" },
					{ key: "version", label: "", type: "readonly", value: `Zulong v${CLI_VERSION}` },
				]
			...
			case "account":
				...
				if (!accountEmail) {
					return [{ key: "login", label: "Sign in with Zulong", type: "action", value: "" }]
				}
	...
	const handleAction = useCallback(() => {
		...
		if (item.type === "editable") {
			...
			if ((item.key === "actModelId" || item.key === "planModelId") && hasModelPicker(provider)) {
				...
				// For Zulong provider, show featured models first
				if (provider === "zulong") {
	...
	const handleModelSelect = useCallback(
		async (modelId: string) => {
			...
			// Use provider-specific model ID keys (e.g., zulong uses actModeOpenRouterModelId)
			const actKey = actProvider ? getProviderModelIdKey(actProvider, "act") : null
			const planKey = planProvider ? getProviderModelIdKey(planProvider, "plan") : null

			// For zulong/openrouter providers, also set model info (like webview does)
			let modelInfo: ModelInfo | undefined
			if (providerForSelection === "zulong" || providerForSelection === "openrouter") {
	...
	const handleProviderSelect = useCallback(
		async (providerId: string) => {
			// Special handling for Zulong - uses OAuth (but skip if already logged in)
		...
		[stateManager, startCodexAuth, handleZulongLogin, startOcaAuth, isOcaAuthenticated, controller, refreshModelIds],
	)
	...
	useInput(
		(input, key) => {
			...
			// Featured model picker mode (Zulong provider)
			...
			// Zulong OAuth waiting mode - escape to cancel
			if (isWaitingForZulongAuth) {
				if (key.escape) {
					setIsWaitingForZulongAuth(false)
				}
				return
	...
	const renderContent = () => {
			...
			return (
				<Box flexDirection="column">
					<Box>
						...
						<Text color="white"> Waiting for Zulong sign-in...</Text>
					</Box>
					<Box marginTop={1}>
		...
		if (currentTab === "account" && !accountEmail && !isAccountLoading) {
			return (
				<Box flexDirection="column">
					<Text color="white">Sign in to access Zulong features:</Text>
