...
import type { ZulongMessage, ExtensionState } from "@shared/ExtensionMessage"
	...
	useEffect(() => {
		...
		const handleStateUpdate = async () => {
			try {
				...
				const hadMessages = (stateRef.current.zulongMessages?.length ?? 0) > 0
				const hasMessages = (newState.zulongMessages?.length ?? 0) > 0
		...
		const unsubscribePartial = registerPartialMessageCallback((protoMessage) => {
			const updatedMessage = convertProtoToZulongMessage(protoMessage) as ZulongMessage
			setState((prevState) => {
				const messages = prevState.zulongMessages || []
				// Find and update the message by timestamp
				const index = messages.findIndex((m) => m.ts === updatedMessage.ts)
				if (index >= 0) {
					const newMessages = [...messages]
					newMessages[index] = updatedMessage
					return { ...prevState, zulongMessages: newMessages }
				}
				return prevState
			})
		})
