 ...
 * - Enables workflows like: git diff | zulong 'explain' | zulong 'summarize'
 */
...
export async function runPlainTextTask(options: PlainTextTaskOptions): Promise<boolean> {
	...
	const processMessage = (message: ZulongMessage) => {
	...
	const requestId = "zulong-cli-plain-text-task"
	subscribeToState(
		controller,
		{},
		async ({ stateJson }) => {
			try {
				...
				for (const message of state.zulongMessages ?? []) {
 ...
 */
function handleMessageForPipeMode(message: ZulongMessage, verbose: boolean): void {
