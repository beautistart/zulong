...
export async function checkAnyProviderConfigured(): Promise<boolean> {
	...
	// Check Zulong account (stored as "zulong:zulongAccountId" in secrets, loaded into config)
	if (config["zulongApiKey"] || config["zulong:zulongAccountId"]) return true
	...
	for (const [provider, keyField] of Object.entries(ProviderToApiKeyMap)) {
		// Skip zulong - already checked above with the correct key
		if (provider === "zulong") continue
