...
function createMcpServerConfig(targetOrCommand: string[], transportType: McpAddTransportType): Record<string, unknown> {
	if (transportType === "stdio") {
		if (targetOrCommand.length < 1) {
			throw new Error("Missing stdio command. Example: zulong mcp add kanban -- kanban mcp")
		}

		// Guard against common mistake:
		// `zulong mcp add <name> <url>` without `--type http`
		if (targetOrCommand.length === 1) {
			...
			try {
				...
				if (parsedUrl.protocol === "http:" || parsedUrl.protocol === "https:") {
					throw new Error(
						`Looks like you provided a URL for '${value}'. Use --type http, for example: zulong mcp add <name> ${value} --type http`,
					)
				}
			} catch (error) {
	...
	if (targetOrCommand.length !== 1) {
		throw new Error(
			"HTTP/SSE MCP servers require exactly one URL. Example: zulong mcp add linear https://mcp.linear.app/mcp --type http",
		)
	}
