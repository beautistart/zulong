/**
 * Terminal display utilities for rendering Zulong messages in the CLI
 */

import type { ZulongAsk, ZulongMessage, ZulongSay, ExtensionState } from "@shared/ExtensionMessage"
 ...
 */
function getMessageIcon(message: ZulongMessage): string {
	if (message.type === "ask") {
		switch (message.ask) {
			case "followup":
				return "❓"
...
/**
 * Format a ZulongMessage for terminal display
 */
export function formatMessage(message: ZulongMessage, verbose: boolean = false): string {
...
function formatAskMessage(message: ZulongMessage, prefix: string, verbose: boolean): string {
	const ask = message.ask as ZulongAsk
...
function formatSayMessage(message: ZulongMessage, prefix: string, verbose: boolean): string {
	const say = message.say as ZulongSay
...
export function formatState(state: ExtensionState, verbose: boolean = false): string {
	...
	// Show messages
	if (state.zulongMessages && state.zulongMessages.length > 0) {
		const messagesToShow = verbose
			? state.zulongMessages
			: state.zulongMessages.filter((m) => {
