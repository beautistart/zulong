...
export const useCompletedAskMessages = () => {
	...
	const getCompletedAskMessages = useCallback(() => {
		const completedAsks: ZulongMessage[] = []

		if (!state.zulongMessages) {
			return completedAsks
		}

		for (let i = 0; i < state.zulongMessages.length; i++) {
			const message = state.zulongMessages[i]
		...
		return completedAsks
	}, [state.zulongMessages, processed])
...
export const useLastCompletedAskMessage = () => {
	...
	const getLastCompletedAskMessage = useCallback((): ZulongMessage | null => {
		if (!state.zulongMessages) {
			return null
		}

		// Find the last ask message that is complete
		for (let i = state.zulongMessages.length - 1; i >= 0; i--) {
			const message = state.zulongMessages[i]
...
export const useCompletionSignals = () => {
	...
	const isTaskComplete = useCallback((): boolean => {
		...
		const lastMessage = state.zulongMessages[state.zulongMessages.length - 1]
		if (!lastMessage) {
			return false
		}
	...
	const getCompletionMessage = useCallback((): ZulongMessage | null => {
		if (!state.zulongMessages || state.zulongMessages.length === 0) {
			return null
		}

		return state.zulongMessages[state.zulongMessages.length - 1] || null
	}, [state.zulongMessages])
...
export const useIsSpinnerActive = (): { isActive: boolean; startTime?: number } => {
	...
	for (let i = state.zulongMessages.length - 1; i >= 0; i--) {
		const msg = state.zulongMessages[i]
		if (msg.say === "api_req_started") {
			// Check if there's an api_req_finished after this
			let hasFinished = false
			for (let j = i + 1; j < state.zulongMessages.length; j++) {
				if (state.zulongMessages[j].say === "api_req_finished") {
					hasFinished = true
					break
				}
			}
