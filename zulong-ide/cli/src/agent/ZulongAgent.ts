/**
 * ZulongAgent - Decoupled ACP Agent implementation for Zulong CLI.
 *
 * This class implements the ACP (Agent Client Protocol) Agent interface,
 * allowing Zulong to be used programmatically without stdio dependency.
...
import type { ZulongMessageChange } from "@core/task/message-state"
...
import type { ZulongAsk, ZulongMessage as ZulongMessageType } from "@shared/ExtensionMessage"
import { CLI_ONLY_COMMANDS, VSCODE_ONLY_COMMANDS } from "@shared/slashCommands"
import { getProviderModelIdKey } from "@shared/storage/provider-keys"
import { ZulongEndpoint } from "@/config.js"
...
import { ZulongSessionEmitter } from "./ZulongSessionEmitter.js"
import { translateMessage } from "./messageTranslator.js"
import { handlePermissionResponse } from "./permissionHandler.js"
import type { ZulongAcpSession, ZulongAgentOptions, PermissionHandler } from "./public-types.js"
...
/**
 * Zulong's implementation of the ACP Agent interface.
 *
 * This agent bridges the ACP protocol with Zulong's core Controller,
 ...
 * For stdio-based ACP communication, use the AcpAgent wrapper class.
 */
export class ZulongAgent implements acp.Agent {
	private readonly options: ZulongAgentOptions
	private readonly ctx: CliContextResult

	/** Map of active sessions by session ID */
	public readonly sessions: Map<string, ZulongAcpSession> = new Map()

	/** WeakMap to associate ZulongAcpSession with its Controller without exposing it to consumers */
	readonly #sessionControllers = new WeakMap<ZulongAcpSession, Controller>()
	...
	private readonly sessionEmitters: Map<string, ZulongSessionEmitter> = new Map()
	...
	constructor(options: ZulongAgentOptions) {
		this.options = options
		setRuntimeHooksDir(options.hooksDir)
		this.ctx = initializeCliContext({ zulongDir: options.zulongDir })
	}
	 ...
	 */
	emitterForSession(sessionId: string): ZulongSessionEmitter {
		let emitter = this.sessionEmitters.get(sessionId)
		if (!emitter) {
			emitter = new ZulongSessionEmitter()
	...
	async initialize(params: acp.InitializeRequest, connection?: acp.AgentSideConnection): Promise<acp.InitializeResponse> {
		...
		await ZulongEndpoint.initialize(this.ctx.EXTENSION_DIR)
		...
		return {
			...
			authMethods: [
				{
					id: "zulong-oauth",
					name: "Sign in with Zulong",
					description: "Authenticate with your Zulong account via browser OAuth",
				},
				{
	...
	async newSession(params: acp.NewSessionRequest): Promise<acp.NewSessionResponse> {
		...
		Logger.debug("[ZulongAgent] newSession called:", {
			sessionId,
			cwd: params.cwd,
		...
		const session: ZulongAcpSession = {
			sessionId,
			cwd: params.cwd,
		...
		await this.sendAvailableCommands(sessionId, controller).catch((error) => {
			Logger.debug("[ZulongAgent] Failed to send available commands:", error)
		})
	...
	private async getSessionModelState(mode: Mode): Promise<acp.SessionModelState> {
		...
		// Use provider-specific model ID key (e.g., zulong uses actModeOpenRouterModelId)
	...
	async unstable_setSessionModel(params: acp.SetSessionModelRequest): Promise<acp.SetSessionModelResponse> {
		...
		Logger.debug("[ZulongAgent] unstable_setSessionModel called:", {
		...
		// Update model ID using provider-specific keys (e.g., zulong uses actModeOpenRouterModelId)
	 ...
	 * 2. Set up internal zulong state subsription
	 * 3. Initialize or continue zulong task
	 * 4. Translate ZulongMessages to ACP SessionUpdates
	 * 5. Handle permission requests for tools/commands
	 * 6. Return when zulong task completes, is cancelled, or needs user input
	 */
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		Logger.debug("[ZulongAgent] prompt called:", {
			sessionId: params.sessionId,
		...
		try {
			...
			if (isLoadedSession && !hasActiveTask) {
				...
				Logger.debug("[ZulongAgent] Resuming loaded session:", params.sessionId)
				...
				Logger.debug("[ZulongAgent] Continuing existing task:", controller.task.taskId)

				// Find the last ask message and respond to it
				const messages = controller.task.messageStateHandler.getZulongMessages()
				...
				if (lastAskMessage) {
					...
					Logger.debug("[ZulongAgent] No pending ask found, starting new task")
					await controller.initTask(textContent, imageContent, fileResources)
				}
			} else {
				// Start new task
				Logger.debug("[ZulongAgent] Starting new task")
				await controller.initTask(textContent, imageContent, fileResources)
			}

			// Subscribe to zulongMessages changes after task is created
			if (controller.task) {
				const onZulongMessagesChanged = (change: ZulongMessageChange) => {
					this.handleZulongMessagesChanged(params.sessionId, sessionState, change, resolvePrompt, promptResolved).catch(
						(error) => {
							Logger.debug("[ZulongAgent] Error handling zulongMessagesChanged:", error)
						},
					)
				}

				controller.task.messageStateHandler.on("zulongMessagesChanged", onZulongMessagesChanged)
				cleanupFunctions.push(() => {
					controller.task?.messageStateHandler.off("zulongMessagesChanged", onZulongMessagesChanged)
				})
			}
			...
			for (const cleanup of cleanupFunctions) {
				try {
					...
					Logger.debug("[ZulongAgent] Error during cleanup:", error)
				}
			}
	...
	private async handleZulongMessagesChanged(
		sessionId: string,
		sessionState: AcpSessionState,
		change: ZulongMessageChange,
		resolvePrompt: (response: acp.PromptResponse) => void,
		promptResolved: { value: boolean },
	): Promise<void> {
		Logger.debug("[ZulongAgent] handleZulongMessagesChanged:", change)
		try {
			switch (change.type) {
				case "add":
	 ...
	 * 3. Responds to Zulong's ask based on the decision
	 *
	 * @param sessionId - The session ID
	 * @param sessionState - The session state
	 * @param message - The Zulong ask message
	 * @param permissionRequest - The permission request details from translateMessage
	 */
	private async handlePermissionRequest(
		sessionId: string,
		sessionState: AcpSessionState,
		message: ZulongMessageType,
		permissionRequest: Omit<acp.RequestPermissionRequest, "sessionId">,
	): Promise<void> {
		const session = this.sessions.get(sessionId)

		if (!session) {
			Logger.debug("[ZulongAgent] No session found for permission request")
			return
		}
		...
		if (!controller?.task) {
			Logger.debug("[ZulongAgent] No active task for permission request")
			return
		}

		const askType = message.ask as ZulongAsk

		try {
			// Request permission from the client
			const response = await this.requestPermission(sessionId, permissionRequest.toolCall, permissionRequest.options)

			Logger.debug("[ZulongAgent] Permission response received:", response.outcome)
	 ...
	 *
	 * This method uses translateMessage to properly map ZulongMessages to ACP SessionUpdates,
	...
	private async processMessageWithDelta(
		...
		if (isTextStreamingMessage && message.text) {
			...
			if (result.requiresPermission && result.permissionRequest && !message.partial) {
				...
				// This will request permission from the client and respond to Zulong
	...
	async cancel(params: acp.CancelNotification): Promise<void> {
		const session = this.sessions.get(params.sessionId)
		if (!session) {
			Logger.debug("[ZulongAgent] cancel called for non-existent session:", params.sessionId)
			return
		}
		const sessionState = this.sessionStates.get(params.sessionId)

		Logger.debug("[ZulongAgent] cancel called:", {
			sessionId: params.sessionId,
		...
		if (sessionState) {
			...
			if (controller?.task) {
				try {
					...
					Logger.debug("[ZulongAgent] Error cancelling task:", error)
				}
			}
		}
	}
	...
	async setSessionMode(params: acp.SetSessionModeRequest): Promise<acp.SetSessionModeResponse> {
		...
		Logger.debug("[ZulongAgent] setSessionMode called:", {
			sessionId: params.sessionId,
	 ...
	 * - Zulong account authentication (zulong-oauth)
	...
	async authenticate(params: acp.AuthenticateRequest): Promise<acp.AuthenticateResponse> {
		Logger.debug("[ZulongAgent] authenticate called:", { methodId: params.methodId })
		...
		if (params.methodId !== "zulong-oauth") {
		...
		Logger.debug("[ZulongAgent] AuthHandler enabled, getting callback URL...")

		// Get the callback URL first to ensure the server is ready
		let callbackUrl: string
		try {
			callbackUrl = await authHandler.getCallbackUrl("/auth")
			Logger.debug("[ZulongAgent] Callback URL ready:", callbackUrl)
		} catch (error) {
			Logger.error("[ZulongAgent] Failed to get callback URL:", error)
		...
		try {
			...
			Logger.debug("[ZulongAgent] Starting OAuth flow...")

			// Start the OAuth flow - this opens the browser
			await authService.createAuthRequest()

			Logger.debug("[ZulongAgent] Browser opened, waiting for callback...")
			...
			while (Date.now() - startTime < AUTH_TIMEOUT_MS) {
				...
				const authData = stateManager.getSecretKey("zulong:zulongAccountId")
				if (authData) {
					Logger.debug("[ZulongAgent] Authentication successful")

					// Set up the provider configuration for zulong
					stateManager.setGlobalState("actModeApiProvider", "zulong")
					stateManager.setGlobalState("planModeApiProvider", "zulong")
					await stateManager.flushPendingState()
			...
			Logger.error("[ZulongAgent] Authentication error:", error)
			throw error
		}
	}
	...
	private async emitSessionUpdate(sessionId: string, update: acp.SessionUpdate): Promise<void> {
		...
		try {
			...
			Logger.debug("[ZulongAgent] Error emitting session update:", error)
	...
	protected async requestPermission(
		...
		Logger.debug("[ZulongAgent] Requesting permission:", {
		...
		if (!this.permissionHandler) {
			...
			Logger.debug("[ZulongAgent] No permission handler set, auto-rejecting")
	 ...
	 *
	 * This fetches commands from Zulong's slash command system, filters out
	...
	private async sendAvailableCommands(sessionId: string, controller: Controller): Promise<void> {
		try {
			// Get all available commands from Zulong
			...
			Logger.debug("[ZulongAgent] Sent available commands:", {
				sessionId,
				commandCount: availableCommands.length,
				commands: availableCommands.map((c) => c.name),
			})
		} catch (error) {
			Logger.debug("[ZulongAgent] Error sending available commands:", error)
		}
	}
	...
	private async authenticateOpenAiCodex(): Promise<acp.AuthenticateResponse> {
		Logger.debug("[ZulongAgent] Starting OpenAI Codex OAuth flow...")

		try {
			// Get the authorization URL and start the callback server
			const authUrl = openAiCodexOAuthManager.startAuthorizationFlow()

			Logger.debug("[ZulongAgent] Opening browser for OpenAI Codex auth:", authUrl)
			...
			Logger.debug("[ZulongAgent] OpenAI Codex authentication successful")
			...
			Logger.error("[ZulongAgent] OpenAI Codex authentication error:", error)
			throw error
		}
	}
}
