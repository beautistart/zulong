...
import type { ZulongSessionEvents } from "./public-types.js"

/**
 * Type-safe EventEmitter for ZulongAgent session events.
 *
 ...
 * @example
 * ```typescript
 * const agent = new ZulongAgent({ version: "1.0.0" })
 ...
 * ```
 */
export class ZulongSessionEmitter {
	 ...
	 */
	on<K extends keyof ZulongSessionEvents>(event: K, listener: ZulongSessionEvents[K]): this {
	 ...
	 * @returns This emitter instance for chaining
	 */
	once<K extends keyof ZulongSessionEvents>(event: K, listener: ZulongSessionEvents[K]): this {
	 ...
	 * @returns This emitter instance for chaining
	 */
	off<K extends keyof ZulongSessionEvents>(event: K, listener: ZulongSessionEvents[K]): this {
	 ...
	 */
	emit<K extends keyof ZulongSessionEvents>(event: K, ...args: Parameters<ZulongSessionEvents[K]>): boolean {
	 ...
	 * @returns This emitter instance for chaining
	 */
	removeAllListeners<K extends keyof ZulongSessionEvents>(event?: K): this {
		if (event) {
	 ...
	 */
	listenerCount<K extends keyof ZulongSessionEvents>(event: K): number {
