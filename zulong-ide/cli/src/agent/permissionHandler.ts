 ...
 * and Zulong's internal permission system. It maps ZulongAsk types to appropriate
 * ACP permission options and translates user responses back to Zulong's format.
 *
 * @module acp/permissionHandler
 */

import type * as acp from "@agentclientprotocol/sdk"
import type { ZulongAsk } from "@shared/ExtensionMessage"
import type { ZulongAskResponse } from "@shared/WebviewMessage"
import { Logger } from "@/shared/services/Logger.js"
import type { AcpSessionState, ZulongPermissionOption } from "./types.js"

/**
 * Standard permission options for operations that support "always allow".
 * Used for commands, tools, and MCP server operations.
 */
const STANDARD_PERMISSION_OPTIONS: ZulongPermissionOption[] = [
 ...
 */
const RESTRICTED_PERMISSION_OPTIONS: ZulongPermissionOption[] = [
	{ kind: "allow_once", optionId: "allow_once", name: "Allow Once" },
	{ kind: "reject_once", optionId: "reject_once", name: "Reject" },
]

/**
 * Mapping of ZulongAsk types to their permission option sets.
 */
const ASK_TYPE_PERMISSION_MAP: Partial<Record<ZulongAsk, ZulongPermissionOption[]>> = {
...
/**
 * ZulongAsk types that require permission handling.
 * Other ask types (like followup, plan_mode_respond) don't need permission UI.
 */
const PERMISSION_REQUIRING_ASK_TYPES: Set<ZulongAsk> = new Set([
	"command",
	"tool",
...
export interface PermissionHandlerResult {
	/** Zulong's internal response type */
...
/**
 * Check if a ZulongAsk type requires permission handling.
 *
 * @param askType - The ZulongAsk type to check
 * @returns True if the ask type requires permission UI
 */
export function requiresPermission(askType: ZulongAsk): boolean {
	return PERMISSION_REQUIRING_ASK_TYPES.has(askType)
}

/**
 * Get the appropriate permission options for a ZulongAsk type.
 *
 * @param askType - The ZulongAsk type
 * @returns Array of permission options, or undefined if the ask type doesn't require permission
 */
export function getPermissionOptionsForAskType(askType: ZulongAsk): acp.PermissionOption[] | undefined {
...
/**
 * Handle an ACP permission response and translate it to Zulong's format.
 *
 * @param response - The ACP permission response from the client
 * @param askType - The original ZulongAsk type that triggered the permission request
 * @returns The translated result for Zulong's handleWebviewAskResponse
 */
export function handlePermissionResponse(response: acp.RequestPermissionResponse, askType: ZulongAsk): PermissionHandlerResult {
	// Check if cancelled
	...
	// Translate the option to Zulong's response format
	switch (optionId) {
		case "allow_once":
			return {
 ...
 * @param askType - The Zulong ask type
...
export class AutoApprovalTracker {
	 ...
	 *
	 * @param askType - The Zulong ask type that was auto-approved
	 * @param identifier - The identifier for the operation (command, tool name, etc.)
	 */
	recordAlwaysAllow(askType: ZulongAsk, identifier: string): void {
		switch (askType) {
			case "command":
	 ...
	 */
	isAutoApproved(askType: ZulongAsk, identifier: string): boolean {
		switch (askType) {
			case "command":
				const commandPrefix = identifier.split(" ")[0]
 ...
 * 4. Returns the translated result for Zulong
 *
 ...
 */
export function getAutoApprovalIdentifier(toolCall: acp.ToolCall, askType: ZulongAsk): string {
