 ...
 *
 * Tests the translation of Zulong messages to ACP session updates,
...
/**
 * Create a minimal ZulongMessage for testing.
 */
function createZulongMessage(overrides: Partial<ZulongMessage>): ZulongMessage {
	return {
		ts: Date.now(),
		type: "say",
		...overrides,
...
describe("translateMessage - say messages", () => {
	...
	describe("text messages", () => {
		it("should translate say:text to agent_message_chunk", () => {
			const message = createZulongMessage({
				type: "say",
				say: "text",
...
describe("translateMessages", () => {
	...
	it("should translate multiple messages in sequence", () => {
		const messages: ZulongMessage[] = [
			createZulongMessage({ type: "say", say: "text", text: "First message" }),
			createZulongMessage({ type: "say", say: "reasoning", reasoning: "Thinking..." }),
			createZulongMessage({ type: "say", say: "text", text: "Second message" }),
		]
	...
	it("should maintain session state across messages", () => {
		const messages: ZulongMessage[] = [
			createZulongMessage({ type: "say", say: "command", text: "npm test" }),
			createZulongMessage({ type: "say", say: "command_output", text: "All tests passed", commandCompleted: true }),
		]
