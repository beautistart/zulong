/**
 * Message translator for converting Zulong messages to ACP session updates.
 *
 * This module handles the translation between Zulong's internal message format
 * (ZulongMessage) and the ACP protocol's session update format. A single Zulong
...
/**
 * Maps Zulong tool types to ACP ToolKind values.
 */
...
/**
 * Translate a single Zulong message to ACP session updates.
 *
 * @param message - The Zulong message to translate
...
/**
 * Translate a "say" type Zulong message to ACP updates.
 */
...
/**
 * Translate a "ask" type Zulong message to ACP updates.
 ...
 */
function translateToolMessage(message: ZulongMessage, sessionState: AcpSessionState): acp.SessionUpdate[] {
	const updates: acp.SessionUpdate[] = []

	if (!message.text) return updates

	try {
		const toolInfo = JSON.parse(message.text) as ZulongSayTool
 ...
 */
function translateCommandMessage(message: ZulongMessage, sessionState: AcpSessionState): acp.SessionUpdate[] {
	const updates: acp.SessionUpdate[] = []
 ...
 */
function translateCommandOutputMessage(message: ZulongMessage, sessionState: AcpSessionState): acp.SessionUpdate[] {
	const updates: acp.SessionUpdate[] = []
 ...
 */
function translateBrowserActionMessage(message: ZulongMessage, sessionState: AcpSessionState): acp.SessionUpdate[] {
	const updates: acp.SessionUpdate[] = []

	try {
		const action = message.text ? (JSON.parse(message.text) as ZulongSayBrowserAction) : null
		const toolCallId = sessionState.currentToolCallId || generateToolCallId()
 ...
 */
function translateMcpMessage(message: ZulongMessage, sessionState: AcpSessionState): acp.SessionUpdate[] {
	const updates: acp.SessionUpdate[] = []
 ...
 */
function translateTaskProgressMessage(message: ZulongMessage): acp.SessionUpdate[] {
	const updates: acp.SessionUpdate[] = []
 ...
 */
function buildToolTitle(toolInfo: ZulongSayTool): string {
...
function parseToolInfo(text: string): { title: string; kind: acp.ToolKind; path?: string; input?: unknown } | null {
	try {
		const info = JSON.parse(text) as ZulongSayTool
		return {
...
/**
 * Translate multiple Zulong messages to ACP session updates.
 *
 * @param messages - Array of Zulong messages to translate
 * @param sessionState - The current session state
 * @returns Combined array of ACP session updates
 */
export function translateMessages(messages: ZulongMessage[], sessionState: AcpSessionState): acp.SessionUpdate[] {
