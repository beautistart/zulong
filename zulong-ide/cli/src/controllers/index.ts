...
export class CliEnvServiceClient implements EnvServiceClientInterface {
	...
	async clipboardWriteText(request: proto.zulong.StringRequest): Promise<proto.zulong.Empty> {
	...
	async clipboardReadText(_request: proto.zulong.EmptyRequest): Promise<proto.zulong.String> {
		return proto.zulong.String.create({ value: this.clipboardContent })
	}
	...
	debugLog(request: proto.zulong.StringRequest): Promise<proto.zulong.Empty> {
		const message = request.value || ""
		if (process.env.IS_DEV) {
			printInfo(`[DebugLog] ${message}`)
		}
		return Promise.resolve(proto.zulong.Empty.create())
	}
...
export class CliWindowServiceClient implements WindowServiceClientInterface {
	...
	async openSettings(_request: proto.host.OpenSettingsRequest): Promise<proto.host.OpenSettingsResponse> {
		printInfo("Settings can be configured in ~/.zulong/data/globalState.json")
