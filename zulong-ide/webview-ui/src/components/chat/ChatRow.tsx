...
import {
	ZulongApiReqInfo,
	ZulongAskQuestion,
	ZulongAskUseMcpServer,
	ZulongMessage,
	ZulongPlanModeResponse,
	ZulongSayGenerateExplanation,
	ZulongSayTool,
	COMPLETION_RESULT_CHANGES_FLAG,
} from "@shared/ExtensionMessage"
import { BooleanRequest, StringRequest } from "@shared/proto/zulong/common"
...
interface ChatRowProps {
	...
	lastModifiedMessage?: ZulongMessage
	isLast: boolean
	...
	({
		...
		const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: ZulongApiReqInfo = JSON.parse(message.text)
			...
			switch (type) {
				...
				case "mistake_limit_reached":
					return [
						...
						<span className="text-error font-bold">Zulong 遇到了问题...</span>,
					]
				case "command":
					return [
						<TerminalIcon className="text-foreground size-2" />,
						<span className="font-bold text-foreground">Zulong 想要执行以下命令：</span>,
					]
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as ZulongAskUseMcpServer
					return [
				...
				case "followup":
					return [
						...
						<span className="font-bold text-foreground">Zulong 有一个问题：</span>,
					]
				default:
					return [null, null]
			}
		...
		const tool = useMemo(() => {
			if (message.ask === "tool" || message.say === "tool") {
				return JSON.parse(message.text || "{}") as ZulongSayTool
			}
			return null
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span style={{ fontWeight: "bold" }}>Zulong 想要删除此文件：</span>
							</div>
							<CodeAccordian
								// isLoading={message.partial}
								code={tool.content}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Zulong 想要创建新文件：</span>
							</div>
							{backgroundEditEnabled && tool.path && tool.content ? (
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Zulong 想要读取此文件：</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Zulong 想要递归查看此目录中的所有文件："
										: "Zulong 递归查看了此目录中的所有文件："}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Zulong 想要查看此目录中使用的源代码定义名称："
										: "Zulong 查看了此目录中使用的源代码定义名称："}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									Zulong 想要在此目录中搜索 <code className="break-all">{tool.regex}</code>:
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Zulong 正在压缩对话：</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Zulong loaded the skill:</span>
							</div>
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as ZulongAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					...
					case "zulongignore_error":
						return <ErrorRow errorType="zulongignore_error" message={message} />
					...
					case "generate_explanation": {
						let explanationInfo: ZulongSayGenerateExplanation = {
							title: "code changes",
							fromRef: "",
							toRef: "",
							status: "generating",
						...
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								...
								<div className="text-foreground opacity-80">
									Zulong may have trouble viewing the command's output. Please update VSCode (
									<code>CMD/CTRL + Shift + P</code> → "Update") and make sure you're using a supported shell:
									zsh, bash, fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal: Select Default
									Profile").
									<a
										className="px-1"
										href="https://github.com/zulong/zulong/wiki/Troubleshooting-%E2%80%90-Shell-Integration-Unavailable">
			...
			case "ask":
				switch (message.ask) {
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as ZulongAskQuestion
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Zulong 想要开始新任务：</span>
								</div>
					...
					case "condense":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Zulong 想要压缩您的对话：</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</div>
						)
					case "report_bug":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Zulong 想要创建 GitHub 问题：</span>
								</div>
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as ZulongPlanModeResponse
