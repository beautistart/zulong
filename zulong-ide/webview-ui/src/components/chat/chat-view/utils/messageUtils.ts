...
import type { ZulongMessage, ZulongSayBrowserAction, ZulongSayTool } from "@shared/ExtensionMessage"
 ...
 */
export function isLowStakesTool(message: ZulongMessage): boolean {
	if (message.say !== "tool" && message.ask !== "tool") {
		return false
	}
	try {
		const tool = JSON.parse(message.text || "{}") as ZulongSayTool
 ...
 */
export function isToolGroup(item: ZulongMessage | ZulongMessage[]): item is ZulongMessage[] & { _isToolGroup: true } {
 ...
 */
export function processMessages(messages: ZulongMessage[]): ZulongMessage[] {
 ...
 */
export function filterVisibleMessages(messages: ZulongMessage[]): ZulongMessage[] {
	return messages.filter((message, index, arr) => {
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if zulong wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes zulong returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
export function isBrowserSessionMessage(message: ZulongMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: ZulongMessage[]): (ZulongMessage | ZulongMessage[])[] {
	const result: (ZulongMessage | ZulongMessage[])[] = []
	let currentGroup: ZulongMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as ZulongSayBrowserAction
 ...
 */
export function getTaskMessage(messages: ZulongMessage[]): ZulongMessage | undefined {
 ...
 */
function isDisplayedCheckpoint(checkpointIndex: number, allMessages: ZulongMessage[]): boolean {
	...
	for (let i = checkpointIndex - 1; i >= 0; i--) {
		...
		if (msg.say === "tool" || msg.ask === "tool") {
			try {
				const tool = JSON.parse(msg.text || "{}") as ZulongSayTool
 ...
 */
export function findNextSegmentCost(checkpointTs: number, allMessages: ZulongMessage[]): number | undefined {
	const checkpointIndex = allMessages.findIndex((m) => m.ts === checkpointTs && m.say === "checkpoint_created")
	if (checkpointIndex === -1) {
		return undefined
	}
 ...
 */
export function isTextMessagePendingToolCall(textTs: number, allMessages: ZulongMessage[]): boolean {
 ...
 */
export function isToolGroupInFlight(toolGroupMessages: ZulongMessage[], allMessages: ZulongMessage[]): boolean {
	if (toolGroupMessages.length === 0) {
		return false
	}

	// Step 1: Find the MOST RECENT api_req_started overall (search backwards)
	let mostRecentApiReq: ZulongMessage | null = null
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: ZulongMessage[], allMessages: ZulongMessage[]): ZulongMessage[] {
 ...
 *
 * Note: this operates on a flat `ZulongMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: ZulongMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (ZulongMessage | ZulongMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (ZulongMessage | ZulongMessage[])[]): (ZulongMessage | ZulongMessage[])[] {
	const result: (ZulongMessage | ZulongMessage[])[] = []
	let toolGroup: ZulongMessage[] = []
	let pendingReasoning: ZulongMessage[] = []
	let pendingApiReq: ZulongMessage[] = []
	let hasTools = false
	const pendingTools: ZulongMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as ZulongMessage[] & { _isToolGroup: boolean }
