import { ZulongMessage, ZulongSayTool } from "@shared/ExtensionMessage"
...
const getCurrentActivities = (allMessages: ZulongMessage[]): ZulongMessage[] => {
	...
	const activities: ZulongMessage[] = []
 ...
 */
export function buildToolsWithReasoning(messages: ZulongMessage[]): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): ZulongSayTool {
	try {
		return JSON.parse(text || "{}") as ZulongSayTool
	} catch {
		return {} as ZulongSayTool
	}
}
 ...
 */
function getToolDisplayInfo(tool: ZulongSayTool) {
 ...
 */
export function getToolGroupSummaryFromParsedTools(tools: ZulongSayTool[]): string {
	...
	return parts.length === 0 ? "Context" : "Zulong" + action + parts.join(", ")
}
