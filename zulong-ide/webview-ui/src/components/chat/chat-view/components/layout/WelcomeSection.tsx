import { BANNER_DATA, BannerAction, BannerActionType, BannerCardData } from "@shared/zulong/banner"
import { EmptyRequest } from "@shared/proto/zulong/common"
import type { Worktree } from "@shared/proto/zulong/worktree"
import { TrackWorktreeViewOpenedRequest } from "@shared/proto/zulong/worktree"
...
export const WelcomeSection: React.FC<WelcomeSectionProps> = ({
	...
	const bannerConfig = useMemo((): BannerCardData[] => {
		...
		return BANNER_DATA.filter((banner) => {
			...
			if (banner.isZulongUserOnly !== undefined) {
				return banner.isZulongUserOnly === !!zulongUser
			}
			...
			switch (action.action) {
				...
				case BannerActionType.SetModel: {
					...
					handleFieldsChange({
						...
						planModeApiProvider: "zulong",
						actModeApiProvider: "zulong",
					})
				...
				case BannerActionType.InstallCli:
					StateServiceClient.installZulongCli({}).catch((error) =>
	...
	}, [bannerConfig, banners, zulongUser, handleBannerAction, handleBannerDismiss])
								...
								{currentWorktree && (
									<Tooltip>
										...
										<TooltipContent side="bottom">
											View and manage git worktrees. Great for running parallel Zulong tasks.
										</TooltipContent>
									</Tooltip>
								)}
							</div>
