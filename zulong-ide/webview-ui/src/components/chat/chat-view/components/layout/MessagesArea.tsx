...
interface MessagesAreaProps {
	task: ZulongMessage
	groupedMessages: (ZulongMessage | ZulongMessage[])[]
	modifiedMessages: ZulongMessage[]
...
export const MessagesArea: React.FC<MessagesAreaProps> = ({
	...
	const { zulongMessages } = useExtensionState()
	const lastRawMessage = useMemo(() => zulongMessages.at(-1), [zulongMessages])
	...
	const scrolledPastUserMessageIndex = useMemo(() => {
		...
		return zulongMessages.findIndex((msg) => msg.ts === scrolledPastUserMessage.ts)
	}, [zulongMessages, scrolledPastUserMessage])
	...
	const displayedGroupedMessages = useMemo<(ZulongMessage | ZulongMessage[])[]>(() => {
		if (!showThinkingLoaderRow) {
			return groupedMessages
		}
		const waitingRow: ZulongMessage = {
