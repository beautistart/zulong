...
import { AskResponseRequest, NewTaskRequest } from "@shared/proto/zulong/task"
 ...
 */
export function useMessageHandlers(messages: ZulongMessage[], chatState: ChatState): MessageHandlers {
		...
		async (text: string, images: string[], files: string[]) => {
			...
			if (hasContent) {
				...
				if (messages.length === 0) {
					...
					if (zulongAsk === "resume_task" || zulongAsk === "resume_completed_task") {
					...
					// No zulongAsk set - check if task is actively running
