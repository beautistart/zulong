...
const getActivityText = (tool: ZulongSayTool): string | null => {
...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: ZulongMessage) => boolean,
...
const findCurrentApiReq = (messages: ZulongMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: ZulongMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = zulongMessages.some(
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, zulongMessages)
	}, [message.ts, zulongMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(zulongMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(zulongMessages, currentApiReq.index + 1, zulongMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return zulongMessages.some((msg, idx) => {
			if (msg.say === "tool" && isLowStakesTool(msg)) {
				...
				for (let i = idx - 1; i >= 0; i--) {
					const prevMsg = zulongMessages[i]
