...
// Mock data factories
const createMockMessage = (overrides: Partial<ZulongMessage> = {}): ZulongMessage => ({
	ts: Date.now(),
	type: "say",
	say: "error",
...
const meta: Meta<typeof ErrorRow> = {
	...
	parameters: {
		docs: {
			description: {
				component:
					"Displays different types of error messages in the chat interface, including API errors, credit limit errors, diff errors, and zulongignore errors. Handles special error parsing for Zulong provider errors and provides appropriate user actions.",
			},
		},
	},
...
export const Default: Story = {
	...
	argTypes: {
		errorType: {
			control: { type: "select" },
			options: ["error", "mistake_limit_reached", "diff_error", "zulongignore_error"],
...
// Zulong-specific errors
export const ZulongBalanceError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
...
export const ZulongRateLimitError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const ZulongSpendLimitDaily: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const ZulongSpendLimitMonthly: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const ZulongSpendLimitMinimal: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const AuthErrorSignedIn: Story = {
	...
	decorators: [
		createStoryDecorator({
			zulongUser: { id: "user123", email: "user@example.com" },
...
export const InteractiveSignIn: Story = {
	args: {
		...
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
			message: "Please sign in to access Zulong services.",
			code: "ERR_BAD_REQUEST",
	...
	play: async ({ canvasElement }) => {
		...
		const signInButton = canvas.getByRole("button", { name: /sign in to zulong/i })
