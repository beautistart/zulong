...
export const ErrorBlockTitle = ({
	...
	const title = (() => {
		...
		if (apiReqCancelReason === "user_cancelled") {
			...
			const zulongError = ZulongError.parse(apiRequestFailedMessage)
			const titleText = zulongError?.isErrorType(ZulongErrorType.Balance)
				? "Credit Limit Reached"
				: zulongError?.isErrorType(ZulongErrorType.SpendLimit)
					? "Spend Limit Reached"
					: "API Request Failed"
