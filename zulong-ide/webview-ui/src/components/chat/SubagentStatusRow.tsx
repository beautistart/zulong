...
function parseSubagentRowData(message: ZulongMessage): SubagentRowData | null {
	if (!message.text) {
		return null
	}
	...
	try {
		if (message.ask === "use_subagents" || message.say === "use_subagents") {
			const parsed = JSON.parse(message.text) as ZulongAskUseSubagents
		...
		const parsed = JSON.parse(message.text) as ZulongSaySubagentStatus
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "Zulong 想要使用子智能体：" : "Zulong 想要使用子智能体："
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{data.items.map((entry, index) => {
					...
					const statsText = `${formatCount(entry.toolCalls)} 工具调用 · ${formatCount(entry.contextTokens)} 令牌 · ${formatCost(entry.totalCost)}`
