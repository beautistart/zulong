...
import { BrowserAction, BrowserActionResult, ZulongMessage, ZulongSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: ZulongMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: ZulongMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: ZulongMessage[] = []
		let nextActionMessages: ZulongMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Zulong 正在使用浏览器：" : "Zulong 想要使用浏览器："}
				</span>
			</div>
			<div
				style={{
