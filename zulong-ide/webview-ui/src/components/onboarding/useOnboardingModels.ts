...
import { ZULONG_ONBOARDING_MODELS } from "@shared/zulong/onboarding"
import { EmptyRequest } from "@shared/proto/zulong/common"
import type { ZulongRecommendedModel } from "@shared/proto/zulong/models"
import type { OnboardingModel, OnboardingModelGroup } from "@shared/proto/zulong/state"
...
interface RecommendedModelsData {
	recommended: ZulongRecommendedModel[]
	free: ZulongRecommendedModel[]
}
...
export function useOnboardingModels(): UseOnboardingModelsResult {
	const { openRouterModels, zulongModels, refreshZulongModels } = useExtensionState()
	...
	useEffect(() => {
		...
		const refreshRecommendedModels = async () => {
			try {
				const response = await ModelsServiceClient.refreshZulongRecommendedModelsRpc(EmptyRequest.create({}))
				if (!cancelled) {
	...
	// Merge openRouter and zulong models into a single catalog for lookups
	const modelCatalog = useMemo<Record<string, ModelInfo>>(() => {
		return { ...openRouterModels, ...(zulongModels ?? {}) }
	}, [openRouterModels, zulongModels])

	return useMemo<UseOnboardingModelsResult>(() => {
		if (fetchState.status !== "success") {
			return { status: fetchState.status, models: { models: ZULONG_ONBOARDING_MODELS } }
		}
