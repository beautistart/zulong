import { UpdateTerminalConnectionTimeoutResponse } from "@shared/proto/index.zulong"
				...
				<div className="mb-5" id="terminal-settings-section">
					<div className="mb-4">
						...
						<p className="text-xs text-(--vscode-descriptionForeground) mt-1">
							选择 Zulong 使用的默认终端。'默认' 使用您的 VS Code 全局设置。
						</p>
					</div>
					...
					<div className="mb-4">
						...
						<p className="text-xs text-(--vscode-descriptionForeground)">
							设置 Zulong 等待 shell 集成激活后再执行命令的时间。如果遇到终端连接超时，请增加此值。
						</p>
					</div>
					...
					<div className="mb-4">
						...
						<p className="text-xs text-(--vscode-descriptionForeground)">
							启用后，Zulong 将复用不在当前工作目录的现有终端窗口。如果在终端命令后遇到任务锁定问题，请禁用此选项。
					...
					{isVsCodePlatform && (
						<div className="mb-4">
							...
							<p className="text-xs text-[var(--vscode-descriptionForeground)] mt-1">
								选择 Zulong 在 VS Code 终端还是后台进程中运行命令。
							</p>
						</div>
					)}
					...
					<div className="mt-5 p-3 bg-(--vscode-textBlockQuote-background) rounded border border-(--vscode-textBlockQuote-border)">
						<p className="text-[13px] m-0">
							...
							<a
								...
								href="https://docs.zulong.ai/troubleshooting/terminal-quick-fixes"
							...
							<a
								className="text-(--vscode-textLink-foreground) underline hover:no-underline"
								href="https://docs.zulong.ai/troubleshooting/terminal-integration-guide"
								rel="noopener noreferrer"
								target="_blank">
								完整故障排除指南
							</a>
