...
import type { UserInfo } from "@shared/proto/zulong/account"
import { EmptyRequest } from "@shared/proto/zulong/common"
import type { OpenRouterCompatibleModelInfo } from "@shared/proto/zulong/models"
import { OnboardingModelGroup, type TerminalProfile } from "@shared/proto/zulong/state"
import { convertProtoToZulongMessage } from "@shared/proto-conversions/zulong-message"
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	zulongModels: Record<string, ModelInfo> | null
	...
	setGlobalZulongRulesToggles: (toggles: Record<string, boolean>) => void
	setLocalZulongRulesToggles: (toggles: Record<string, boolean>) => void
...
export const ExtensionStateContextProvider: React.FC<{
	...
	const [state, setState] = useState<ExtensionState>({
		...
		globalZulongRulesToggles: {},
		localZulongRulesToggles: {},
		...
		zulongWebToolsEnabled: { user: true, featureFlag: false },
	...
	const [zulongModels, setZulongModels] = useState<Record<string, ModelInfo> | null>(null)
				...
				if (response.stateJson) {
					try {
						...
						setState((prevState) => {
							...
							// HACK: Preserve zulongMessages if currentTaskItem is the same
							if (stateData.currentTaskItem?.id === prevState.currentTaskItem?.id) {
								stateData.zulongMessages = stateData.zulongMessages?.length
									? stateData.zulongMessages
									: prevState.zulongMessages
							}
		...
		partialMessageUnsubscribeRef.current = UiServiceClient.subscribeToPartialMessage(EmptyRequest.create({}), {
			onResponse: (protoMessage) => {
				try {
					...
					const partialMessage = convertProtoToZulongMessage(protoMessage)
					setState((prevState) => {
						// worth noting it will never be possible for a more up-to-date message to be sent here or in normal messages post since the presentAssistantContent function uses lock
						const lastIndex = findLastIndex(prevState.zulongMessages, (msg) => msg.ts === partialMessage.ts)
						if (lastIndex !== -1) {
							const newZulongMessages = [...prevState.zulongMessages]
							newZulongMessages[lastIndex] = partialMessage
							return { ...prevState, zulongMessages: newZulongMessages }
						}
						return prevState
					})
				} catch (error) {
	...
	// Refresh Zulong models function
	const refreshZulongModels = useCallback(() => {
		ModelsServiceClient.refreshZulongModelsRpc(EmptyRequest.create({}))
			.then((response: OpenRouterCompatibleModelInfo) => {
				const models = fromProtobufModels(response.models)
				setZulongModels((prev) => (Object.keys(models).length > 0 ? models : (prev ?? null)))
			})
			.catch((error: Error) => console.error("Failed to refresh Zulong models:", error))
	}, [])

	// Auto-refresh Zulong models when provider is zulong
	useEffect(() => {
		const hasZulongProvider =
			state.apiConfiguration?.actModeApiProvider === "zulong" || state.apiConfiguration?.planModeApiProvider === "zulong"
		if (hasZulongProvider && zulongModels === null) {
			refreshZulongModels()
		}
	}, [state.apiConfiguration?.actModeApiProvider, state.apiConfiguration?.planModeApiProvider, zulongModels, refreshZulongModels])
	...
	const contextValue: ExtensionStateContextType = {
		...
		showWorktrees,
		showAnnouncement,
		globalZulongRulesToggles: state.globalZulongRulesToggles || {},
		localZulongRulesToggles: state.localZulongRulesToggles || {},
		...
		setShowMcp,
		closeMcpView,
		setGlobalZulongRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				globalZulongRulesToggles: toggles,
			})),
		setLocalZulongRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				localZulongRulesToggles: toggles,
			})),
