...
import { ZulongAuthContext, ZulongAuthContextType, ZulongAuthProvider, useZulongAuth } from "@/context/ZulongAuthContext"
import {
...
const ZulongAuthProviderWithOverrides: React.FC<{
	overrides?: Partial<ZulongAuthContextType>
	children: React.ReactNode
}> = ({ overrides, children }) => {
	const authContext = useZulongAuth()
	return <ZulongAuthContext.Provider value={{ ...authContext, ...overrides }}>{children}</ZulongAuthContext.Provider>
}

export const createStorybookDecorator =
	(overrideStates?: Partial<ExtensionStateContextType>, classNames?: string, authOverrides?: Partial<ZulongAuthContextType>) =>
	(Story: any) => (
		<ExtensionStateProviderWithOverrides overrides={overrideStates}>
			<ZulongAuthProviderWithOverrides overrides={authOverrides}>
				<div className={cn("max-w-lg mx-auto", classNames)}>
					<Story />
				</div>
			</ZulongAuthProviderWithOverrides>
