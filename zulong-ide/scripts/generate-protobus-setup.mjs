...
// Services whose controllers have been deleted (Zulong-only architecture)
// Proto files kept for message type definitions, but no handler code generated
const SKIP_SERVICES = new Set(["AccountService", "OcaAccountService"])
...
async function generateWebviewProtobusClients(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		if (SKIP_SERVICES.has(serviceName)) continue
		const rpcs = []
	...
	static override serviceName: string = "zulong.${serviceName}"
${rpcs.join("\n")}
}`)
	}
...
async function generateVscodeProtobusServers(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		servers.push(`} \n`)
		serviceMap.push(`    "zulong.${serviceName}": ${serviceName}Handlers,`)
		imports.push("")
	}
...
async function generateStandaloneProtobusServiceSetup(protobusServices) {
	...
	for (const [name, def] of Object.entries(protobusServices)) {
		...
		handlerSetup.push(`    server.addService(zulong.${name}Service, {`)
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			imports.push(`import { ${rpcName} } from "@core/controller/${dir}/${rpcName}"`)
			const requestType = "zulong." + rpc.requestType.type.name
			const responseType = "zulong." + rpc.responseType.type.name
			if (rpc.requestStream) {
				throw new Error("Request streaming is not supported")
			}
			if (rpc.responseStream) {
...
import { zulong } from "@generated/grpc-js"
import { Controller } from "@core/controller"
