...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "zulong-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .zulongrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const zulongrulesDirTest = path.join(tmpDir, "zulongrules-test")
		const zulongrulesDirPath = path.join(zulongrulesDirTest, ".zulongrules")

		// Create .zulongrules directory and root files
		await fs.mkdir(zulongrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(zulongrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(zulongrulesDirPath, "settings.js"), "// settings")

		// Create .zulongrules/other directory and files
		const otherDirPath = path.join(zulongrulesDirPath, "other")
		...
		// Create .zulongrules/workflows directory and files
		const workflowsDirPath = path.join(zulongrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(zulongrulesDirPath)
		...
		const filteredFiles = await readDirectory(zulongrulesDirPath, [[".zulongrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(zulongrulesDirPath, "config.json"),
			path.resolve(zulongrulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(zulongrulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(zulongrulesDirPath, "config.json"), path.resolve(zulongrulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .zulongrules/hooks directory specifically", async () => {
		// Create a test directory structure
		const zulongrulesDirTest = path.join(tmpDir, "zulongrules-hooks-test")
		const zulongrulesDirPath = path.join(zulongrulesDirTest, ".zulongrules")
		...
		// Create .zulongrules/hooks directory and files
		const hooksDirPath = path.join(zulongrulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(zulongrulesDirPath, [[".zulongrules", "hooks"]])
