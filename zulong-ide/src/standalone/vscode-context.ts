...
log("Running standalone zulong", ExtensionRegistryInfo.version)
log(`ZULONG_ENVIRONMENT: ${process.env.ZULONG_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(zulongDir?: string) {
	const ZULONG_DIR = zulongDir || process.env.ZULONG_DIR || `${os.homedir()}/.zulong`
	const DATA_DIR = path.join(ZULONG_DIR, SETTINGS_SUBFOLDER)
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in zulong.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in zulong.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in zulong.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in zulong.
