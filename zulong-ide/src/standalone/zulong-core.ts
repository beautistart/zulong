...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// zulong-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the zulong-core process by:
...
function showHelp() {
	Logger.log(`
Zulong Core - Standalone Server

Usage: node zulong-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Zulong data storage (default: ~/.zulong)
