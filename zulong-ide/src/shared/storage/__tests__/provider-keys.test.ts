...
describe("Provider key mapping", () => {
	...
	it("uses provider-specific model key behavior for Zulong", () => {
		expect(getProviderModelIdKey("zulong", "act")).to.equal("actModeZulongModelId")
		expect(getProviderModelIdKey("zulong", "plan")).to.equal("planModeZulongModelId")
	})
})
