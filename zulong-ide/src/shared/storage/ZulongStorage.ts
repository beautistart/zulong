...
export interface ZulongStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: ZulongStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and ZulongSyncStorage both satisfy this interface.
 */
export interface ZulongMemento {
 ...
 */
export abstract class ZulongStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "ZulongStorage"
	/**
 ...
 * Unlike ZulongStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: ZulongStorageChangeEvent) => void

export abstract class ZulongSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of ZulongStorage using a Map.
 */
export class ZulongInMemoryStorage extends ZulongStorage {
	/**
