...
import { ZulongStorage } from "./ZulongStorage"
import type { BlobStoreSettings } from "./types"

export type { BlobStoreSettings } from "./types"

/**
 * S3/R2/Azure blob storage implementation of ZulongStorage.
 * Uses AWS S3, Cloudflare R2, or Azure Blob Storage as the backend storage.
 */
export class ZulongBlobStorage extends ZulongStorage {
	override name = "ZulongBlobStorage"

	private static store: ZulongBlobStorage | null = null
	static get instance(): ZulongBlobStorage {
		if (!ZulongBlobStorage.store) {
			ZulongBlobStorage.store = new ZulongBlobStorage()
		}
		return ZulongBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!ZulongBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[ZulongBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[ZulongBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = ZulongBlobStorage.instance
