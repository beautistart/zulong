...
describe("Remote Config Schema", () => {
	...
	describe("ZulongSettingsSchema", () => {
		it("should accept valid Zulong provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = ZulongSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = ZulongSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => ZulongSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				ZulongSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			const config = {
				...
				providerSettings: {
					...
					Anthropic: {
						...
						baseUrl: "https://example.zulong.ai",
					},
				},
				enterpriseTelemetry: {
					promptUploading: {
			...
			// Verify Zulong settings
			expect(result.providerSettings?.Zulong?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.Zulong?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.Zulong?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
			...
			expect(result.providerSettings?.Anthropic?.baseUrl).to.equal("https://example.zulong.ai")
