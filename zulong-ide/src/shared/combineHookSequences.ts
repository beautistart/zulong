import { ZulongMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the ZulongSayHook interface but represents parsed data.
 */
...
function getSay(msg: ZulongMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: ZulongMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: ZulongMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: ZulongMessage[]): ZulongMessage[] {
...
function combineHookWithOutputs(
	hookMessage: ZulongMessage,
	startIndex: number,
	messages: ZulongMessage[],
): { combined: ZulongMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: ZulongMessage[]): ZulongMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, ZulongMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: ZulongMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: ZulongMessage[], originalMessages: ZulongMessage[]): Map<number, ZulongMessage[]> {
	const map = new Map<number, ZulongMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: ZulongMessage[], preToolUseMap: Map<number, ZulongMessage[]>): ZulongMessage[] {
	const result: ZulongMessage[] = []
 ...
 *
 * @param messages Array of ZulongMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: ZulongMessage[]): ZulongMessage[] {
