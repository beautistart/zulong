...
import { ZulongFeatureSetting } from "./ZulongFeatureSetting"
import { BannerCardData } from "./zulong/banner"
import { ZulongRulesToggles } from "./zulong-rules"
import { FocusChainSettings } from "./FocusChainSettings"
import { HistoryItem } from "./HistoryItem"
import { McpDisplayMode } from "./McpDisplayMode"
import { ZulongMessageModelInfo } from "./messages"
import { OnboardingModelGroup } from "./proto/zulong/state"
...
export const COMMAND_CANCEL_TOKEN = "__zulong_command_cancel__"
export interface ExtensionState {
	...
	version: string
	distinctId: string
	globalZulongRulesToggles: ZulongRulesToggles
	localZulongRulesToggles: ZulongRulesToggles
	localWorkflowToggles: ZulongRulesToggles
	globalWorkflowToggles: ZulongRulesToggles
	localCursorRulesToggles: ZulongRulesToggles
	localWindsurfRulesToggles: ZulongRulesToggles
	remoteRulesToggles?: ZulongRulesToggles
	remoteWorkflowToggles?: ZulongRulesToggles
	localAgentsRulesToggles: ZulongRulesToggles
	mcpResponsesCollapsed?: boolean
	strictPlanModeEnabled?: boolean
	yoloModeToggled?: boolean
	useAutoCondense?: boolean
	subagentsEnabled?: boolean
	zulongWebToolsEnabled?: ZulongFeatureSetting
	worktreesEnabled?: ZulongFeatureSetting
	...
	multiRootSetting: ZulongFeatureSetting
...
export interface ZulongSayTool {
	tool:
		| "editedExistingFile"
		| "newFileCreated"
		| "fileDeleted"
...
export interface ZulongSayHook {
...
export interface ZulongSayBrowserAction {
	action: BrowserAction
	coordinate?: string
	text?: string
}

export interface ZulongSayGenerateExplanation {
	title: string
	fromRef: string
	toRef: string
...
export interface ZulongSaySubagentStatus {
...
export interface ZulongAskUseMcpServer {
	serverName: string
...
export interface ZulongAskUseSubagents {
	prompts: string[]
}

export interface ZulongPlanModeResponse {
	response: string
	options?: string[]
	selected?: string
}

export interface ZulongAskQuestion {
	question: string
	options?: string[]
	selected?: string
}

export interface ZulongAskNewTask {
	context: string
}

export interface ZulongApiReqInfo {
	request?: string
	tokensIn?: number
	tokensOut?: number
	cacheWrites?: number
...
export interface ZulongSubagentUsageInfo {
	source: "subagents"
	tokensIn: number
	tokensOut: number
	cacheWrites: number
...
export type ZulongApiReqCancelReason = "streaming_failed" | "user_cancelled" | "retries_exhausted"
