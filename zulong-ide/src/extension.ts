...
import { createZulongAPI } from "./exports"
...
import { addToZulong } from "./core/controller/commands/addToZulong"
import { explainWithZulong } from "./core/controller/commands/explainWithZulong"
import { fixWithZulong } from "./core/controller/commands/fixWithZulong"
import { improveWithZulong } from "./core/controller/commands/improveWithZulong"
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// After this, all platforms (VSCode, CLI, JetBrains) read from ~/.zulong/data/.
	...
	*/
	...
	const handleUri = async (uri: vscode.Uri) => {
		...
		if (isTaskUri) {
			await openZulongSidebarForTaskUri()
		}
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "zulong.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[Zulong Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log("[Zulong Dev] Failed to register dev commands: " + error)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Zulong (Always available)
					const addAction = new vscode.CodeAction("Add to Zulong", vscode.CodeActionKind.QuickFix)
					addAction.command = {
					...
					// Explain with Zulong (Always available)
					const explainAction = new vscode.CodeAction("Explain with Zulong", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with Zulong",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Zulong (Always available)
					const improveAction = new vscode.CodeAction("Improve with Zulong", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with Zulong",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Zulong (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Zulong", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithZulong,
							title: "Fix with Zulong",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.AddToChat, async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			...
			await addToZulong(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.FixWithZulong, async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
			await fixWithZulong(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ExplainCode, async (range: vscode.Range) => {
			...
			await explainWithZulong(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ImproveCode, async (range: vscode.Range) => {
			...
			await improveWithZulong(context.controller, context.commandContext)
		}),
	)
...
\`\`\``
				...
				await addToZulong(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
				...
				await explainWithZulong(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
...
\`\`\``
				...
				await improveWithZulong(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	const unsubSecrets = storageContext.secrets.onDidChange((event) => {
		if (event.key === "zulong:zulongAccountId") {
	...
	Logger.log(`[Zulong] extension activated in ${performance.now() - activationStartTime} ms`)

	return createZulongAPI(webview.controller)
}
...
function setupHostProvider(context: ExtensionContext) {
	...
	outputChannel.appendLine("[Zulong] Setting up VS Code host...")
...
async function openZulongSidebarForTaskUri(): Promise<void> {
	...
	Logger.warn("Task URI handling timed out waiting for Zulong sidebar visibility")
}
...
async function cleanupLegacyVSCodeStorage(context: ExtensionContext): Promise<void> {
	try {
		...
		// Migrate custom instructions to global Zulong rules (one-time cleanup)
