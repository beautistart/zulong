...
describe("Zulong Extension", () => {
	...
	it("should verify extension ID matches package.json", async () => {
		...
		const zulongExtensionApi = vscode.extensions.getExtension(id)

		zulongExtensionApi?.id.should.equal(id)
	})

	it("should successfully execute the plus button command", async () => {
		await new Promise((resolve) => setTimeout(resolve, 400))
		await vscode.commands.executeCommand("zulong.plusButtonClicked")
	})
