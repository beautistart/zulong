 ...
 * to prevent race conditions, particularly the TOCTOU bug in addToZulongMessages
 */
describe("MessageStateHandler Mutex Protection", () => {
	...
	/**
	 * Helper to create a test ZulongMessage
	 */
	function createTestMessage(text: string): ZulongMessage {
		return {
			ts: Date.now(),
			type: "say",
			say: "text",
	...
	it("should initialize with empty message arrays", () => {
		...
		handler.getZulongMessages().length.should.equal(0)
	...
	it("should set and get zulong messages", () => {
		const handler = createTestHandler()
		const testMessages = [createTestMessage("test1"), createTestMessage("test2")]

		handler.setZulongMessages(testMessages)
		handler.getZulongMessages().should.deepEqual(testMessages)
	})

	/**
	 * CRITICAL TEST: Verify that addToZulongMessages is atomic
	 * This test simulates the race condition that can occur when multiple
	 * addToZulongMessages calls happen concurrently without proper mutex protection
	 */
	it("should handle concurrent addToZulongMessages atomically", async function () {
		...
		await handler.addToZulongMessages(initialMsg)

		// Verify initial state
		const messages = handler.getZulongMessages()
		...
		const results = await Promise.all([
			handler.addToZulongMessages(msg1),
			handler.addToZulongMessages(msg2),
			handler.addToZulongMessages(msg3),
		])
		...
		// Get final state
		const finalMessages = handler.getZulongMessages()
	...
	/**
	 * Test that updateZulongMessage operations are atomic
	 */
	it("should handle concurrent updateZulongMessage atomically", async function () {
		this.timeout(5000)
		...
		handler.setZulongMessages(msgs)

		// Perform concurrent updates to different messages
		await Promise.all([
			handler.updateZulongMessage(0, { text: "updated1" }),
			handler.updateZulongMessage(1, { text: "updated2" }),
			handler.updateZulongMessage(2, { text: "updated3" }),
		])
	...
	/**
	 * Test that deleteZulongMessage operations are atomic
	 */
	it("should handle deleteZulongMessage with proper validation", async () => {
		const handler = createTestHandler()
		...
		await handler.deleteZulongMessage(1)
	 ...
	 */
	it("should throw error for invalid message index in updateZulongMessage", async () => {
		const handler = createTestHandler()
		handler.setZulongMessages([createTestMessage("msg1")])

		try {
			await handler.updateZulongMessage(5, { text: "invalid" })
	...
	/**
	 * Test that invalid indices are rejected in deleteZulongMessage
	 */
	it("should throw error for invalid message index in deleteZulongMessage", async () => {
		const handler = createTestHandler()
		handler.setZulongMessages([createTestMessage("msg1")])
	 ...
	 */
	it("should handle overwriteZulongMessages atomically", async () => {
		const handler = createTestHandler()

		// Set initial messages
		handler.setZulongMessages([createTestMessage("old1"), createTestMessage("old2")])

		// Overwrite with new messages
		const newMessages = [createTestMessage("new1"), createTestMessage("new2"), createTestMessage("new3")]
		await handler.overwriteZulongMessages(newMessages)
