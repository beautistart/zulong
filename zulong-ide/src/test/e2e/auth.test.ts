...
e2e("Views - can set up API keys and navigate to Settings from Chat", async ({ sidebar }) => {
	...
	await expect(sidebar.getByRole("button", { name: "Login to Zulong" })).toBeVisible()
	...
	// Test Zulong provider option
	await providerSelectorInput.click({ delay: 100 })
	// Wait for dropdown to appear and find Zulong option
	await expect(sidebar.getByTestId("provider-option-zulong")).toBeVisible()
	await sidebar.getByTestId("provider-option-zulong").click({ delay: 100 })
	await expect(sidebar.getByRole("button", { name: "Sign Up with Zulong" })).toBeVisible()
	...
	await expect(sidebar.getByRole("button", { name: "Login to Zulong" })).not.toBeVisible()
	...
	const kanbanDialog = sidebar.getByRole("heading", {
		name: "Introducing Zulong Kanban",
	})
	try {
	...
	// zulong logo container
	const zulongLogo = sidebar.locator(".size-20")
	await expect(zulongLogo).toBeVisible()
