...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/ZulongAccount"
import { E2E_MOCK_API_RESPONSES, E2E_REGISTERED_MOCK_ENDPOINTS } from "./api"
import { ZulongDataMock } from "./data"

const E2E_API_SERVER_PORT = 7777

export const MOCK_ZULONG_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

const useVerboseLogging = process.env.ZULONG_E2E_TESTS_VERBOSE === "true"
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[ZulongApiServerMock]", ...args)
	}
}

export class ZulongApiServerMock {
	static globalSharedServer: ZulongApiServerMock | null = null
	...
	public readonly API_USER = new ZulongDataMock("personal")
	...
	public static async startGlobalServer(): Promise<ZulongApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (ZulongApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return ZulongApiServerMock.globalSharedServer
		}

		log("Starting global server...")
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			// Parse URL and method
			const parsedUrl = new URL(req.url || "/", MOCK_ZULONG_API_SERVER_URL)
			...
			if (isAuthRequired && authToken) {
				...
				const user = ZulongApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				ZulongApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = ZulongApiServerMock.matchRoute(path, method)
				...
				const controller = ZulongApiServerMock.globalSharedServer!
				...
				if (baseRoute === "/api/v1") {
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching ZulongAuthProvider expectations
						return sendApiResponse({
					...
					if (endpoint === "/chat/completions" && method === "POST") {
						...
						if (body.includes("[diff.test.ts] Hello, Zulong!")) {
							...
							choices: [
								{
									...
									message: {
										role: "assistant",
										content: "Hello! I'm a mock Zulong API response.",
									},
									finish_reason: "stop",
								},
							],
		...
		const controller = new ZulongApiServerMock(server)
		ZulongApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			ZulongApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				ZulongApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`ZulongApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = ZulongApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		ZulongApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		ZulongApiServerMock.globalSockets.clear()
		...
		ZulongApiServerMock.globalSharedServer = null
	}
}
