...
import { ZulongApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public async signin(webview: Frame): Promise<void> {
		await webview.getByRole("button", { name: "Login to Zulong" }).click({ delay: 100 })

		// Verify start up page is no longer visible
		await expect(webview.getByRole("button", { name: "Login to Zulong" })).not.toBeVisible()

		await webview.getByRole("button", { name: "Close" }).click({ delay: 50 })
	}

	public static async openZulongSidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /Zulong/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the Zulong extension.
 *
 * Extended Playwright test configuration for Zulong E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the Zulong VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared ZulongApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with Zulong sidebar opened
 * - `sidebar`: Playwright Frame object representing the Zulong extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a ZulongApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and Zulong sidebar open
 * - **sidebar**: Provides access to the Zulong extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the Zulong extension in development mode
...
export const e2e = test
	.extend<{ server: ZulongApiServerMock | null }>({
		server: async ({}, use) => {
			// Start server if it doesn't exist
			if (!ZulongApiServerMock.globalSharedServer) {
				await ZulongApiServerMock.startGlobalServer()
			}
			await use(ZulongApiServerMock.globalSharedServer)
		},
	})
	...
	.extend<{ openVSCode: (workspacePath: string) => Promise<ElectronApplication> }>({
		openVSCode: async ({ userDataDir, channel }, use, testInfo) => {
			...
			await use(async (workspacePath: string) => {
				// Create isolated Zulong data directory for this test
				const zulongTestDir = mkdtempSync(path.join(os.tmpdir(), "zulong-e2e-"))
				...
				const app = await _electron.launch({
					...
					env: {
						...
						TEMP_PROFILE: "true",
						E2E_TEST: "true",
						ZULONG_ENVIRONMENT: "local",
						ZULONG_DIR: zulongTestDir, // Isolate test data from user's ~/.zulong
	...
	.extend<{ app: ElectronApplication; zulongTestDir: string }>({
		app: async ({ openVSCode, userDataDir, extensionsDir, workspaceType, workspaceDir, multiRootWorkspaceDir }, use) => {
			...
			// Track the zulongTestDir created in openVSCode
			let zulongTestDir: string | undefined
			const originalOpenVSCode = openVSCode
			const wrappedOpenVSCode = async (wp: string) => {
				const app = await originalOpenVSCode(wp)
				// Extract ZULONG_DIR from the launched app's environment
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openZulongSidebar(page)
