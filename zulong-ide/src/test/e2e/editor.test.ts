...
import { addSelectedCodeToZulongWebview, openTab, toggleNotifications } from "./utils/common"
...
e2e.describe("Code Actions and Editor Panel", () => {
	E2E_WORKSPACE_TYPES.forEach(({ title, workspaceType }) => {
		e2e.extend({
			...
			// Sidebar should be opened and visible after adding code to Zulong
			await addSelectedCodeToZulongWebview(page)
