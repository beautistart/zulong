...
describe("ZulongAccountService.fetchUserRemoteConfig", () => {
	let service: ZulongAccountService
	...
	beforeEach(() => {
		...
		service = new ZulongAccountService()
	})
...
describe("ZulongAccountService.fetchFeaturebaseToken", () => {
	let service: ZulongAccountService
	let sandbox: sinon.SinonSandbox
	...
	it("returns undefined when the request throws due to missing auth token", async () => {
		sandbox
			.stub(service as unknown as { authenticatedRequest: () => unknown }, "authenticatedRequest")
			.rejects(new Error("No Zulong account auth token found"))
