...
import { ZulongEndpoint, ZulongEnv } from "@/config"
...
describe("Controller Marketplace Filtering", () => {
	...
	// Initialize ZulongEndpoint before tests run (required for ZulongEnv.config() to work)
	before(async () => {
		if (!ZulongEndpoint.isInitialized()) {
			await ZulongEndpoint.initialize("/test/extension")
		}
	})
	...
	describe("API interaction", () => {
		it("should call marketplace API with correct parameters", async () => {
			...
			callArgs[0].should.equal(`${ZulongEnv.config().mcpBaseUrl}/marketplace`)
		})
