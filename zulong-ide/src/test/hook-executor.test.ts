...
describe("Hook Executor", () => {
	...
	let mockMessages: ZulongMessage[]
...
`
	...
	beforeEach(async () => {
		...
		// Create .zulongrules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".zulongrules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.zulongrules/hooks/
	...
	describe("Error Handling", () => {
		...
		it("should update message state on hook failure", async function () {
			...
			const mockHandler = {
				...testHandler,
				getZulongMessages: () => messages,
				addToZulongMessages: async (msg: ZulongMessage) => {
					messages.push(msg)
				},
				updateZulongMessage: async (index: number, updates: Partial<ZulongMessage>) => {
					if (messages[index]) {
