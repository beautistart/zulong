/**
 * ZulongTempManager - Manages temporary files for Zulong with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "zulong" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Zulong's temporary files.
 */
class ZulongTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "zulong" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/zulong
		// Windows: C:\Users\{user}\AppData\Local\Temp\zulong
		// Linux: /tmp/zulong
		const baseTempDir = os.tmpdir()
		const zulongTempDir = path.join(baseTempDir, "zulong")

		try {
			fs.mkdirSync(zulongTempDir, { recursive: true })
			this.tempDir = zulongTempDir
		} catch {
	...
	/**
	 * Clean up old Zulong temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Zulong temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Zulong temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Zulong temp cleanup", error)
		}
...
export const ZulongTempManager = new ZulongTempManagerImpl()
