...
import { ZulongEnv, EnvironmentConfig } from "@/config"
import { Controller } from "@/core/controller"
import { HostProvider } from "@/hosts/host-provider"
import { buildBasicZulongHeaders } from "@/services/EnvUtils"
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/ZulongError"
import { telemetryService } from "@/services/telemetry"
import { ZULONG_API_ENDPOINT } from "@/shared/zulong/api"
import { fetch, getAxiosSettings } from "@/shared/net"
import { Logger } from "@/shared/services/Logger"
import { type ZulongAccountUserInfo, type ZulongAuthInfo } from "../AuthService"
import { parseJwtPayload } from "../oca/utils/utils"

interface ZulongAuthApiUser {
	subject: string | null
	email: string
	name: string
	zulongUserId: string | null
	accounts: string[] | null
}

// Unified API response data shape for token exchange/refresh
interface ZulongAuthResponseData {
	/**
...
export interface ZulongAuthApiTokenExchangeResponse {
	success: boolean
	data: ZulongAuthResponseData
}

export interface ZulongAuthApiTokenRefreshResponse {
	success: boolean
	data: ZulongAuthResponseData
}

export class ZulongAuthProvider {
	readonly name = "zulong"
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: ZulongAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("zulong:zulongAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: ZulongAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Zulong auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<ZulongAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveZulongAuthInfo(controller: Controller): Promise<ZulongAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("zulong:zulongAccountId")
			...
			let storedAuthData: ZulongAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("zulongAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("zulong:zulongAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Zulong, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<ZulongAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: ZulongAuthInfo): Promise<ZulongAuthInfo> {
		try {
			const endpoint = new URL(ZULONG_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: ZulongAuthApiTokenExchangeResponse = await response.json()
	...
	async getAuthRequest(callbackUrl: string): Promise<string> {
		const authUrl = new URL(ZULONG_API_ENDPOINT.AUTH, this.config.apiBaseUrl)
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<ZulongAuthInfo | null> {
		try {
			...
			const tokenUrl = new URL(ZULONG_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: ZulongAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("zulong:zulongAccountId", JSON.stringify(zulongAuthInfo))

			return zulongAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: ZulongAuthApiTokenExchangeResponse["data"]): Promise<ZulongAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${ZulongEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.zulongUserId || "",
