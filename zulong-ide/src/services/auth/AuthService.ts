import { AuthState, UserInfo } from "@shared/proto/zulong/account"
import { type EmptyRequest, String } from "@shared/proto/zulong/common"
...
import { AuthInvalidTokenError, AuthNetworkError } from "../error/ZulongError"
import { featureFlagsService } from "../feature-flags"
import { ZulongAuthProvider } from "./providers/ZulongAuthProvider"
...
export interface ZulongAuthInfo {
	/**
	 * accessToken
	 */
	idToken: string
	 ...
	 */
	expiresAt?: number
	userInfo: ZulongAccountUserInfo
	provider: string
	startedAt?: number
}

export interface ZulongAccountUserInfo {
	createdAt: string
	displayName: string
	email: string
	id: string
	organizations: ZulongAccountOrganization[]
	/**
	 * Zulong app base URL, used for webview UI and other client-side operations
	 */
	appBaseUrl?: string
	/**
...
export interface ZulongAccountOrganization {
	active: boolean
	memberId: string
	name: string
...
export class AuthService {
	...
	protected _zulongAuthInfo: ZulongAuthInfo | null = null
	protected _provider: ZulongAuthProvider
	...
	protected constructor(controller: Controller) {
		this._provider = new ZulongAuthProvider()
	...
	getActiveOrganizationId(): string | null {
		if (!this._zulongAuthInfo?.userInfo?.organizations) {
			return null
		}
		const activeOrg = this._zulongAuthInfo.userInfo.organizations.find((org) => org.active)
	 ...
	 */
	getUserOrganizations(): ZulongAccountOrganization[] | undefined {
		return this._zulongAuthInfo?.userInfo?.organizations
	}

	private async internalGetAuthToken(provider: ZulongAuthProvider): Promise<string | null> {
		try {
			let zulongAccountAuthToken = this._zulongAuthInfo?.idToken
			if (!this._zulongAuthInfo || !zulongAccountAuthToken || this._zulongAuthInfo.provider !== provider.name) {
				// Not authenticated
				return null
			}

			// Check if token has expired
			if (await provider.shouldRefreshIdToken(zulongAccountAuthToken, this._zulongAuthInfo.expiresAt)) {
				...
				this._refreshPromise = (async () => {
					...
					try {
						const updatedAuthInfo = await provider.retrieveZulongAuthInfo(this._controller)
						if (updatedAuthInfo) {
							this._zulongAuthInfo = updatedAuthInfo
							this._authenticated = true
							zulongAccountAuthToken = updatedAuthInfo.idToken
					...
					return zulongAccountAuthToken
				})()

				zulongAccountAuthToken = await this._refreshPromise
			}

			return zulongAccountAuthToken || null
		} catch (error) {
	 ...
	 * @returns The provider name (e.g., "zulong", "firebase"), or null if not authenticated
	 */
	getProviderName(): string | null {
		return this._zulongAuthInfo?.provider ?? null
	}
	...
	getInfo(): AuthState {
		...
		let user: any = null
		if (this._zulongAuthInfo && this._authenticated) {
			const userInfo = this._zulongAuthInfo.userInfo
			this._zulongAuthInfo.userInfo.appBaseUrl = ZulongEnv.config()?.appBaseUrl
	...
	async handleAuthCallback(authorizationCode: string, provider: string): Promise<void> {
		try {
			this._zulongAuthInfo = await this._provider.signIn(this._controller, authorizationCode, provider)
			this._authenticated = this._zulongAuthInfo?.idToken !== undefined
	...
	async restoreRefreshTokenAndRetrieveAuthInfo(): Promise<void> {
		try {
			this._zulongAuthInfo = await this.retrieveAuthInfo()
	...
	private async retrieveAuthInfo(): Promise<ZulongAuthInfo | null> {
		// If a refresh is already in progress, wait for it to complete
		if (this._refreshPromise) {
			Logger.info("Token refresh already in progress, waiting for completion")
		...
		return this._provider.retrieveZulongAuthInfo(this._controller)
	}
	...
	async sendAuthStatusUpdate(): Promise<void> {
		...
		if (this._zulongAuthInfo?.userInfo?.id) {
			telemetryService.identifyAccount(this._zulongAuthInfo.userInfo)
			// Poll feature flags immediately for authenticated users to ensure cache is populated
			await featureFlagsService.poll(this._zulongAuthInfo.userInfo?.id)
		} else {
		...
		BannerService.onAuthUpdate(this._zulongAuthInfo?.userInfo?.id || null).catch((error) => {
	...
	private destroyTokens() {
		this._controller.stateManager.setSecret("zulongAccountId", undefined)
		this._controller.stateManager.setSecret("zulong:zulongAccountId", undefined)
	}
}
