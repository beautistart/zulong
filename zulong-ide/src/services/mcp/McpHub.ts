...
export class McpHub {
	...
	isConnecting = false
	/**
	 * Flag to skip file watcher processing when we're updating Zulong-specific settings
	 ...
	 */
	private isUpdatingZulongSettings = false
	...
	private async watchMcpSettingsFile(): Promise<void> {
		...
		this.settingsWatcher.on("change", async () => {
			...
			// Skip processing if we're updating Zulong-specific settings (autoApprove, timeout)
			if (this.isUpdatingZulongSettings) {
				return
			}
	...
	async updateServerConnectionsRPC(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		for (const [name, config] of Object.entries(newServers)) {
			...
			if (!currentConnection) {
				...
				// Existing server with changed connection config (excludes Zulong-specific settings)
				try {
					if (config.type === "stdio") {
						this.setupFileWatcher(name, config)
					}
				...
				// Only Zulong-specific settings changed - update in-memory state without restart
				...
				// Also update Zulong-specific settings in the stored config.
	...
	async updateServerConnections(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		// Track if any connection-level changes occurred (excludes Zulong-specific settings)
		...
		// For Zulong-specific settings changes, the RPC response already updated the webview,
	 ...
	 * Excludes Zulong-specific settings since they don't affect the MCP server transport connection.
	 *
	 * ## Zulong-specific settings (don't require restart):
	 ...
	 *
	 * ## Adding new Zulong-specific settings:
	 ...
	 * 2. Add it to `isUpdatingZulongSettings` flag usage in the update function
	...
	private configsRequireRestart(oldConfig: McpServerConfig, newConfig: McpServerConfig): boolean {
		// Exclude Zulong-specific settings from comparison (add new ones here)
		const {
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { type: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or Zulong (and we want to detect save events, not every file change)
	...
	async toggleToolAutoApproveRPC(serverName: string, toolNames: string[], shouldAllow: boolean): Promise<McpServer[]> {
		...
		this.isUpdatingZulongSettings = true
		try {
			const settingsPath = await getMcpSettingsFilePathHelper(await this.getSettingsDirectoryPath())
			const content = await fs.readFile(settingsPath, "utf-8")
			const config = JSON.parse(content)
			...
			setTimeout(() => {
				this.isUpdatingZulongSettings = false
			}, 300)
		}
	}
