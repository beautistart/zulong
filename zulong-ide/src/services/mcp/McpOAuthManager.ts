...
/**
 * Implementation of OAuthClientProvider for Zulong
 * Manages OAuth state and token storage for a single MCP server
 */
class ZulongOAuthClientProvider implements OAuthClientProvider {
...
export class McpOAuthManager {
	...
	async getOrCreateProvider(serverName: string, serverUrl: string): Promise<OAuthClientProvider> {
		...
		// Create provider
		const provider = new ZulongOAuthClientProvider(serverName, serverUrl)
