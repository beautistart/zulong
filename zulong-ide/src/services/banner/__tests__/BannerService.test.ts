...
import type { BannerRules } from "@shared/ZulongBanner"
import { expect } from "chai"
import { afterEach, beforeEach, describe, it } from "mocha"
import * as sinon from "sinon"
import { ZulongEnv, Environment } from "@/config"
...
describe("BannerService", () => {
	...
	beforeEach(() => {
		...
		// Mock ZulongEnv.config() to return a valid config
		sandbox.stub(ZulongEnv, "config").returns({
			environment: Environment.production,
			appBaseUrl: "https://app.zulong-mock.bot",
			apiBaseUrl: "https://api.zulong-mock.bot",
			mcpBaseUrl: "https://api.zulong-mock.bot/v1/mcp",
		})
	...
	describe("IDE Type Detection", () => {
		...
		it('should return "jetbrains" when ide is "Zulong for JetBrains" (case-insensitive)', async () => {
			stubHostInfo({ ide: "Zulong for JetBrains" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
		...
		it("should prefer ide field over platform field for detection", async () => {
			stubHostInfo({ ide: "Zulong for JetBrains", platform: "Visual Studio Code 1.103.0" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
