...
import type { ZulongAccountUserInfo } from "@/services/auth/AuthService"
...
function makeUserInfo(
	overrides: Partial<ZulongAccountUserInfo> & { orgOverrides?: Record<string, unknown> } = {},
): ZulongAccountUserInfo {
...
describe("OpenTelemetryTelemetryProvider.identifyUser", () => {
	...
	it("should handle user with no active organization", () => {
		...
		const userInfo: ZulongAccountUserInfo = {
			id: "user-1",
	...
	it("should clear stale org attributes when switching from org to no-org", () => {
		...
		const userNoOrg: ZulongAccountUserInfo = {
			id: "user-1",
			displayName: "Test User",
			email: "test@example.com",
			createdAt: new Date().toISOString(),
