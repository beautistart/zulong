...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the Zulong extension.
	 * this is specifically to avoid capturing errors from anything other than Zulong
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from Zulong
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "zulong"
			if (stacktrace?.value?.toLowerCase().includes("zulong")) {
				return event
			}
			...
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					...
					// The CLI filename will include "zulong"
					if (fileName?.includes("saoudrizwan") || fileName?.includes("zulong")) {
						return event
					}
				}
			}
