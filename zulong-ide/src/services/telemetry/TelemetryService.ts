...
import { ApiFormat, apiFormatToJSON } from "@shared/proto/zulong/models"
import { ShowMessageType } from "@shared/proto/host/window"
import type { TaskFeedbackType } from "@shared/WebviewMessage"
import * as os from "os"
import { ZulongAccountUserInfo } from "@/services/auth/AuthService"
...
export type TelemetryMetadata = {
	/**
	 * The extension or zulong-core version. JetBrains and CLI have different
	 * versioning than the VSCode Extension, but on those platforms this will be the _zulong-core version_
	 * which uses the same as the versioning as the VSCode extension.
	 */
	extension_version: string
	/**
	 * The type of zulong distribution, e.g VSCode Extension, JetBrains Plugin or CLI. This
...
/**
 * TelemetryService handles telemetry event tracking for the Zulong extension
...
export class TelemetryService {
	...
	public static readonly METRICS = {
		TASK: {
			TURNS_TOTAL: "zulong.turns.total",
			TURNS_PER_TASK: "zulong.turns.per_task",
			TOKENS_INPUT_TOTAL: "zulong.tokens.input.total",
			TOKENS_INPUT_PER_RESPONSE: "zulong.tokens.input.per_response",
			TOKENS_OUTPUT_TOTAL: "zulong.tokens.output.total",
			TOKENS_OUTPUT_PER_RESPONSE: "zulong.tokens.output.per_response",
			COST_TOTAL: "zulong.cost.total",
			COST_PER_EVENT: "zulong.cost.per_event",
		},
		CACHE: {
			WRITE_TOTAL: "zulong.cache.write.tokens.total",
			WRITE_PER_EVENT: "zulong.cache.write.tokens.per_event",
			READ_TOTAL: "zulong.cache.read.tokens.total",
			READ_PER_EVENT: "zulong.cache.read.tokens.per_event",
			HITS_TOTAL: "zulong.cache.hits.total",
		},
		TOOLS: {
			CALLS_TOTAL: "zulong.tool.calls.total",
			CALLS_PER_TASK: "zulong.tool.calls.per_task",
		},
		ERRORS: {
			TOTAL: "zulong.errors.total",
			PER_TASK: "zulong.errors.per_task",
		},
		API: {
			TTFT_SECONDS: "zulong.api.ttft.seconds",
			DURATION_SECONDS: "zulong.api.duration.seconds",
			THROUGHPUT_TOKENS_PER_SECOND: "zulong.api.throughput.tokens_per_second",
		},
		HOOKS: {
			EXECUTIONS_TOTAL: "zulong.hooks.executions.total",
			DURATION_SECONDS: "zulong.hooks.duration.seconds",
			FAILURES_TOTAL: "zulong.hooks.failures.total",
			CANCELLATIONS_TOTAL: "zulong.hooks.cancellations.total",
			CONTEXT_MODIFICATIONS_TOTAL: "zulong.hooks.context_modifications.total",
			CACHE_ACCESSES_TOTAL: "zulong.hooks.cache.accesses.total",
		},
		AI_OUTPUT: {
			ACCEPTED_LINES_ADDED: "zulong.ai_output.accepted.lines_added.total",
			ACCEPTED_LINES_DELETED: "zulong.ai_output.accepted.lines_deleted.total",
			ACCEPTED_LINES_CHANGED: "zulong.ai_output.accepted.lines_changed.total",
			ACCEPTED_FILES_CREATED: "zulong.ai_output.accepted.files_created.total",
			ACCEPTED_FILES_DELETED: "zulong.ai_output.accepted.files_deleted.total",
			ACCEPTED_FILES_MOVED: "zulong.ai_output.accepted.files_moved.total",
			REJECTED_LINES_ADDED: "zulong.ai_output.rejected.lines_added.total",
			REJECTED_LINES_DELETED: "zulong.ai_output.rejected.lines_deleted.total",
			REJECTED_LINES_CHANGED: "zulong.ai_output.rejected.lines_changed.total",
			REJECTED_FILES_CREATED: "zulong.ai_output.rejected.files_created.total",
			REJECTED_FILES_DELETED: "zulong.ai_output.rejected.files_deleted.total",
			REJECTED_FILES_MOVED: "zulong.ai_output.rejected.files_moved.total",
		},
		GRPC: {
			RESPONSE_SIZE_BYTES: "zulong.grpc.response.size_bytes",
		},
	}
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual Zulong rules are toggled on/off
			...
			// Tracks when Zulong web tools setting is toggled on/off
			ZULONG_WEB_TOOLS_TOGGLED: "task.zulong_web_tools_toggled",
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (hostSetting.isEnabled === Setting.DISABLED) {
			// Only show warning if user has opted in to Zulong telemetry but host telemetry is disabled
			if (didUserOptIn) {
				void HostProvider.window
					.showMessage({
						type: ShowMessageType.WARNING,
						message:
							"Anonymous Zulong error and usage reporting is enabled, but IDE telemetry is disabled. To enable error and usage reporting for this extension, enable telemetry in IDE settings.",
						options: {
							items: ["Open Settings"],
						},
					})
	 ...
	 */
	public identifyAccount(userInfo: ZulongAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when zulong calls the task completion_result tool signifying that zulong is done with the task
	 ...
	 * @param provider The API provider identifier (e.g. "anthropic", "openai", "zulong")
	...
	/**
	 * Records when individual Zulong rules are toggled on/off
	 ...
	 */
	public captureZulongRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
	...
	/**
	 * Records when Zulong web tools are enabled/disabled by the user
	 * @param ulid Unique identifier for the task
	 * @param enabled Whether Zulong web tools are enabled (true) or disabled (false)
	 */
	public captureZulongWebToolsToggle(ulid: string, enabled: boolean) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.ZULONG_WEB_TOOLS_TOGGLED,
			properties: {
				ulid,
				enabled,
			},
	...
	public captureWorkspaceInitialized(
		...
		this.recordGauge("zulong.workspace.active_roots", rootCount, {
			is_multi_root: isMultiRoot,
		})
		// Retire the previous series to avoid leaking gauge entries when the flag flips.
		this.recordGauge("zulong.workspace.active_roots", null, {
			is_multi_root: !isMultiRoot,
		})
	}
