import { getSavedApiConversationHistory, getSavedZulongMessages } from "@core/storage/disk"
...
export async function createTestServer(controller: Controller): Promise<http.Server> {
	// Try to show the Zulong sidebar
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Zulong instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "zulong" as ApiProvider,
							zulongAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.stateManager.setSecret("zulongAccountId", apiKey)

						visibleWebview.controller.stateManager.setApiConfiguration(updatedConfig)

						// Update cache service to use zulong provider
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedZulongMessages(taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Zulong messages: ${error}`)
						}
