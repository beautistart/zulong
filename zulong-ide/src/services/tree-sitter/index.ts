...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = zulongIgnoreController ? zulongIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, zulongIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (zulongIgnoreController && !zulongIgnoreController.validateAccess(filePath)) {
		return null
	}
