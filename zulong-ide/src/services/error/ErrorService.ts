...
import { ZulongError } from "./ZulongError"
import { ErrorProviderFactory } from "./ErrorProviderFactory"
import { IErrorProvider } from "./providers/IErrorProvider"

/**
 * ErrorService handles error logging and tracking for the Zulong extension
...
export class ErrorService {
	...
	captureException(error: Error | ZulongError, properties?: Record<string, unknown>) {
		return this.provider.captureException(error, properties)
	}

	public logException(error: Error | ZulongError, properties?: Record<string, unknown>): void {
	...
	public toZulongError(rawError: unknown, modelId?: string, providerId?: string): ZulongError {
		const transformed = ZulongError.transform(rawError, modelId, providerId)
