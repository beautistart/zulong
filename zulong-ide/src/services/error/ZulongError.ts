...
import { ZULONG_ACCOUNT_AUTH_ERROR_MESSAGE } from "../../shared/ZulongAccount"

export enum ZulongErrorType {
	Auth = "auth",
	Network = "network",
	RateLimit = "rateLimit",
	Balance = "balance",
...
export class ZulongError extends Error {
	readonly title = "ZulongError"
	...
	/**
	 * Parses a stringified error into a ZulongError instance.
	 */
	static parse(errorStr?: string, modelId?: string): ZulongError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return ZulongError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a ZulongError instance.
	 * Always returns a ZulongError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): ZulongError {
		try {
			// If already a ZulongError, return it directly to prevent infinite recursion
			if (error instanceof ZulongError) {
				return error
			}
			return new ZulongError(JSON.parse(error), modelId, providerId)
		} catch {
			return new ZulongError(error, modelId, providerId)
		}
	}

	public isErrorType(type: ZulongErrorType): boolean {
		return ZulongError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: ZulongError): ZulongErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return ZulongErrorType.Balance
		}
		...
		if (code === "SPEND_LIMIT_EXCEEDED" || details?.code === "SPEND_LIMIT_EXCEEDED") {
			return ZulongErrorType.SpendLimit
		}
		...
		if (code === "ERR_BAD_REQUEST" || err instanceof AuthInvalidTokenError || isAuthStatus) {
			return ZulongErrorType.Auth
		}

		if (code === "INFERENCE_CAP_ERROR") {
			return ZulongErrorType.QuotaExceeded
		}
		...
		if (message) {
			...
			if (message?.includes(ZULONG_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return ZulongErrorType.Auth
			}
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return ZulongErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = ZulongErrorType.Network
	}
}
...
export class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = ZulongErrorType.Auth
	}
}
