...
export class StandaloneTerminalManager implements ITerminalManager {
	...
	async getOrCreateTerminal(cwd: string): Promise<TerminalInfo> {
		...
		const newTerminalInfo = this.registry.createTerminal({
			cwd: cwd,
			name: `Zulong Terminal ${this.registry.size + 1}`,
		})
	...
	trackBackgroundCommand(
		...
		// Use ZulongTempManager for proper temp file management and cleanup
		const logFilePath = ZulongTempManager.createTempFilePath("background")
