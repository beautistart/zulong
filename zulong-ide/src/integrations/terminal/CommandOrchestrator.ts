...
export async function orchestrateCommandExecution(
	...
	const clearCommandState = async () => {
		...
		const zulongMessages = callbacks.getZulongMessages()
		const lastCommandIndex = findLastIndex(zulongMessages, (m) => m.ask === "command" || m.say === "command")
		if (lastCommandIndex !== -1) {
			await callbacks.updateZulongMessage(lastCommandIndex, {
	...
	const switchToFileBased = async () => {
		...
		// Set up file logging using ZulongTempManager for proper cleanup
		largeOutputLogPath = ZulongTempManager.createTempFilePath("large-output")
