...
import { ZulongToolResponseContent } from "@shared/messages"
...
export class CommandExecutor {
	...
	async cancelBackgroundCommand(): Promise<boolean> {
		...
		if (cancelled) {
			...
			const messages = this.callbacks.getZulongMessages()
			...
			if (lastCommandOutputIndex !== -1) {
				...
				await this.callbacks.updateZulongMessage(lastCommandOutputIndex, {
