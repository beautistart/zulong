...
/**
 * Zulong's GitHub avatar URL
 */
const ZULONG_AVATAR_URL = "https://avatars.githubusercontent.com/u/184127137"
...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController("zulong-ai-review", "Zulong AI Review")

		// Configure options for the reply input
		this.commentController.options = {
			placeHolder: "Ask a question about this code...",
			prompt: "Reply to Zulong",
		}
		...
		this.disposables.push(
			vscode.commands.registerCommand("zulong.reviewComment.reply", async (reply: vscode.CommentReply) => {
				await this.handleReply(reply)
			}),
		)

		// Register add to chat command - sends the conversation to Zulong's main chat
		this.disposables.push(
			vscode.commands.registerCommand("zulong.reviewComment.addToChat", async (thread: vscode.CommentThread) => {
	...
	addReviewComment(comment: ReviewComment): void {
		...
		const commentObj: vscode.Comment = {
			...
			author: {
				name: "Zulong",
				iconPath: vscode.Uri.parse(ZULONG_AVATAR_URL),
			},
		}
	...
	/**
	 * Handle adding the thread conversation to Zulong's main chat
	 */
...
Please continue helping the user with their question about this code.`
	...
	/**
	 * Close all tabs that use the zulong-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with zulong-diff scheme (opened during comment reveal)
