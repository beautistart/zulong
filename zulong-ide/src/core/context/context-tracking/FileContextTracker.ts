...
// If a user modifies a file outside of Zulong, the context may become stale and need to be updated.
// We do not want Zulong to reload the context every time a file is modified, so we use this class merely
// to inform Zulong that the change has occurred, and tell Zulong to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Zulong was unable to complete a diff edit.
// a diff edit because the file was modified since Zulong last read it.

// FileContextTracker
/**
This class is responsible for tracking file operations.
If the full contents of a file are passed to Zulong via a tool, mention, or edit, the file is marked as active.
If a file is modified outside of Zulong, we detect and track this change to prevent stale context.
...
export class FileContextTracker {
	...
	private recentlyEditedByZulong = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.on("change", () => {
			if (this.recentlyEditedByZulong.has(filePath)) {
				this.recentlyEditedByZulong.delete(filePath) // This was an edit by Zulong, no need to inform Zulong
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Zulong
	 ...
	 * This is the main entry point for FileContextTracker and is called when a file is passed to Zulong via a tool, mention, or edit.
	 */
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "zulong_edited" | "file_mentioned") {
		try {
			const cwd = await getCwd()
			if (!cwd) {
				Logger.info("No workspace folder available - cannot determine current working directory")
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: FileMetadataEntry["record_source"]) {
		try {
			...
			const newEntry: FileMetadataEntry = {
				...
				zulong_read_date: getLatestDateForField(filePath, "zulong_read_date"),
				zulong_edit_date: getLatestDateForField(filePath, "zulong_edit_date"),
			...
			switch (source) {
				...
				// zulong_edited: Zulong has edited the file
				case "zulong_edited":
					newEntry.zulong_read_date = now
					newEntry.zulong_edit_date = now
					break

				// read_tool/file_mentioned: Zulong has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.zulong_read_date = now
					break
	...
	/**
	 * Marks a file as edited by Zulong to prevent false positives in file watchers
	 */
	markFileAsEditedByZulong(filePath: string): void {
		this.recentlyEditedByZulong.add(filePath)
	}
	...
	/**
	 * Detects files that were edited by Zulong or users after a specific message timestamp
	 * This is used when restoring checkpoints to warn about potential file content mismatches
	 */
	async detectFilesEditedAfterMessage(messageTs: number, deletedMessages: ZulongMessage[]): Promise<string[]> {
		const editedFiles: string[] = []

		try {
			// Check task metadata for files that were edited by Zulong or users after the message timestamp
			...
			if (taskMetadata?.files_in_context) {
				for (const fileEntry of taskMetadata.files_in_context) {
					const zulongEditedAfter = fileEntry.zulong_edit_date && fileEntry.zulong_edit_date > messageTs
					const userEditedAfter = fileEntry.user_edit_date && fileEntry.user_edit_date > messageTs

					if (zulongEditedAfter || userEditedAfter) {
