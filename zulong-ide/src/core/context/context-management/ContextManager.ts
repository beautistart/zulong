...
import { ZulongApiReqInfo, ZulongMessage } from "@shared/ExtensionMessage"
...
export class ContextManager {
	...
	shouldCompactContextWindow(
		...
		if (previousApiReqIndex >= 0) {
			const previousRequestText = zulongMessages[previousApiReqIndex]?.text
			if (previousRequestText) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: ZulongApiReqInfo = JSON.parse(previousRequestText)
	...
	getContextTelemetryData(
		...
		if (triggerIndex !== undefined) {
			...
			const apiReqIndices = zulongMessages
		...
		if (targetIndex >= 0) {
			const targetRequestText = zulongMessages[targetIndex]?.text
			if (targetRequestText) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: ZulongApiReqInfo = JSON.parse(targetRequestText)
	...
	async getNewContextMessagesAndMetadata(
		...
		if (!useAutoCondense) {
			...
			if (previousApiReqIndex >= 0) {
				const previousRequestText = zulongMessages[previousApiReqIndex]?.text
				if (previousRequestText) {
					const timestamp = zulongMessages[previousApiReqIndex].ts
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: ZulongApiReqInfo = JSON.parse(previousRequestText)
					const totalTokens = (tokensIn || 0) + (tokensOut || 0) + (cacheWrites || 0) + (cacheReads || 0)
	...
	public getNextTruncationRange(
		...
		// NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout zulong)
	...
	async attemptFileReadOptimization(
		...
		const previousRequest = zulongMessages[previousApiReqIndex]
