...
type MessageStateHandlerLike = {
	getZulongMessages(): ZulongMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Zulong Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in zulongMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const zulongMessages = deps.messageStateHandler.getZulongMessages()
		...
		const lastUserMsg = [...zulongMessages]
			.reverse()
		...
		// (3) Files edited by Zulong during this task (completed operations):
