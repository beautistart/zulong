...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".zulong", "skills")
	...
	beforeEach(() => {
		...
		sandbox.stub(disk, "getSkillsDirectoriesForScan").returns([
			{ path: path.join(TEST_CWD, ".zulongrules", "skills"), source: "project" },
			{ path: path.join(TEST_CWD, ".zulong", "skills"), source: "project" },
...
Instructions here`)
		...
		it("should discover skills from project .zulongrules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".zulongrules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .zulong/skills directory", async () => {
			const zulongSkillsDir = path.join(TEST_CWD, ".zulong", "skills")
			const skillDir = path.join(zulongSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(zulongSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(zulongSkillsDir).resolves(true)
			readdirStub.withArgs(zulongSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
	...
	describe("Remote Skills", () => {
		...
		describe("Override resolution (remote > disk-global > project)", () => {
			...
			it("remote overrides project skill of same name", async () => {
				const entries = [makeEntry("coding", "Remote coding")]
				const projDir = path.join(TEST_CWD, ".zulongrules", "skills")
