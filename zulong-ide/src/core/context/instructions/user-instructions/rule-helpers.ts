...
export async function synchronizeRuleToggles(
	...
	currentToggles: ZulongRulesToggles,
	allowedFileExtension: string = "",
	excludedPaths: string[][] = [],
): Promise<ZulongRulesToggles> {
...
export function synchronizeRemoteRuleToggles(
	...
	const updatedToggles: ZulongRulesToggles = {}
 ...
 */
export function combineRuleToggles(toggles1: ZulongRulesToggles, toggles2: ZulongRulesToggles): ZulongRulesToggles {
 ...
 */
export const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: ZulongRulesToggles) => {
 ...
 * Used by getGlobalZulongRules and getLocalZulongRules.
 */
...
export function getRemoteRulesTotalContentWithMetadata(
	remoteRules: GlobalInstructionsFile[],
	remoteToggles: ZulongRulesToggles,
...
/**
 * Handles converting any directory into a file (specifically used for .zulongrules and .zulongrules/workflows)
 * The old .zulongrules file or .zulongrules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
export async function ensureLocalZulongDirExists(zulongrulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(zulongrulePath)

		if (exists && !(await isDirectory(zulongrulePath))) {
			// logic to convert .zulongrules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(zulongrulePath, "utf8")
			const tempPath = zulongrulePath + ".bak"
			await fs.rename(zulongrulePath, tempPath) // create backup
			try {
				await fs.mkdir(zulongrulePath, { recursive: true })
				await fs.writeFile(path.join(zulongrulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(zulongrulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, zulongrulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const globalZulongWorkflowFilePath = await ensureWorkflowsDirectoryExists()
				filePath = path.join(globalZulongWorkflowFilePath, filename)
			} else {
				const globalZulongRulesFilePath = await ensureRulesDirectoryExists()
				filePath = path.join(globalZulongRulesFilePath, filename)
			}
		} else {
			const localZulongRulesFilePath = path.resolve(cwd, GlobalFileNames.zulongRules)

			const hasError = await ensureLocalZulongDirExists(localZulongRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localZulongRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, GlobalFileNames.workflows)

				const hasError = await ensureLocalZulongDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// zulongrules file creation
				filePath = path.join(localZulongRulesFilePath, filename)
			}
		}
