...
export class ToolResultUtils {
	...
	static pushToolResult(
		...
		if (typeof content === "string") {
			...
			// Get tool_use_id from map using call_id, or use "zulong" as fallback for backward compatibility
			const toolUseId = toolUseIdMap?.get(block.call_id || "") || "zulong"
			...
			// If using backward-compatible "zulong" ID and content is an array, spread it directly
			// instead of wrapping it (which would cause JSON.stringify in createToolResultBlock)
			if ((toolUseId === "zulong" || !toolUseId) && Array.isArray(content)) {
	...
	private static createToolResultBlock(content: ToolResponse, id?: string, call_id?: string) {
		// If id is "zulong", we treat it as a plain text result for backward compatibility
	 ...
	 */
	static async askApprovalAndPushFeedback(type: ZulongAsk, completeMessage: string, config: TaskConfig) {
