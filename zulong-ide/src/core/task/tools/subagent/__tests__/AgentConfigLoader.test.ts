...
import { ZulongDefaultTool, getToolUseNames } from "@/shared/tools"
...
You are a code reviewer.`
		...
		assert.deepEqual(parsed.tools, [ZulongDefaultTool.FILE_READ, ZulongDefaultTool.LIST_FILES, ZulongDefaultTool.SEARCH])
		assert.equal(parsed.systemPrompt, "You are a code reviewer.")
	})

	it("supports raw Zulong tool ids in tools", () => {
		const content = `---
name: cli-agent
...
Prompt body`
		...
		const parsed = parseAgentConfigFromYaml(content)
		assert.deepEqual(parsed.tools, [ZulongDefaultTool.FILE_READ, ZulongDefaultTool.LIST_FILES])
	})
...
Prompt body`
	...
	it("loads all yaml/yml files from homeDir/.zulong/data/agents", async () => {
		const tempHome = await createTempHomeDir()
		tempDirs.push(tempHome)
...
Reviewer prompt`,
		...
		assert.deepEqual(localAgent?.tools, [ZulongDefaultTool.FILE_READ])
		assert.equal(localAgent?.systemPrompt, "Prompt body")
		assert.equal(reviewer?.name, "reviewer")
		assert.deepEqual(reviewer?.tools, [ZulongDefaultTool.LIST_FILES])
