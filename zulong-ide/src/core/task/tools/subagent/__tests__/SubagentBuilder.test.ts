...
import { ZulongToolSet } from "@core/prompts/system-prompt/registry/ZulongToolSet"
...
describe("SubagentBuilder", () => {
	...
	it("uses cached config by subagent name and applies act-mode provider model override", () => {
			...
			getCachedConfig: (subagentName?: string) =>
				subagentName === "cached-agent"
					? {
							...
							tools: [ZulongDefaultTool.LIST_FILES],
							modelId: "gpt-5",
		...
		assert.deepEqual(builder.getAllowedTools(), [ZulongDefaultTool.LIST_FILES, ZulongDefaultTool.ATTEMPT])
			...
			getCachedConfig: (subagentName?: string) =>
				subagentName === "openrouter-agent"
					? {
							...
							tools: [ZulongDefaultTool.FILE_READ],
	...
	it("builds native tools by filtering allowed ids and context requirements then converting", () => {
		...
		const getToolsStub = sinon.stub(ZulongToolSet, "getToolsForVariantWithFallback").returns([
			{
				config: {
					id: ZulongDefaultTool.LIST_FILES,
					contextRequirements: () => true,
				},
			},
			{
				config: {
					id: ZulongDefaultTool.SEARCH,
					contextRequirements: () => true,
				},
			},
			{
				config: {
					id: ZulongDefaultTool.ATTEMPT,
					contextRequirements: () => false,
				},
			},
		] as never)
		const converter = sinon.stub().callsFake((tool: { id: string }) => ({ converted: tool.id }))
		const getConverterStub = sinon.stub(ZulongToolSet, "getNativeConverter").returns(converter as never)
		...
		assert.deepEqual(result, [{ converted: ZulongDefaultTool.LIST_FILES }])
	})
})
