...
function createTaskConfig(
	...
	return {
		...
		coordinator: {
			getHandler: sinon.stub().callsFake((toolName: ZulongDefaultTool) => {
				if (toolName === ZulongDefaultTool.LIST_FILES) {
					return {
...
describe("SubagentRunner", () => {
	...
	it("emits native tool_use blocks with matching tool_result tool_use_id across turns", async () => {
		...
		createMessage.onSecondCall().callsFake(async function* (_systemPrompt: string, conversation: unknown[]) {
			...
			assert.equal(toolUse.name, ZulongDefaultTool.LIST_FILES)
			...
			yield {
				...
				tool_call: {
					function: {
						...
						name: ZulongDefaultTool.ATTEMPT,
