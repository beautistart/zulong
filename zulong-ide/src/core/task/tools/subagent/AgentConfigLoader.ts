...
import { ZulongDefaultTool, setDynamicToolUseNames } from "@shared/tools"
...
/** Default Directory for agent configurations: ~/Documents/Zulong/Agents */
...
const AgentBaseConfigSchema = z.object({
	...
	tools: z.array(z.nativeEnum(ZulongDefaultTool)).default([]),
...
function normalizeToolName(toolName: string): ZulongDefaultTool {
	const trimmed = toolName.trim()
	if (!trimmed) {
		throw new Error("Tool name cannot be empty.")
	}

	const asDefaultTool = trimmed as ZulongDefaultTool
	if (Object.values(ZulongDefaultTool).includes(asDefaultTool)) {
		return asDefaultTool
	}

	throw new Error(
		`Unknown tool '${trimmed}'. Expected a ZulongDefaultTool value (for example: read_file, list_files, search_files).`,
	)
}

function parseTools(tools: string | string[] | undefined): ZulongDefaultTool[] {
	if (!tools) {
		return []
	}
...
export function getAgentsConfigPath(homeDir = os.homedir()): string {
	return path.join(homeDir, "Documents", "Zulong", AGENTS_CONFIG_DIRECTORY_NAME)
}
