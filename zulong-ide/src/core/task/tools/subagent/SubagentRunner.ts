...
import { ZulongAssistantToolUseBlock, ZulongStorageMessage, ZulongTextContentBlock, ZulongUserContent } from "@shared/messages"
import { Logger } from "@shared/services/Logger"
import { ZulongDefaultTool, ZulongTool } from "@shared/tools"
import { ContextManager } from "@/core/context/context-management/ContextManager"
import { checkContextWindowExceededError } from "@/core/context/context-management/context-error-handling"
import { getContextWindowInfo } from "@/core/context/context-management/context-window-utils"
import { HostRegistryInfo } from "@/registry"
import { ZulongError, ZulongErrorType } from "@/services/error"
...
function toAssistantToolUseBlock(call: SubagentToolCall): ZulongAssistantToolUseBlock {
	return {
		type: "tool_use",
		id: call.toolUseId,
		name: call.name,
...
export class SubagentRunner {
	...
	private readonly allowedTools: ZulongDefaultTool[]
	...
	async run(prompt: string, onProgress: (update: SubagentProgressUpdate) => void): Promise<SubagentRunResult> {
		...
		try {
			...
			const conversation: ZulongStorageMessage[] = [
				{
					role: "user",
					content: [
						{
			...
			while (true) {
				...
				const toolResultBlocks = [] as ZulongUserContent[]
				for (const call of finalizedToolCalls) {
					const toolName = call.name as ZulongDefaultTool
					const toolCallParams = toToolUseParams(call.input)

					if (toolName === ZulongDefaultTool.ATTEMPT) {
	...
	private createSubagentTaskConfig(state: TaskState): TaskConfig {
		...
		const validator = new ToolValidator(this.baseConfig.services.zulongIgnoreController)
	...
	private shouldRetryInitialStreamError(error: unknown, providerId: string, modelId: string): boolean {
		...
		const parsedError = ZulongError.transform(error, modelId, providerId)
		const isAuthError = parsedError.isErrorType(ZulongErrorType.Auth)
		const isBalanceError = parsedError.isErrorType(ZulongErrorType.Balance)
	...
	private compactConversationForContextWindow(
		...
		conversation: ZulongStorageMessage[],
	...
	private optimizeConversationForContextWindow(
		...
		const optimizedConversation = optimizationResult.optimizedConversationHistory.map(
			(message) => message as ZulongStorageMessage,
		)
	...
	private async *createMessageWithInitialChunkRetry(
		api: ReturnType<typeof buildApiHandler>,
		systemPrompt: string,
		fullConversation: ZulongStorageMessage[],
		nativeTools: ZulongTool[] | undefined,
		providerId: string,
		modelId: string,
		contextManager: ContextManager,
		...
		for (let attempt = 1; attempt <= MAX_INITIAL_STREAM_ATTEMPTS; attempt += 1) {
			const truncatedConversation = contextManager
				...
				.map((message) => message as ZulongStorageMessage)
