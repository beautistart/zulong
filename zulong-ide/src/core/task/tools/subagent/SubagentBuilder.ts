...
export const SUBAGENT_DEFAULT_ALLOWED_TOOLS: ZulongDefaultTool[] = [
...
export class SubagentBuilder {
	...
	getAllowedTools(): ZulongDefaultTool[] {
	...
	buildNativeTools(context: SystemPromptContext) {
		...
		const toolSets = ZulongToolSet.getToolsForVariantWithFallback(family, this.allowedTools)
	...
	private resolveAllowedTools(configuredTools?: ZulongDefaultTool[]): ZulongDefaultTool[] {
		const sourceTools = configuredTools && configuredTools.length > 0 ? configuredTools : SUBAGENT_DEFAULT_ALLOWED_TOOLS
		return Array.from(new Set([...sourceTools, ZulongDefaultTool.ATTEMPT]))
	}
