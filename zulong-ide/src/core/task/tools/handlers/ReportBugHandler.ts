...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ZulongDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Zulong wants to create a github issue...",
				message: `Zulong is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const zulongVersion = ExtensionRegistryInfo.version
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("zulong-version", zulongVersion)
				...
				await createAndOpenGitHubIssue("zulong", "zulong", "bug_report.yml", params)
			} catch (error) {
