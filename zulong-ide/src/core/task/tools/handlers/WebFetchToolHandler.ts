import { ZulongAsk, ZulongSayTool } from "@shared/ExtensionMessage"
import { ZulongDefaultTool } from "@shared/tools"
import axios from "axios"
import { ZulongEnv } from "@/config"
import { AuthService } from "@/services/auth/AuthService"
import { buildZulongExtraHeaders } from "@/services/EnvUtils"
...
export class WebFetchToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.WEB_FETCH
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await uiHelpers.ask("tool" as ZulongAsk, partialMessage, block.partial).catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		try {
			...
			// Check if Zulong web tools are enabled (both user setting and feature flag)
			const zulongWebToolsEnabled = config.services.stateManager.getGlobalSettingsKey("zulongWebToolsEnabled")
			const featureFlagEnabled = featureFlagsService.getWebtoolsEnabled()
			if (provider !== "zulong" || !zulongWebToolsEnabled || !featureFlagEnabled) {
				return formatResponse.toolError("Zulong web tools are currently disabled.")
			}
			...
			if (config.callbacks.shouldAutoApproveTool(this.name)) {
				...
				showNotificationForApproval(
					`Zulong wants to fetch content from ${url}`,
			...
			const baseUrl = ZulongEnv.config().apiBaseUrl
			const authToken = await AuthService.getInstance().getAuthToken()

			if (!authToken) {
				throw new Error(ZULONG_ACCOUNT_AUTH_ERROR_MESSAGE)
			}
