...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ZulongDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const result = await executePreCompactHookWithCleanup({
						...
						zulongMessages: config.messageState.getZulongMessages(),
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .zulongignore first and skip ignored files
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(ZulongDefaultTool.FILE_READ, relPath)) {
						try {
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getZulongMessages(),
				config.api,
