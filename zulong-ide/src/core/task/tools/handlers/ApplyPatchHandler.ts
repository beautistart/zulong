...
import type { ZulongSayTool } from "@shared/ExtensionMessage"
...
export const PatchZulongSayMap = {
...
export class ApplyPatchHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.APPLY_PATCH
	...
	private async previewPatchStream(rawInput: string, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await config.callbacks
			.ask(
				"tool",
				JSON.stringify({
					tool: PatchZulongSayMap[actionType],
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			for (const changedFilePath of changedFiles) {
				...
				config.services.fileContextTracker.markFileAsEditedByZulong(pathToTrack)
				await config.services.fileContextTracker.trackFileContext(pathToTrack, "zulong_edited")
	...
	private async generateChangeSummary(changes: Record<string, FileChange>): Promise<ZulongSayTool[]> {
	...
	private async handleApproval(
		...
		showNotificationForApproval(`Zulong wants to edit '${message.path}'`, config.autoApprovalSettings.enableNotifications)
