import { BrowserAction, BrowserActionResult, browserActions, ZulongSayBrowserAction } from "@shared/ExtensionMessage"
...
export class BrowserToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.BROWSER
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		if (action === "launch") {
			...
			await uiHelpers.say(
				...
				} satisfies ZulongSayBrowserAction),
				undefined,
				undefined,
				block.partial,
			)
			...
			if (action === "launch") {
				...
				if (autoApprover.shouldAutoApproveTool(block.name)) {
					...
					showNotificationForApproval(
						`Zulong wants to use a browser and launch ${url}`,
