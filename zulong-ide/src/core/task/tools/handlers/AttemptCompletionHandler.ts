...
export class AttemptCompletionHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ZulongDefaultTool.ATTEMPT
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const addNewChangesFlagToLastCompletionResultMessage = async () => {
			...
			const zulongMessages = config.messageState.getZulongMessages()

			const lastCompletionResultMessageIndex = findLastIndex(zulongMessages, (m: any) => m.say === "completion_result")
			const lastCompletionResultMessage =
				lastCompletionResultMessageIndex !== -1 ? zulongMessages[lastCompletionResultMessageIndex] : undefined
			if (
				...
				await config.messageState.updateZulongMessage(lastCompletionResultMessageIndex, {
		...
		if (partialCompletionIndex !== -1) {
			const updatedMessages = [
				...zulongMessages.slice(0, partialCompletionIndex),
				...zulongMessages.slice(partialCompletionIndex + 1),
			]
			config.messageState.setZulongMessages(updatedMessages)
			await config.messageState.saveZulongMessagesAndUpdateHistory()
		}

		let commandResult: any
		const lastMessage = config.messageState.getZulongMessages().at(-1)
		...
		if (command) {
			...
			const autoApproveResult = config.autoApprover?.shouldAutoApproveTool(ZulongDefaultTool.BASH)
			...
			if (autoApproveSafe) {
				...
				showNotificationForApproval(
					`Zulong wants to execute a command: ${command}`,
		...
		if (config.messageState.getZulongMessages().at(-1)?.ask === "command_output") {
