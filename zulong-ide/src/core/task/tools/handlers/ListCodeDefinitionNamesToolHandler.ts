...
export class ListCodeDefinitionNamesToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.LIST_CODE_DEF
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			result = await parseSourceCodeForDefinitionsTopLevel(absolutePath, config.services.zulongIgnoreController)
		} catch (error) {
			config.taskState.consecutiveMistakeCount++
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Zulong wants to analyze code definitions in ${getWorkspaceBasename(absolutePath, "ListCodeDefinitionNamesToolHandler.notification")}`
