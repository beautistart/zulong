...
export class WriteToFileToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.FILE_NEW // This handler supports write_to_file, replace_in_file, and new_rule
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		try {
			...
			const sharedMessageProps: ZulongSayTool = {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (block.name === "replace_in_file" && !rawDiff) {
			...
			await config.callbacks.say(
				"error",
				`Zulong tried to use replace_in_file for '${relPath}' without value for required parameter 'diff'. Retrying...`,
			)
		...
		if (block.name === "write_to_file" && !rawContent) {
			...
			const lastApiReqTotalTokens = getLastApiReqTotalTokens(config.messageState.getZulongMessages())
			...
			await config.callbacks.say(
				"error",
				`Zulong tried to use write_to_file for '${relPath}' without value for required parameter 'content'. ${
					config.taskState.consecutiveMistakeCount >= 2
						? "This has happened multiple times — Zulong will try a different approach."
						: "Retrying..."
				}`,
			)
		...
		try {
			...
			if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)) {
				...
				const notificationMessage = `Zulong wants to ${fileExists ? "edit" : "create"} ${getWorkspaceBasename(relPath, "WriteToFile.notification")}`
			...
			// Mark the file as edited by Zulong
			config.services.fileContextTracker.markFileAsEditedByZulong(relPath)
			...
			await config.services.fileContextTracker.trackFileContext(relPath, "zulong_edited")
	...
	async validateAndPrepareFileOperation(config: TaskConfig, block: ToolUse, relPath: string, diff?: string, content?: string) {
		...
		// Check zulongignore access first
		const accessValidation = this.validator.checkZulongIgnorePath(resolvedPath)
		if (!accessValidation.ok) {
			// Show error and return early (full original behavior)
			await config.callbacks.say("zulongignore_error", resolvedPath)

			// Push tool result and save checkpoint using existing utilities
			const errorResponse = formatResponse.toolError(formatResponse.zulongIgnoreError(resolvedPath))
		...
		if (diff) {
			...
			// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but Zulong throws error
