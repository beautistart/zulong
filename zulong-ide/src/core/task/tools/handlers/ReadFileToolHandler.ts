...
async function emitReadFileToolUiComplete(
	config: TaskConfig,
	sharedMessageProps: ZulongSayTool,
	block: ToolUse,
	fileContent: FileContentResult,
): Promise<void> {
	...
	const payload: ZulongSayTool = { ...sharedMessageProps }
	if (range) {
...
export class ReadFileToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.FILE_READ
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Zulong wants to read ${getWorkspaceBasename(absolutePath, "ReadFileToolHandler.notification")}`
