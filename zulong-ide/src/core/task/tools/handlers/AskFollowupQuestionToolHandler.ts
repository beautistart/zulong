...
import { ZulongAsk, ZulongAskQuestion } from "@shared/ExtensionMessage"
...
export class AskFollowupQuestionToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ZulongDefaultTool.ASK
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		} satisfies ZulongAskQuestion

		await uiHelpers.ask("followup" as ZulongAsk, JSON.stringify(sharedMessage), block.partial).catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Zulong has a question...",
		...
		if (optionsRaw && text && options.includes(text)) {
			...
			const lastFollowupMessage = findLast(zulongMessages, (m: any) => m.ask === "followup")
