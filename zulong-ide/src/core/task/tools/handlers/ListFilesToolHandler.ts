...
export class ListFilesToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.LIST_FILES
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Check zulongignore access before performing any IO.
		// Increment the counter so repeated attempts at blocked paths
		// accumulate toward the yolo-mode mistake limit.
		const accessValidation = this.validator.checkZulongIgnorePath(relDirPath!)
		if (!accessValidation.ok) {
			config.taskState.consecutiveMistakeCount++
			if (!config.isSubagentExecution) {
				await config.callbacks.say("zulongignore_error", relDirPath)
			}
			return formatResponse.toolError(formatResponse.zulongIgnoreError(relDirPath!))
		}
		...
		const result = formatResponse.formatFilesList(absolutePath, files, didHitLimit, config.services.zulongIgnoreController)
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Zulong wants to view directory ${getWorkspaceBasename(absolutePath, "ListFilesToolHandler.notification")}/`
