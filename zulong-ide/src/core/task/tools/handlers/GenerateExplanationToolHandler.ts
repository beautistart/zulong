...
import type { ZulongSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified ZulongSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: ZulongSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: ZulongSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = ZulongDefaultTool.GENERATE_EXPLANATION
