...
import {
	ZulongAskUseSubagents,
	ZulongSaySubagentStatus,
	ZulongSubagentUsageInfo,
	SubagentStatusItem,
...
export class UseSubagentsToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.USE_SUBAGENTS
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const partialMessage = JSON.stringify({ prompts } satisfies ZulongAskUseSubagents)
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const approvalPayload: ZulongAskUseSubagents = { prompts }
		...
		if (didAutoApprove) {
			...
			showNotificationForApproval(
				prompts.length === 1
					? `Zulong wants to use ${configuredSubagentName ? `the '${configuredSubagentName}' subagent` : "a subagent"}`
					: `Zulong wants to use ${prompts.length} subagents`,
		...
		const emitStatus = async (status: ZulongSaySubagentStatus["status"], partial: boolean) => {
			...
			const payload: ZulongSaySubagentStatus = {
				status,
		...
		const queueStatusUpdate = (status: ZulongSaySubagentStatus["status"], partial: boolean): Promise<void> => {
		...
		const subagentUsagePayload: ZulongSubagentUsageInfo = {
			source: "subagents",
