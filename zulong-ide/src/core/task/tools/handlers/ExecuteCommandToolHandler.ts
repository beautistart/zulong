...
import { ZulongAsk } from "@shared/ExtensionMessage"
...
export class ExecuteCommandToolHandler implements IFullyManagedTool {
	readonly name = ZulongDefaultTool.BASH
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await uiHelpers
			.ask("command" as ZulongAsk, uiHelpers.removeClosingTag(block, "command", command), block.partial)
			.catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!command) {
			...
			await config.callbacks.say(
				"error",
				"Zulong tried to use execute_command without value for required parameter 'command'. Retrying...",
			)
		...
		// Check command permission validation (ZULONG_COMMAND_PERMISSIONS env var)
		...
		if (!permissionResult.allowed) {
			...
			if (permissionResult.failedSegment) {
				errorMessage =
					`Command "${actualCommand}" was denied by ZULONG_COMMAND_PERMISSIONS. ` +
		...
		// Check zulongignore validation for command
		const ignoredFileAttemptedToAccess = config.services.zulongIgnoreController.validateCommand(actualCommand)
		if (ignoredFileAttemptedToAccess) {
			if (!config.isSubagentExecution) {
				await config.callbacks.say("zulongignore_error", ignoredFileAttemptedToAccess)
			}
			return formatResponse.toolError(formatResponse.zulongIgnoreError(ignoredFileAttemptedToAccess))
		}
