...
function createConfig() {
	...
	const config = {
		...
		services: {
			...
			urlContentFetcher: {},
			diffViewProvider: {},
			zulongIgnoreController: { validateAccess: () => true, filterPaths: (paths: string[]) => paths },
...
describe("ListCodeDefinitionNamesToolHandler.execute – error recovery", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zulong-listdef-test-"))
	...
	function makeBlock(relPath?: string) {
		return {
			...
			name: ZulongDefaultTool.LIST_CODE_DEF,
...
describe("ListFilesToolHandler.execute – error recovery", () => {
	...
	beforeEach(async () => {
		sandbox = sinon.createSandbox()
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zulong-listfiles-test-"))
		sandbox.stub(pathUtils, "isLocatedInWorkspace").resolves(true)
	})
	...
	function makeBlock(relPath?: string, recursive?: string) {
		...
		return {
			type: "tool_use" as const,
			name: ZulongDefaultTool.LIST_FILES,
			params,
			partial: false,
		}
	}
	...
	it("increments consecutiveMistakeCount on zulongignore denial", async () => {
		const { config, taskState } = createConfig()
		// Create a validator whose zulongIgnoreController blocks all paths
		...
		assert.equal(typeof result, "string")
		assert.ok((result as string).includes("zulongignore"))
		assert.equal(taskState.consecutiveMistakeCount, 1)
	})

	it("accumulates zulongignore denials across repeated calls", async () => {
		const { config, taskState } = createConfig()
		const blockingValidator = new ToolValidator({ validateAccess: () => false } as any)
		const handler = new ListFilesToolHandler(blockingValidator)
...
describe("SearchFilesToolHandler.execute – error recovery", () => {
	...
	beforeEach(async () => {
		sandbox = sinon.createSandbox()
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zulong-search-test-"))
		sandbox.stub(pathUtils, "isLocatedInWorkspace").resolves(true)
	})
