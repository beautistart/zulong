...
export class ToolValidator {
	constructor(private readonly zulongIgnoreController: ZulongIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .zulongignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkZulongIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.zulongIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .zulongignore settings.`,
			}
		}
		return { ok: true }
	}
