...
export interface PendingReasoning {
	...
	content: string
	signature: string
	redactedThinking: ZulongAssistantRedactedThinkingBlock[]
...
/**
 * Handles streaming native tool use blocks and converts them to ZulongAssistantToolUseBlock format
 */
class ToolUseHandler {
	...
	getFinalizedToolUse(id: string): ZulongAssistantToolUseBlock | undefined {
	...
	getAllFinalizedToolUses(summary?: ZulongAssistantToolUseBlock["reasoning_details"]): ZulongAssistantToolUseBlock[] {
		const results: ZulongAssistantToolUseBlock[] = []
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			if (pending.name.includes(ZULONG_MCP_TOOL_IDENTIFIER)) {
				const [key, toolName] = pending.name.split(ZULONG_MCP_TOOL_IDENTIFIER)
				results.push({
					type: "tool_use",
					name: ZulongDefaultTool.MCP_USE,
					params: {
				...
				results.push({
					type: "tool_use",
					name: pending.name as ZulongDefaultTool,
...
class ReasoningHandler {
	...
	getCurrentReasoning(): ZulongAssistantThinkingBlock | null {
		if (!this.pendingReasoning) {
			return null
		}
	...
	getRedactedThinking(): ZulongAssistantRedactedThinkingBlock[] {
