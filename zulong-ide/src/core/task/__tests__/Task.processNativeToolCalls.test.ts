	...
	it("finalizes a partial text row before handing off to native tool calls", async () => {
		...
		const fakeTask = {
			messageStateHandler: {
				...
				saveZulongMessagesAndUpdateHistory: async () => {
					saveCalls += 1
				},
			},
			taskState: {
		...
		try {
			...
			assert.equal(zulongMessages[0].text, "visible streamed text")
			assert.equal(zulongMessages[0].partial, false)
