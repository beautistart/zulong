...
function createFakeTask(taskState: {
	...
	const fakeTask = {
		...
		taskId: "task-1",
		messageStateHandler: {
			addToZulongMessages: async (message: ZulongMessage) => {
				zulongMessages.push(message)
			},
			getZulongMessages: () => zulongMessages,
		},
		postStateToWebview: async () => undefined,
	}

	return { zulongMessages, fakeTask }
}
...
describe("Task.ask", () => {
	it("keeps resume asks waiting for a user response even when the task is aborted", async () => {
		...
		const { zulongMessages, fakeTask } = createFakeTask(taskState)
		...
		try {
			...
			assert.equal(zulongMessages.length, 1)
			assert.equal(zulongMessages[0].ask, "resume_task")
	...
	it("keeps resume-completed asks waiting for a user response even when the task is aborted", async () => {
		...
		try {
			...
			await flushMicrotasks()
			assert.equal(zulongMessages.length, 1)
			assert.equal(zulongMessages[0].ask, "resume_completed_task")
			assert.notEqual(taskState.lastMessageTs, undefined)
	...
	it("still wakes non-resume asks when abort is triggered after the ask is shown", async () => {
		...
		try {
			...
			await flushMicrotasks()
			assert.equal(zulongMessages.length, 1)
			assert.equal(zulongMessages[0].ask, "completion_result")

			const rejectionPromise = assert.rejects(askPromise, /Zulong instance aborted/)
