...
import { ZulongApiReqCancelReason, ZulongApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const zulongMessages = params.messageStateHandler.getZulongMessages()
	const currentApiReqInfo: ZulongApiReqInfo = JSON.parse(zulongMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateZulongMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
