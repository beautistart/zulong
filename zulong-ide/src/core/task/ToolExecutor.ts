...
import { ZulongAsk, ZulongSay } from "@shared/ExtensionMessage"
import { ZulongContent } from "@shared/messages/content"
import { ZulongDefaultTool, toolUseNames } from "@shared/tools"
...
export function canonicalizeAttemptCompletionParams(block: ToolUse): boolean {
	if (block.name === ZulongDefaultTool.ATTEMPT && !block.params?.result && typeof block.params?.response === "string") {
...
export class ToolExecutor {
	...
	private shouldAutoApproveTool(toolName: ZulongDefaultTool): boolean | [boolean, boolean] {
	...
	constructor(
		...
		private sayAndCreateMissingParamError: (toolName: ZulongDefaultTool, paramName: string, relPath?: string) => Promise<any>,
		private removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: ZulongAsk | ZulongSay) => Promise<void>,
	...
	private asToolConfig(): TaskConfig {
		const config: TaskConfig = {
			...
			services: {
				...
				zulongIgnoreController: this.zulongIgnoreController,
	...
	private registerToolHandlers(): void {
		const validator = new ToolValidator(this.zulongIgnoreController)
	 ...
	 */
	private static readonly PLAN_MODE_RESTRICTED_TOOLS: ZulongDefaultTool[] = [
		ZulongDefaultTool.FILE_NEW,
		ZulongDefaultTool.FILE_EDIT,
		ZulongDefaultTool.NEW_RULE,
		ZulongDefaultTool.APPLY_PATCH,
	]
	 ...
	 */
	private isPlanModeToolRestricted(toolName: ZulongDefaultTool): boolean {
