...
import {
	getGlobalZulongRules,
	getLocalZulongRules,
	refreshZulongRulesToggles,
} from "@core/context/instructions/user-instructions/zulong-rules"
import {
	getLocalAgentsRules,
	getLocalCursorRules,
...
import { ZulongIgnoreController } from "@core/ignore/ZulongIgnoreController"
...
import { convertZulongMessageToProto } from "@shared/proto-conversions/zulong-message"
import { ZulongDefaultTool, READ_ONLY_TOOLS } from "@shared/tools"
import { ZulongAskResponse } from "@shared/WebviewMessage"
import {
...
import { ZulongError, ZulongErrorType, ErrorService } from "@/services/error"
import { telemetryService } from "@/services/telemetry"
import { ZulongClient } from "@/shared/zulong"
import {
	ZulongAssistantContent,
	ZulongContent,
	ZulongImageContentBlock,
	ZulongMessageModelInfo,
	ZulongStorageMessage,
	ZulongTextContentBlock,
	ZulongToolResponseContent,
	ZulongUserContent,
} from "@/shared/messages"
import { ApiFormat } from "@/shared/proto/zulong/models"
import { ShowMessageType } from "@/shared/proto/index.host"
import { Logger } from "@/shared/services/Logger"
import { Session } from "@/shared/services/Session"
import { RuleContextBuilder } from "../context/instructions/user-instructions/RuleContextBuilder"
import { ensureLocalZulongDirExists } from "../context/instructions/user-instructions/rule-helpers"
...
export type ToolResponse = ZulongToolResponseContent
...
export class Task {
	...
	private zulongIgnoreController: ZulongIgnoreController
	...
	constructor(params: TaskParams) {
		...
		this.zulongIgnoreController = new ZulongIgnoreController(cwd)
		...
		const effectiveApiConfiguration: ApiConfiguration = {
			...
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				...
				const lastApiReqStartedIndex = findLastIndex(zulongMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: ZulongApiReqInfo = JSON.parse(zulongMessages[lastApiReqStartedIndex].text || "{}")
						...
						await this.messageStateHandler.updateZulongMessage(lastApiReqStartedIndex, {
		...
		const commandExecutorCallbacks: CommandExecutorCallbacks = {
			...
			ask: async (type: string, text?: string, partial?: boolean) => {
				const result = await this.ask(type as ZulongAsk, text, partial)
				return {
			...
			resolvePendingAsk: (response) => {
				void this.handleWebviewAskResponse(response as ZulongAskResponse)
			},
			updateBackgroundCommandState: (isRunning: boolean) =>
				this.controller.updateBackgroundCommandState(isRunning, this.taskId),
			updateZulongMessage: async (index: number, updates: { commandCompleted?: boolean; text?: string }) => {
				await this.messageStateHandler.updateZulongMessage(index, updates)
			},
			getZulongMessages: () => this.messageStateHandler.getZulongMessages() as Array<{ ask?: string; say?: string }>,
			addToUserMessageContent: (content: { type: string; text: string }) => {
				// Cast to ZulongTextContentBlock which is compatible with ZulongContent
				this.taskState.userMessageContent.push({ type: "text", text: content.text } as ZulongTextContentBlock)
			},
		}
		...
		// before streaming begins (in recursivelyMakeZulongRequests) so the cadence is always
		...
		this.presentationScheduler = new TaskPresentationScheduler({
			...
			getDelayMs: (priority) => {
				if (!this.remoteWorkspaceDetectionSettled) {
					// This should never fire in production because recursivelyMakeZulongRequests
	...
	async ask(
		...
		if (partial !== undefined) {
			const zulongMessages = this.messageStateHandler.getZulongMessages()
			const lastMessage = zulongMessages.at(-1)
			const lastMessageIndex = zulongMessages.length - 1
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.messageStateHandler.updateZulongMessage(lastMessageIndex, {
						text,
						partial,
					})
					// todo be more efficient about saving and posting only new data or one whole message at a time so ignore partial for saves, and only post parts of partial message instead of whole array in new listener
					// await this.saveZulongMessagesAndUpdateHistory()
					// await this.postStateToWebview()
					const protoMessage = convertZulongMessageToProto(lastMessage)
			...
			// const lastMessage = this.zulongMessages.at(-1)
			this.taskState.askResponse = undefined
			this.taskState.askResponseText = undefined
			this.taskState.askResponseImages = undefined
			this.taskState.askResponseFiles = undefined
	...
	async handleWebviewAskResponse(askResponse: ZulongAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async say(
		...
		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getZulongMessages().at(-1)
			const isUpdatingPreviousPartial =
				lastMessage && lastMessage.partial && lastMessage.type === "say" && lastMessage.say === type
			if (partial) {
				if (isUpdatingPreviousPartial) {
					// existing partial message, so update it
					const lastIndex = this.messageStateHandler.getZulongMessages().length - 1
					await this.messageStateHandler.updateZulongMessage(lastIndex, {
						text,
						images,
						files,
						partial,
			...
			if (isUpdatingPreviousPartial) {
				...
				const lastIndex = this.messageStateHandler.getZulongMessages().length - 1
				// updateZulongMessage emits the change event and saves to disk
				await this.messageStateHandler.updateZulongMessage(lastIndex, {
					text,
					images,
					files,
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: ZulongAsk | ZulongSay) {
		const zulongMessages = this.messageStateHandler.getZulongMessages()
		const lastMessage = zulongMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setZulongMessages(zulongMessages.slice(0, -1))
			await this.messageStateHandler.saveZulongMessagesAndUpdateHistory()
		}
	}
	 ...
	 */
	private calculatePreCompactDeletedRange(apiConversationHistory: ZulongStorageMessage[]): [number, number] {
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.zulongIgnoreController.initialize()
		} catch (error) {
			Logger.error("Failed to initialize ZulongIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and zulongMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the zulongMessages might not be empty, so we need to set it to [] when we create a new Zulong client (otherwise webview would show stale messages from previous session)
		this.messageStateHandler.setZulongMessages([])
		...
		const imageBlocks: ZulongImageContentBlock[] = formatResponse.imageBlocks(images)

		const userContent: ZulongUserContent[] = [
			{
				type: "text",
				text: `<task>\n${task}\n</task>`,
			},
	...
	public async resumeTaskFromHistory() {
		...
		const savedZulongMessages = await getSavedZulongMessages(this.taskId)
		...
		if (lastRelevantMessageIndex !== -1) {
			savedZulongMessages.splice(lastRelevantMessageIndex + 1)
		}

		// since we don't use api_req_finished anymore, we need to check if the last api_req_started has a cost value, if it doesn't and no cancellation reason to present, then we remove it since it indicates an api request without any partial content streamed
		const lastApiReqStartedIndex = findLastIndex(savedZulongMessages, (m) => m.type === "say" && m.say === "api_req_started")
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savedZulongMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: ZulongApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				savedZulongMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.messageStateHandler.overwriteZulongMessages(savedZulongMessages)
		this.messageStateHandler.setZulongMessages(await getSavedZulongMessages(this.taskId))

		// Now present the zulong messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		const lastZulongMessage = this.messageStateHandler
			.getZulongMessages()
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks

		let askType: ZulongAsk
		if (lastZulongMessage?.ask === "completion_result") {
		...
		const newUserContent: ZulongContent[] = []
			...
			const taskResumeResult = await executeHook({
				...
				hookInput: {
					taskResume: {
						...
						previousState: {
							lastMessageTs: lastZulongMessage?.ts?.toString() || "",
							messageCount: zulongMessages.length.toString(),
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with zulong messages
		...
		let modifiedOldUserContent: ZulongContent[] // either the last message if its user message, or the user message before the last (assistant) message
		let modifiedApiConversationHistory: ZulongStorageMessage[] // need to remove the last user message to replace with new modified user message
		if (existingApiConversationHistory.length > 0) {
			...
			if (lastMessage.role === "assistant") {
				...
				const existingUserContent: ZulongContent[] = Array.isArray(lastMessage.content)
					? lastMessage.content
		...
		const agoText = (() => {
			const timestamp = lastZulongMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastZulongMessage?.ts && Date.now() - lastZulongMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: ZulongContent[]): Promise<void> {
		let nextUserContent = userContent
		let includeFileDetails = true
		while (!this.taskState.abort) {
			const didEndLoop = await this.recursivelyMakeZulongRequests(nextUserContent, includeFileDetails)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that zulong will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			...
			// this.say(
			// 	"tool",
			// 	"Zulong responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
			// )
			nextUserContent = [
				{
					type: "text",
	...
	async abortTask() {
		try {
			...
			this.zulongIgnoreController.dispose()
	...
	): Promise<[boolean, ZulongToolResponseContent]> {
	...
	public async cancelHookExecution(): Promise<boolean> {
		...
		try {
			...
			const zulongMessages = this.messageStateHandler.getZulongMessages()
			const hookMessageIndex = zulongMessages.findIndex((m) => m.ts === messageTs)
			if (hookMessageIndex !== -1) {
				...
				await this.messageStateHandler.updateZulongMessage(hookMessageIndex, {
	...
	private async writePromptMetadataArtifacts(params: { systemPrompt: string; providerInfo: ApiProviderInfo }): Promise<void> {
		const enabledFlag = process.env.ZULONG_WRITE_PROMPT_ARTIFACTS?.toLowerCase()
		...
		try {
			const configuredDir = process.env.ZULONG_PROMPT_ARTIFACT_DIR?.trim()
			const artifactDir = configuredDir
				? path.isAbsolute(configuredDir)
					? configuredDir
					: path.resolve(this.cwd, configuredDir)
				: path.resolve(this.cwd, ".zulong-prompt-artifacts")
		...
		if (hooksEnabled) {
			try {
				...
				await executePreCompactHookWithCleanup({
					...
					zulongMessages: this.messageStateHandler.getZulongMessages(),
					messageStateHandler: this.messageStateHandler,
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const { globalToggles, localToggles } = await refreshZulongRulesToggles(this.controller, this.cwd)
		...
		const globalRules = await getGlobalZulongRules(globalZulongRulesFilePath, globalToggles, { evaluationContext })
		let globalZulongRulesFileInstructions = globalRules.instructions
		...
		if (lazyTeammateModeEnabled) {
			...
			globalZulongRulesFileInstructions = globalZulongRulesFileInstructions
				? `${globalZulongRulesFileInstructions}\n\n${LAZY_TEAMMATE_RULES}`
				: LAZY_TEAMMATE_RULES
		}

		const localRules = await getLocalZulongRules(this.cwd, localToggles, { evaluationContext })
		const localZulongRulesFileInstructions = localRules.instructions
		...
		const zulongIgnoreContent = this.zulongIgnoreController.zulongIgnoreContent
		let zulongIgnoreInstructions: string | undefined
		if (zulongIgnoreContent) {
			zulongIgnoreInstructions = formatResponse.zulongIgnoreInstructions(zulongIgnoreContent)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			globalZulongRulesFileInstructions,
			localZulongRulesFileInstructions,
			localCursorRulesFileInstructions,
			localCursorRulesDirInstructions,
			localWindsurfRulesFileInstructions,
			localAgentsRulesFileInstructions,
			zulongIgnoreInstructions,
			...
			zulongWebToolsEnabled:
				this.stateManager.getGlobalSettingsKey("zulongWebToolsEnabled") && featureFlagsService.getWebtoolsEnabled(),
			...
			enableNativeToolCalls:
				...
				providerInfo.providerId === "zulong" || // Zulong: backend handles FC natively
		...
		const contextManagementMetadata = await this.contextManager.getNewContextMessagesAndMetadata(
			...
			this.messageStateHandler.getZulongMessages(),
			this.api,
			this.taskState.conversationHistoryDeletedRange,
			previousApiReqIndex,
			await ensureTaskDirectoryExists(this.taskId),
		...
		try {
			...
			const zulongError = ErrorService.get().toZulongError(error, model.id, providerId)

			// Capture provider failure telemetry using zulongError
			ErrorService.get().logMessage(zulongError.message)
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						zulongError.message = "Context window exceeded. Click retry to truncate the conversation and try again."
						this.taskState.didAutomaticallyRetryFailedApiRequest = false
					}
				}

				const streamingFailedMessage = zulongError.serialize()
				...
				if (lastApiReqStartedIndex !== -1) {
					...
					await this.messageStateHandler.updateZulongMessage(lastApiReqStartedIndex, {
						...
						} satisfies ZulongApiReqInfo),
					})
					// this.ask will trigger postStateToWebview, so this change should be picked up.
				}

				const isAuthError = zulongError.isErrorType(ZulongErrorType.Auth)
				const isSpendLimitError = zulongError.isErrorType(ZulongErrorType.SpendLimit)
				const quotaExceeded = zulongError.isErrorType(ZulongErrorType.QuotaExceeded)

				// Check if this is a Zulong provider insufficient credits error - don't auto-retry these
				const isZulongProviderInsufficientCredits = (() => {
					if (providerId !== "zulong") {
						return false
					}
					try {
						const parsedError = ZulongError.transform(error, model.id, providerId)
						return parsedError.isErrorType(ZulongErrorType.Balance)
					} catch {
						return false
					}
				})()

				let response: ZulongAskResponse
				// Skip auto-retry for Zulong provider insufficient credits, auth errors, or spend limit errors
				const shouldRetry =
					!isZulongProviderInsufficientCredits &&
					!isAuthError &&
					!isSpendLimitError &&
					!quotaExceeded &&
				...
				if (shouldRetry) {
					...
					if (autoRetryApiReqIndex !== -1) {
						const zulongMessages = this.messageStateHandler.getZulongMessages()
						const currentApiReqInfo: ZulongApiReqInfo = JSON.parse(zulongMessages[autoRetryApiReqIndex].text || "{}")
						delete currentApiReqInfo.streamingFailedMessage
						await this.messageStateHandler.updateZulongMessage(autoRetryApiReqIndex, {
							text: JSON.stringify(currentApiReqInfo),
						})
					}
					...
					const showRetry = !isZulongProviderInsufficientCredits && !isAuthError && !isSpendLimitError && !quotaExceeded
					if (showRetry) {
						await this.say(
							"error_retry",
							JSON.stringify({
				...
				if (manualRetryApiReqIndex !== -1) {
					const zulongMessages = this.messageStateHandler.getZulongMessages()
					const currentApiReqInfo: ZulongApiReqInfo = JSON.parse(zulongMessages[manualRetryApiReqIndex].text || "{}")
					delete currentApiReqInfo.streamingFailedMessage
					await this.messageStateHandler.updateZulongMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				{
					const prevContentLen = this.taskState.userMessageContent.length
					await this.toolExecutor.executeTool(block)
					if (block.call_id) {
						Session.get().updateToolCall(block.call_id, block.name)
					}
					// Zulong provider: send tool result back to backend via WebSocket
					if (this.api.sendToolResult && block.call_id) {
						const newItems = this.taskState.userMessageContent.slice(prevContentLen)
						const resultText = newItems
							.map((item) => {
								if ("text" in item && typeof item.text === "string") return item.text
								if ("content" in item) return typeof item.content === "string" ? item.content : JSON.stringify(item.content)
								return ""
							})
							.filter(Boolean)
							.join("\n")
						this.api.sendToolResult(block.call_id, block.name, resultText || "[Tool completed with no output]")
					}
				}
				break
		}
	...
	async recursivelyMakeZulongRequests(userContent: ZulongContent[], includeFileDetails = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Zulong is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images, files } = await this.ask(
				"mistake_limit_reached",
				this.api.getModel().id.includes("claude")
					? `This may indicate a failure in Zulong's thought process or inability to use a tool properly, which can be mitigated with some user guidance (e.g. "Try breaking down the task into smaller steps").`
					: "Zulong uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4.5 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				...
				const feedbackUserContent: ZulongUserContent[] = []
		...
		const previousApiReqIndex = findLastIndex(this.messageStateHandler.getZulongMessages(), (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.messageStateHandler.getZulongMessages().filter((m) => m.say === "api_req_started").length === 0
		...
		if (
			...
			try {
				...
				this.taskState.checkpointManagerErrorMessage = errorMessage // will be displayed right away since we saveZulongMessages next which posts state to webview
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
			...
			if (lastCheckpointMessageIndex !== -1) {
				...
				commitPromise
					?.then(async (commitHash) => {
						if (commitHash) {
							await this.messageStateHandler.updateZulongMessage(lastCheckpointMessageIndex, {
								lastCheckpointHash: commitHash,
							})
							// saveZulongMessagesAndUpdateHistory will be called later after API response,
		...
		if (shouldCompact) {
			...
			;[parsedUserContent, environmentDetails, zulongrulesError] = await this.loadContext(
				userContent,
				includeFileDetails,
				useCompactPrompt,
			)
		...
		// error handling if the user uses the /newrule command & their .zulongrules is a file, for file read operations didnt work properly
		if (zulongrulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.zulongrules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getZulongMessages(), (m) => m.say === "api_req_started")
		await this.messageStateHandler.updateZulongMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies ZulongApiReqInfo),
		})
		...
		try {
			...
			*/
			...
			const updateApiReqMsgFromMetrics = async (
				cancelReason?: ZulongApiReqCancelReason,
			...
			const finalizeApiReqMsg = async (cancelReason?: ZulongApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			const abortStream = async (cancelReason: ZulongApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			const finalizePendingReasoningMessage = async (thinking: string): Promise<boolean> => {
				...
				await this.messageStateHandler.updateZulongMessage(pendingReasoningIndex, {
					text: thinking,
					partial: false,
				})
				const completedReasoning = this.messageStateHandler.getZulongMessages()[pendingReasoningIndex]
				if (completedReasoning) {
					await sendPartialMessageEvent(convertZulongMessageToProto(completedReasoning))
				}
				return true
			}
			...
			try {
				...
				while (true) {
					...
					switch (chunk.type) {
						...
						case "tool_calls": {
							...
							// Zulong provider: when isComplete is set, the backend is waiting for
							// tool results via WebSocket. Mark blocks as non-partial so they execute
							// immediately during streaming (instead of waiting for stream end).
							if (chunk.isComplete && this.api.sendToolResult) {
								for (const block of this.taskState.assistantMessageContent) {
									if (block.type === "tool_use" && block.partial) {
										block.partial = false
									}
								}
								toolUseHandler.reset()
							}
							...
							// Zulong provider: await flush to ensure tools execute and send results
							// back to backend BEFORE we try to read the next stream chunk.
							if (chunk.isComplete && this.api.sendToolResult) {
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of zulong)
				...
				await streamCoordinator?.stop()
				// abandoned happens when extension is no longer waiting for the zulong instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
					const zulongError = ErrorService.get().toZulongError(error, this.api.getModel().id)
					const errorMessage = zulongError.serialize()
					const isStreamingSpendLimitError = zulongError.isErrorType(ZulongErrorType.SpendLimit)
			...
			// OpenRouter/Zulong may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantHasContent) {
				...
				const assistantContent: Array<ZulongAssistantContent> = [
				...
				if (hasAssistantText) {
					assistantContent.push({
						...
						// reasoning_details only exists for zulong/openrouter providers
			...
			if (assistantHasContent) {
				...
				// Zulong provider: entire multi-turn conversation runs in one stream.
				// Tools already executed and results sent back during streaming.
				// Don't recurse — the task is complete when stream ends.
				if (this.api.sendToolResult) {
					didEndLoop = true
				} else {
					const recDidEndLoop = await this.recursivelyMakeZulongRequests(this.taskState.userMessageContent)
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that Zulong cannot process. Retrying the request may help resolve this issue."
	...
	async loadContext(
	...
	): Promise<[ZulongContent[], string, boolean]> {
		let needsZulongrulesFileCheck = false
		...
		const parseTextBlock = async (text: string): Promise<string> => {
			...
			const { processedText, needsZulongrulesFileCheck: needsCheck } = await parseSlashCommands(
				parsedText,
			...
			if (needsCheck) {
				needsZulongrulesFileCheck = true
			}

			return processedText
		}

		const processTextContent = async (block: ZulongTextContentBlock): Promise<ZulongTextContentBlock> => {
		...
		const processContentBlock = async (block: ZulongContent): Promise<ZulongContent> => {
			if (block.type === "text") {
		...
		// Check zulongrulesData if needed
		const zulongrulesError = needsZulongrulesFileCheck
			? await ensureLocalZulongDirExists(this.cwd, GlobalFileNames.zulongRules)
			: false
		...
		return [processedUserContent, environmentDetails, zulongrulesError]
	}
	...
	protected async processNativeToolCalls(assistantTextOnly: string, toolBlocks: ToolUse[]) {
		...
		// IMPORTANT: Finalize any partial text ZulongMessage before we skip over it.
		//
		...
		//
		// Without this fix, the partial text ZulongMessage remains with partial=true. In the UI
	...
	async getEnvironmentDetails(includeFileDetails = false) {
		...
		// It could be useful for zulong to know if the user went from one or no file to another between messages, so we always include this context
		...
		// Filter paths through zulongIgnoreController
		const allowedVisibleFiles = this.zulongIgnoreController
		...
		// Filter paths through zulongIgnoreController
		const allowedOpenTabs = this.zulongIgnoreController
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.zulongIgnoreController)
				details += result
			}
		...
		const getTotalTokensFromApiReqMessage = (msg: ZulongMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const zulongMessages = this.messageStateHandler.getZulongMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(zulongMessages.slice(1)))
