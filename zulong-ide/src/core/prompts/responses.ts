...
import { ZulongIgnoreController, LOCK_TEXT_SYMBOL } from "../ignore/ZulongIgnoreController"
...
export const formatResponse = {
	...
	zulongIgnoreError: (path: string) =>
		`Access to ${path} is blocked by the .zulongignore file settings. You must try to continue in the task without using this file, or ask the user to update the .zulongignore file.`,

	permissionDeniedError: (reason: string) =>
		`Command execution blocked by ZULONG_COMMAND_PERMISSIONS: ${reason}. You must try a different approach or ask the user to update the permission settings.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		files: string[],
		didHitLimit: boolean,
		zulongIgnoreController?: ZulongIgnoreController,
	): string => {
		const sorted = files
			.map((file) => {
			...
			// Sort so files are listed under their respective directories to make it clear what files are children of what directories. Since we build file list top down, even if file list is truncated it will show directories that zulong can then explore further.
			.sort((a, b) => {
		...
		const zulongIgnoreParsed = zulongIgnoreController
			? sorted.map((filePath) => {
					// path is relative to absolute path, not cwd
					// validateAccess expects either path relative to cwd or absolute path
					// otherwise, for validating against ignore patterns like "assets/icons", we would end up with just "icons", which would result in the path not being ignored.
					const absoluteFilePath = path.resolve(absolutePath, filePath)
					const isIgnored = !zulongIgnoreController.validateAccess(absoluteFilePath)
					if (isIgnored) {
		...
		if (didHitLimit) {
			return `${zulongIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		}
		if (zulongIgnoreParsed.length === 0 || (zulongIgnoreParsed.length === 1 && zulongIgnoreParsed[0] === "")) {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	zulongIgnoreInstructions: (content: string) =>
		`# .zulongignore\n\n(The following is provided by a root-level .zulongignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.zulongignore`,

	zulongRulesGlobalDirectoryInstructions: (globalZulongRulesFilePath: string, content: string) =>
		`# .zulongrules/\n\nThe following is provided by a global .zulongrules/ directory, located at ${globalZulongRulesFilePath.toPosix()}, where the user has specified instructions for all working directories:\n\n${content}`,

	zulongRulesLocalDirectoryInstructions: (cwd: string, content: string) =>
		`# .zulongrules/\n\nThe following is provided by a root-level .zulongrules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,

	zulongRulesLocalFileInstructions: (cwd: string, content: string) =>
		`# .zulongrules\n\nThe following is provided by a root-level .zulongrules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,
