...
export interface ZulongToolSpec {
	variant: ModelFamily
	id: ZulongDefaultTool
	name: string
	description: string
	instruction?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
	parameters?: Array<ZulongToolSpecParameter>
}

interface ZulongToolSpecParameter {
	name: string
	required: boolean
	instruction: string | ((context: SystemPromptContext) => string)
	usage?: string
	dependencies?: ZulongDefaultTool[]
	description?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
...
/**
 * Converts a ZulongToolSpec into an OpenAI ChatCompletionTool definition
 * Docs: https://openrouter.ai/docs/features/tool-calling#step-1-inference-request-with-tools
 */
export function toolSpecFunctionDefinition(tool: ZulongToolSpec, context: SystemPromptContext): OpenAITool {
...
/**
 * Converts a ZulongToolSpec into an Anthropic Tool definition
 */
export function toolSpecInputSchema(tool: ZulongToolSpec, context: SystemPromptContext): AnthropicTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
...
/**
 * Converts a ZulongToolSpec into a Google Gemini function.
 * Docs: https://ai.google.dev/gemini-api/docs/function-calling
 */
export function toolSpecFunctionDeclarations(tool: ZulongToolSpec, context: SystemPromptContext): GoogleTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
