...
This directory contains the tool registration system for Zulong tools. The system automatically collects and registers all tool variants with the `ZulongToolSet` provider.

## Overview

Each tool file in this directory exports a `{toolName}_variants` array containing tool specifications for different prompt variants (e.g., Claude, GPT). The registration system automatically imports all these variants and registers them with the `ZulongToolSet` provider.
...
const claudeTools = ZulongToolSet.getTools(PromptVariant.CLAUDE);

// Get a specific tool by name
const writeToFileTool = ZulongToolSet.getToolByName("write_to_file", PromptVariant.CLAUDE);
```
...
```typescript
import { ZulongDefaultTool, PromptVariant, type ZulongToolSpec } from "@/shared/tools";

const claude: ZulongToolSpec = {
