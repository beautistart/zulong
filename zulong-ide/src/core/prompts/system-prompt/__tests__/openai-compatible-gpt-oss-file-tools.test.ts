...
describe("OpenAI-compatible gpt-oss native tools smoke test", () => {
	...
	it("exposes apply_patch for gpt-oss-120b so file editing remains native", async () => {
		...
		expect(toolNames).to.include(ZulongDefaultTool.BASH)
		expect(toolNames).to.include(ZulongDefaultTool.FILE_READ)
		expect(toolNames).to.include(ZulongDefaultTool.APPLY_PATCH)
		expect(toolNames).to.not.include(ZulongDefaultTool.FILE_NEW)
		expect(toolNames).to.not.include(ZulongDefaultTool.FILE_EDIT)
	})
