...
describe("new_task tool contextRequirements", () => {
	...
	it("should follow the same pattern as ask_followup_question", () => {
		const newTaskTool = ZulongToolSet.getToolByNameWithFallback(ZulongDefaultTool.NEW_TASK, ModelFamily.GENERIC)
		const askTool = ZulongToolSet.getToolByNameWithFallback(ZulongDefaultTool.ASK, ModelFamily.GENERIC)
