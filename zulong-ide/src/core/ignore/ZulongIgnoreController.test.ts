...
import { ZulongIgnoreController } from "./ZulongIgnoreController"
import "should"

describe("ZulongIgnoreController", () => {
	let tempDir: string
	let controller: ZulongIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .zulongignore file
		await fs.writeFile(
			path.join(tempDir, ".zulongignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new ZulongIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .zulongignore file", async () => {
			const result = controller.validateAccess(".zulongignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .zulongignore", async () => {
			// Create a new .zulongignore with comments
			await fs.writeFile(
				path.join(tempDir, ".zulongignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .zulongignore gracefully", async () => {
			// Create a new controller in a directory without .zulongignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new ZulongIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .zulongignore", async () => {
			await fs.writeFile(path.join(tempDir, ".zulongignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .zulongignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".zulongignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .zulongignore
			controller = new ZulongIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .zulongignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .zulongignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".zulongignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".zulongignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
