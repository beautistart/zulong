 ...
 * Designed to be instantiated once in Zulong.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .zulongignore files.
 */
export class ZulongIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	zulongIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.zulongIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .zulongignore
		this.setupFileWatcher()
		await this.loadZulongIgnore()
	}

	/**
	 * Set up the file watcher for .zulongignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".zulongignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .zulongignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .zulongignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadZulongIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.zulongIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".zulongignore")
			} else {
				this.zulongIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .zulongignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processZulongIgnoreIncludes(content)
	 ...
	 */
	private async processZulongIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[ZulongIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .zulongignore does not exist
		if (!this.zulongIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .zulongignore exists
		if (!this.zulongIgnoreContent) {
			return undefined
		}
