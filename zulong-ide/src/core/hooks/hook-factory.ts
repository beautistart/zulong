...
import { version as zulongVersion } from "../../../package.json"
...
} from "../../shared/proto/zulong/hooks"
 ...
 * provide--the other common parameters like zulongVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - zulongVersion: Current Zulong extension version
	 ...
	 * - userId: Zulong user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: Zulong User ID, machine ID, or generated UUID
			...params,
			model,
		}
	}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/Zulong/Hooks/)
 * and workspace-specific hooks (from each workspace's .zulongrules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .zulongrules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "Zulong/Hooks" or "zulong/hooks".
	 */
	 ...
	 *
	 * - Global hooks (from ~/Documents/Zulong/Hooks/): run from the primary workspace root
	 * - Workspace hooks (from workspaceRoot/.zulongrules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.zulongrules/hooks/
	 ...
	 * Global hooks are located in ~/Documents/Zulong/Hooks/
	 * Workspace hooks are located in workspace .zulongrules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/Zulong/Hooks/) and workspace hooks
	 * (from .zulongrules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .zulongrules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .zulongrules directory path to search
