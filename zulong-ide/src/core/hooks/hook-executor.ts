...
import type { HookOutput } from "@shared/proto/zulong/hooks"
...
async function updateHookMessage(
	...
	const zulongMessages = messageStateHandler.getZulongMessages()
	const hookMessageIndex = zulongMessages.findIndex((m: ZulongMessage) => m.ts === hookMessageTs)
	if (hookMessageIndex !== -1) {
		await messageStateHandler.updateZulongMessage(hookMessageIndex, {
...
async function reorderHookAndToolMessages(messageStateHandler: MessageStateHandler): Promise<void> {
	...
	for (let i = zulongMessages.length - 1; i >= 0; i--) {
		const msgType = zulongMessages[i].ask || zulongMessages[i].say
	...
	for (let i = lastToolMessageIndex + 1; i < zulongMessages.length; i++) {
		if (zulongMessages[i].say === "hook_status" || zulongMessages[i].say === "hook_output_stream") {
	...
	const toolMessage = { ...zulongMessages[lastToolMessageIndex] }

	// Delete the tool message at its current position
	await messageStateHandler.deleteZulongMessage(lastToolMessageIndex)

	// Re-add the tool message at the end (after hook messages)
	await messageStateHandler.addToZulongMessages(toolMessage)
}
