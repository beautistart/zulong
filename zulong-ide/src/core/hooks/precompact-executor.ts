...
import type { ZulongStorageMessage } from "@shared/messages/content"
 ...
 */
export function extractTokenUsageFromMessage(message: ZulongMessage | undefined): TokenUsage {
...
export async function writePreCompactContextFiles(
	taskId: string,
	currentContext: ZulongStorageMessage[],
...
export interface PreCompactHookParams {
	...
	apiConversationHistory: ZulongStorageMessage[]
	/** Current deleted range (if any) */
	conversationHistoryDeletedRange?: [number, number]
	/** Zulong messages for extracting token usage */
...
export async function executePreCompactHookWithCleanup(params: PreCompactHookParams): Promise<PreCompactHookResult> {
	...
	try {
		...
		const previousApiReqIndex = findLastIndex(params.zulongMessages, (m) => m.say === "api_req_started")
		const previousRequest = previousApiReqIndex !== -1 ? params.zulongMessages[previousApiReqIndex] : undefined
		...
		if (preCompactResult.cancel === true) {
			...
			await params.messageStateHandler.saveZulongMessagesAndUpdateHistory()
