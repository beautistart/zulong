...
/**
 * All valid hook types that can be created and executed by Zulong.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Zulong", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".zulongrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".zulongrules", "hooks")
}
