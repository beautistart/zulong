...
export async function openFocusChainFile(controller: Controller, request: StringRequest): Promise<Empty> {
	...
	if (currentTask) {
		...
		// TODO - can we decouple this from ZulongMessages?
		const zulongMessages = currentTask.messageStateHandler.getZulongMessages()
		const lastProgressMessage = zulongMessages
			.slice()
			.reverse()
