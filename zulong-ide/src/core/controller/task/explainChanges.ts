...
import { Empty } from "@shared/proto/zulong/common"
import { ExplainChangesRequest } from "@shared/proto/zulong/task"
...
export async function explainChanges(controller: Controller, request: ExplainChangesRequest): Promise<Empty> {
	...
	try {
		...
		const messageIndex = zulongMessages.findIndex((m: any) => m.ts === request.messageTs)
		const message = zulongMessages[messageIndex]
		...
		const lastTaskCompletedMessageCheckpointHash = findLast(
			zulongMessages.slice(0, messageIndex),
			(m: any) => m.say === "completion_result",
		)?.lastCheckpointHash

		const firstCheckpointMessageCheckpointHash = zulongMessages.find(
