import { Empty, EmptyRequest } from "@shared/proto/zulong/common"
import { OpenRouterCompatibleModelInfo } from "@shared/proto/zulong/models"
...
import { refreshZulongModels } from "../models/refreshZulongModels"
...
export async function initializeWebview(controller: Controller, _request: EmptyRequest): Promise<Empty> {
	try {
		...
		refreshZulongModels(controller).then(async (models) => {
			if (models && Object.keys(models).length > 0) {
				// Update model info in state for Zulong (this needs to be done here since we don't want to update state while settings is open, and we may refresh models there)
				const apiConfiguration = controller.stateManager.getApiConfiguration()
				const planActSeparateModelsSetting = controller.stateManager.getGlobalSettingsKey("planActSeparateModelsSetting")
				const currentMode = controller.stateManager.getGlobalSettingsKey("mode")
				...
				if (planActSeparateModelsSetting) {
					// Separate models: update only current mode
					const modelIdField = currentMode === "plan" ? "planModeZulongModelId" : "actModeZulongModelId"
					const modelInfoField = currentMode === "plan" ? "planModeZulongModelInfo" : "actModeZulongModelInfo"
					const modelId = apiConfiguration[modelIdField]
					...
					// Shared models: update both plan and act modes
					const planModelId = apiConfiguration.planModeZulongModelId
					const actModelId = apiConfiguration.actModeZulongModelId
					const updates: Partial<GlobalStateAndSettings> = {}

					// Update plan mode model info if we have a model ID
					if (planModelId && models[planModelId]) {
						updates.planModeZulongModelInfo = models[planModelId]
					}

					// Update act mode model info if we have a model ID
					if (actModelId && models[actModelId]) {
						updates.actModeZulongModelInfo = models[actModelId]
					}
