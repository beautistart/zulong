...
import { getZulongOnboardingModels } from "./models/getZulongOnboardingModels"
import { appendZulongStealthModels } from "./models/refreshOpenRouterModels"
...
export class Controller {
	...
	mcpHub: McpHub
	accountService: ZulongAccountService
	...
	constructor(readonly context: ZulongExtensionContext) {
		...
		this.accountService = ZulongAccountService.getInstance()
		...
		Logger.log("[Controller] ZulongProvider instantiated")
	}
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			HostProvider.window.showMessage({
				...
				message: "Successfully logged out of Zulong",
			})
		} catch (_error) {
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async cancelTask() {
		...
		try {
			...
			if (this.task) {
				// 'abandoned' will prevent this zulong instance from affecting future zulong instance gui. this may happen if its hanging on a streaming request
	...
	async handleAuthCallback(customToken: string, provider: string | null = null) {
		try {
			...
			if (planActSeparateModelsSetting) {
				...
				if (currentMode === "plan") {
					updatedConfig.planModeApiProvider = zulongProvider
				} else {
					updatedConfig.actModeApiProvider = zulongProvider
				}
			} else {
			...
			HostProvider.window.showMessage({
				...
				message: "Failed to log in to Zulong",
			})
	...
	private async fetchMcpMarketplaceFromApi(): Promise<McpMarketplaceCatalog> {
		const response = await axios.get(`${ZulongEnv.config().mcpBaseUrl}/marketplace`, {
			headers: {
				"Content-Type": "application/json",
				"User-Agent": "zulong-vscode-extension",
			},
			...getAxiosSettings(),
		})
	...
	async readOpenRouterModels(): Promise<Record<string, ModelInfo> | undefined> {
		...
		try {
			if (await fileExistsAtPath(openRouterModelsFilePath)) {
				...
				return appendZulongStealthModels(models)
			}
		} catch (error) {
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const onboardingModels = getZulongOnboardingModels()
		const apiConfiguration = this.stateManager.getApiConfiguration()
		...
		const globalZulongRulesToggles = this.stateManager.getGlobalSettingsKey("globalZulongRulesToggles")
		...
		const localZulongRulesToggles = this.stateManager.getWorkspaceStateKey("localZulongRulesToggles")
		...
		const zulongMessages = [...(this.task?.messageStateHandler.getZulongMessages() || [])]
		...
		const zulongConfig = ZulongEnv.config()
		const environment = zulongConfig.environment
		...
		return {
			...
			environment,
			distinctId,
			globalZulongRulesToggles: globalZulongRulesToggles || {},
			localZulongRulesToggles: localZulongRulesToggles || {},
			...
			zulongWebToolsEnabled: {
				user: this.stateManager.getGlobalSettingsKey("zulongWebToolsEnabled"),
	...
	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of zulong messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between ZulongProvider instances since there could be multiple instances of the extension running at once. For example when we cached zulong messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
	- Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

	We need to use a unique identifier for each ZulongProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
