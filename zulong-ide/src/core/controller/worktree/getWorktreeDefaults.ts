...
import { WorktreeDefaults } from "@shared/proto/zulong/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/zulong-${suffix}`

	// Generate suggested path in Documents/Zulong/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "Zulong", "Worktrees", `${projectName}-${suffix}`)
