...
import { TestConnectionResult } from "@shared/proto/zulong/state"
import { Logger } from "@/shared/services/Logger"
import { blobStorage } from "@/shared/storage/ZulongBlobStorage"
...
export async function testPromptUploading(_controller: Controller, _: EmptyRequest): Promise<TestConnectionResult> {
	try {
		...
		const testKey = `zulong-test-${Date.now()}.json`
