...
export function transformRemoteConfigToStateShape(remoteConfig: RemoteConfig): Partial<RemoteConfigFields> {
	...
	const zulongSettings = remoteConfig.providerSettings?.Zulong
	if (zulongSettings) {
		transformed.planModeApiProvider = "zulong"
		transformed.actModeApiProvider = "zulong"
		providers.push("zulong")
	}
...
export function clearRemoteConfig() {
	try {
		...
		// the remote config zulong rules toggle state is stored in global state
