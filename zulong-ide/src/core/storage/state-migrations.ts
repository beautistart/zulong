...
export async function migrateMcpMarketplaceEnableSetting(mcpMarketplaceEnabledRaw: boolean | undefined): Promise<boolean> {
	const config = vscode.workspace.getConfiguration("zulong")
...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global Zulong rules...")

			// Create global .zulongrules directory if it doesn't exist
			...
			Logger.log("Successfully migrated custom instructions to global Zulong rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const zulongAccountId = await context.secrets.get("zulongAccountId")
...
export async function cleanupOldApiKey(context: vscode.ExtensionContext) {
	try {
		...
		await context.secrets.delete("zulongApiKey")
	} catch (error) {
		Logger.error("Failed to cleanup old zulongApiKey", error)
	}
}
