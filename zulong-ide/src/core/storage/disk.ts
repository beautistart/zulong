...
export const GlobalFileNames = {
	...
	zulongRecommendedModels: "zulong_recommended_models.json",
	zulongModels: "zulong_models.json",
	openRouterModels: "openrouter_models.json",
	vercelAiGatewayModels: "vercel_ai_gateway_models.json",
	groqModels: "groq_models.json",
	basetenModels: "baseten_models.json",
	hicapModels: "hicap_models.json",
	mcpSettings: "zulong_mcp_settings.json",
	zulongRules: ".zulongrules",
	workflows: ".zulongrules/workflows",
	hooksDir: ".zulongrules/hooks",
	zulongruleSkillsDir: ".zulongrules/skills",
	zulongSkillsDir: ".zulong/skills",
...
/**
 * Returns the cross-platform path to the Zulong home directory (~/.zulong).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.zulong
 * - Linux: /home/username/.zulong
 * - Windows: C:\Users\username\.zulong
 *
 * This is intended to eventually replace ~/Documents/Zulong as the global config location.
 */
export function getZulongHomePath(): string {
	return path.join(os.homedir(), ".zulong")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const zulongRulesDir = path.join(userDocumentsPath, "Zulong", "Rules")
	try {
		await fs.mkdir(zulongRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Zulong", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return zulongRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const zulongWorkflowsDir = path.join(userDocumentsPath, "Zulong", "Workflows")
	try {
		await fs.mkdir(zulongWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Zulong", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return zulongWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Zulong", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Zulong", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const zulongHooksDir = path.join(userDocumentsPath, "Zulong", "Hooks")
	try {
		await fs.mkdir(zulongHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Zulong", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return zulongHooksDir
}

/**
 * Returns the global skills directory path (~/.zulong/skills) without creating it.
 */
function getZulongSkillsDirectoryPath(): string {
	return path.join(getZulongHomePath(), "skills")
}
...
export function getSkillsDirectoriesForScan(cwd: string): SkillsScanDirectory[] {
	return [
		{ path: path.join(cwd, GlobalFileNames.zulongruleSkillsDir), source: "project" },
		{ path: path.join(cwd, GlobalFileNames.zulongSkillsDir), source: "project" },
		...
		{ path: getZulongSkillsDirectoryPath(), source: "global" },
...
export async function getSavedZulongMessages(taskId: string): Promise<ZulongMessage[]> {
...
export async function saveZulongMessages(taskId: string, uiMessages: ZulongMessage[]) {
	try {
...
export async function collectEnvironmentMetadata(): Promise<Omit<EnvironmentMetadataEntry, "ts">> {
	try {
		...
		return {
			...
			zulong_version: ExtensionRegistryInfo.version,
		}
	} catch (error) {
 ...
 * 3. Each workspace root's .zulongrules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .zulongrules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .zulongrules/hooks folder in this workspace root.
