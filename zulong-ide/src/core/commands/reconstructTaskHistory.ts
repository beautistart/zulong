import { getSavedZulongMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const zulongMessages = await getSavedZulongMessages(taskId)
		if (zulongMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(zulongMessages, metadata)
...
function extractTaskInformation(zulongMessages: ZulongMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = zulongMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = zulongMessages.length > 0 ? zulongMessages[0].ts : Date.now()
	...
	const apiReqMessages = zulongMessages.filter(
	...
	const messageSize = JSON.stringify(zulongMessages).length
