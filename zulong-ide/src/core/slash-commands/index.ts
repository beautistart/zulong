...
export async function parseSlashCommands(
...
): Promise<{ processedText: string; needsZulongrulesFileCheck: boolean }> {
	...
	for (const { regex } of tagPatterns) {
		...
		if (tagMatch) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsZulongrulesFileCheck: commandName === "newrule" }
			}
				...
				if (parts.length >= 3) {
					...
					try {
						...
						if (promptResponse) {
							...
							return { processedText, needsZulongrulesFileCheck: false }
						}
	...
	return { processedText: text, needsZulongrulesFileCheck: false }
}
