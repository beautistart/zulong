...
test.describe("zulong interactive basics", () => {
	test.use({
	...
	test("opens /settings and navigates tabs with left/right arrows", async ({ terminal }) => {
		...
		// Account tab
		terminal.keyRight()
		await expectVisible(terminal, ["Sign in to access Zulong features", "Sign in with Zulong"])
		...
		terminal.keyLeft()
		await expectVisible(terminal, "Sign in to access Zulong features")
