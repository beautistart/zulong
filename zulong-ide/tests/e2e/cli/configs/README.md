...
Each subdirectory here is a **Zulong config directory** used by one or more test suites.
Tests point to a config via `zulongEnv("<name>")` in `tests/e2e/cli/helpers/env.ts`.
...
2. Create `configs/<name>/data/settings/zulong_mcp_settings.json` (can be `{ "mcpServers": {} }`).
3. Reference it in tests with `zulongEnv("<name>")`.
