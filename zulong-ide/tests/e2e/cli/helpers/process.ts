...
//
// These helpers spawn zulong as a child process and return stdout, stderr,
// and exit code — without going through the TUI harness. Use these for:
//   - Exit code assertions
//   - Pure stdout/stderr contract tests
//   - Piped stdin tests (echo "..." | zulong -y ...)
//   - JSON output validation
//   - Timeout behavior
// ---------------------------------------------------------------------------

import { SpawnSyncOptions, spawnSync } from "child_process"
import { ZULONG_BIN } from "./constants.js"
import { zulongEnv } from "./env.js"
...
/**
 * Run `zulong [args]` synchronously and return stdout/stderr/exitCode.
 *
 * Suitable for deterministic, fast-exiting commands like:
 *   zulong --help, zulong --version, zulong auth -p ... -k ..., etc.
 */
export function runZulong(args: string[], opts: RunOptions = {}): RunResult {
	...
	const spawnOpts: SpawnSyncOptions = {
		...
		timeout,
		cwd,
		env: { ...zulongEnv(config), ...extraEnv },
		input: stdin,
	}

	const result = spawnSync(ZULONG_BIN, args, spawnOpts)
...
/**
 * Assert that a runZulong result exited with the expected code.
