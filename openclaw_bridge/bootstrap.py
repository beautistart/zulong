...
class OpenClawBridge:
    ...
    def __init__(self, config: Optional[BridgeConfig] = None):
        ...
        if self.config.web_enabled:
            ...
            # 订阅 L2_THINKING_STEP 事件（用于推理过程实时推送到前端）
            self.event_bus.subscribe(
                OpenClawEventType.L2_THINKING_STEP,
                self.web_response_listener.on_thinking_step_event,
                "WebResponseListener"
            )
            
            # 订阅 L2_OUTPUT_STREAM 事件（用于流式输出实时推送到前端）
            self.event_bus.subscribe(
                OpenClawEventType.L2_OUTPUT_STREAM,
                self.web_response_listener.on_stream_event,
                "WebResponseListener"
            )
            
            # 订阅 MEMORY_GRAPH_UPDATED 事件（用于记忆图谱可视化推送到前端）
            self.event_bus.subscribe(
                OpenClawEventType.MEMORY_GRAPH_UPDATED,
                self.web_response_listener.on_memory_graph_update_event,
                "WebResponseListener"
            )
            
            logger.info("✅ Web 适配器已初始化（已订阅 ACTION_SPEAK + L2_OUTPUT + L2_OUTPUT_STREAM + L2_THINKING_STEP + MEMORY_GRAPH_UPDATED）")
        else:
