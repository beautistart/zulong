...
class OpenClawAPIServer:
    ...
    async def _perform_search(self, query: str, count: int) -> List[Dict[str, Any]]:
        """
        执行搜索（优先使用 SearXNG，降级到 Puppeteer）
        ...
        # 方案 1：使用 SearXNG（推荐，无需 API Key，无人机验证）
        try:
            results = await self._search_with_searxng(query, count)
            if results:
                logger.info(f"[OpenClawAPIServer] SearXNG 搜索成功：{len(results)} 个结果")
                return results
        except Exception as e:
            logger.warning(f"[OpenClawAPIServer] SearXNG 失败：{e}，降级到 Puppeteer")
    ...
    async def _search_with_puppeteer(self, query: str, count: int) -> List[Dict[str, Any]]:
        """
        使用 Puppeteer 进行搜索（降级方案）
